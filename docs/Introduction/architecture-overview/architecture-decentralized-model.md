...
In order for an update to take place, the price feed contract must receive responses from a minimum number of oracles or the latest answer will not be updated. Fore the ETH/USD Price Feed, you can see the minimum number of oracles that must respond at [data.chain.link](https://data.chain.link/ethereum/mainnet/crypto-usd/eth-usd).
