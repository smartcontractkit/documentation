...
Let's use these elements to create a Keepers-compatible contract that will automatically increment a counter after every `updateInterval` seconds. Once the contract is registered as an Upkeep, the Keepers Network will simulate our `checkUpkeep` off-chain during every block to determine if `updateInterval` seconds have passed since the last increment (timestamp). Once `checkUpkeep` returns true, the Keepers Network will call `performUpkeep` on-chain and increment the counter. This cycle will repeat until the Upkeep is cancelled or runs out of funding.
...
> ❗️ Revalidate **`performUpkeep`**
>
> We recommend that you [revalidate](#revalidate-performupkeep) conditions for your Upkeep at the start of your `performUpkeep` function.
>
...
- **Trigger ONLY when data is verified**: Some actions must be performed using data you intend to use. Revalidate that the `performData` is allowed before execution.
