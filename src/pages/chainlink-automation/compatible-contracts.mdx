...
<Aside type="tip" title="Considerations and Best Practice">

We recommend reading about the Chainlink Automation [best practices](/chainlink-automation/compatible-contract-best-practice) before deploying your contract. You can also read more about the Chainlink Automation architecture [here](/chainlink-automation/overview).
...
Ensure that your `performUpkeep` is _idempotent_. Your `performUpkeep` function should change state such that `checkUpkeep` will not return `true` for the same subset of work once said work is complete. Otherwise the Upkeep will remain eligible and result in multiple performances by the Chainlink Automation Network on the exactly same subset of work. As a best practice, always [revalidate](/chainlink-automation/compatible-contract-best-practice#revalidate-performupkeep) conditions for your upkeep at the start of your `performUpkeep` function.
