...
When working with Chainlink on non-Ethereum chains, you must export your LINK tokens from Ethereum to the target chain via a cross-chain bridge. Follow this [video](https://www.youtube.com/watch?v=WKvIGkBWRUA) for an example of moving LINK tokens from Ethereum to Polygon.

Cross-chain bridges come with their own risks. In fact, bridge attacks constitute some of the [largest cryptocurrency hacks](https://rekt.news/leaderboard/) by value. When moving your LINK tokens or any asset across chains, understand the risks you are taking with your assets. Chainlink Labs does not endorse any bridge. Ultimately, you are responsible for assessing the bridge you will use to move your assets.

Read [What is a Cross-chain bridge](#what-is-a-cross-chain-bridge), [Trade-offs](#trade-offs), and [Risks](#risks) sections to learn more about bridges and trust assumptions in their designs. After you read these sections, we hope that you will have a better understanding of bridge risks and which aspects you should evaluate when using a bridge.
...
However, blockchains are not natively capable of communicating with each other, making [blockchain interoperability protocols](https://blog.chain.link/blockchain-interoperability/) critical for allowing dApps to interact with any on-chain environment and tap into each blockchain's unique assets and features.
...
- dApps to leverage the strengths and benefits of different chains.
...
With the growing number of layer-1 & layer-2 chains, the number of bridges has also grown, [surpassing one hundred](https://debridges.com/). So, how do you choose the correct bridge?
...
When choosing a bridge, users should be aware of the following risks:
...
Some externally verified bridges are secured by multisig wallets (e.g., [Ronin](https://whitepaper.axieinfinity.com/technology/ronin-ethereum-sidechain)). They are also referred to as m-of-n multisigs, with M being the required number of signatures/keys and N being the total number of signatures/keys (m≤n). Meaning that an attacker only needs to exploit M keys to be able to hack the whole system. In this case, users must trust that the third party is decentralized enough, signers are independent of others, and that each signer has proper key management in place. Read this [detailed analysis](https://rekt.news/ronin-rekt/) for an example of a bridge exploit where the attacker could compromise M keys.
:::
