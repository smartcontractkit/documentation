...
<Aside type="danger" title="">
  ...
  tasks](/chainlink-nodes/oracle-jobs/task-types/tasks/) instead.
</Aside>
...
- The [Task list](#task-spec): The `tasks` that specify all of the computation steps to perform when executing a Job Spec. The Task list is sometimes referred to as the [Job Pipeline](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#writing-pipelines) because all of the Tasks' operations are performed in order, with the result being fed into the next task.
