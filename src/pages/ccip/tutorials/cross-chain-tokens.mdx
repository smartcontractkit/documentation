...
1.  The CCIP transaction is completed once the status is marked as "Success". The data field is empty because you only transfer tokens. Also, note that the fees are in LINK. As discussed in the introduction, even if you pay CCIP fees in native gas, CCIP will convert native gas to LINK.
