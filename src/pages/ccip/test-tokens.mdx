...
Two ERC20 test tokens are currently available on each testnet. You can find the token addresses for each testnet on the [Supported Networks](/ccip/supported-networks) page.
...
You can mint both of these tokens using the following function call, which acts like a faucet. Each call will mint 10\*\*18 units of a token to the specified address.
...
- Go to the [Supported Networks page](/ccip/supported-networks).
- Go to [Ethereum Sepolia section](/ccip/supported-networks#ethereum-sepolia). You will find a list of active lanes where the source chain is _Ethereum Sepolia_. You will find the list of supported tokens you can transfer for each lane. You should find _CCIP-BnM_ in the list for each testnet.
...
- Fill in the text field with your EOA address, then click _Write_.
- Metamask will open and asks you to confirm the transaction.
- After the transaction is confirmed, your can click _View your transaction_ to view your transaction.
