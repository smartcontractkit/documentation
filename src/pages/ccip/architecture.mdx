...
The [Committing DON](#committing-don) interacts with the `CommitStore` contract on the destination blockchain to store the Merkle root of the finalized messages on the source blockchain. This Merkle root must be blessed by the [Active Risk Management](/ccip/concepts#active-risk-management-arm-network) before [Executing DON](#executing-don) can execute them on the destination blockchain. The `CommitStore` ensures the message is blessed by the [Active Risk Management](#active-risk-management-arm-network) Network and only one `CommitStore` exists per [lane](/ccip/concepts#lane).
...
- Some tokens can be minted on multiple chains. Examples of such tokens include stablecoins like USDC, TUSD, USDT, and FRAX. The linked token pools use a "Burn & Mint" method to burn the token at its source and then mint it natively on the destination blockchain. Wrapped assets such as WBTC or WETH are other examples that use the "Burn and Mint" approach.
- A token with a PoR (Proof Of Reserve) feed on a specific chain poses a challenge for the "Burn & Mint" method when applied to other chains because it conflicts with the PoR feed. For these tokens, "Lock and Mint" is the preferred approach.
...
The ARM contract maintains the list of ARM nodes' addresses allowed to bless/curse and holds the quorum logic for blessing a committed Merkle Root and cursing CCIP on a destination blockchain. Read the [ARM concepts](/ccip/concepts#active-risk-management-arm-network) to learn more.
...
- The job waits for [finality](/ccip/concepts#finality), which depends one the source blockchain.
...
Separating commitment and execution permits the [Active Risk Management](#active-risk-management-arm-network) network to have enough time to check the commitment of messages before executing them. The delay between commitment and execution also permits additional checks such as abnormal reorg depth, potential simulation, and slashing.
