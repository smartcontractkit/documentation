...
The [Committing DON](#committing-don) interacts with the `CommitStore` contract on the destination blockchain to store the Merkle root of the finalized messages on the source blockchain. This Merkle root must be blessed by the [Risk Management](/ccip/concepts#risk-management-network) before [Executing DON](#executing-don) can execute them on the destination blockchain. The `CommitStore` ensures the message is blessed by the [Risk Management](#risk-management-network) Network and only one `CommitStore` exists per [lane](/ccip/concepts#lane).
...
The Risk Management contract maintains the list of Risk Management nodes' addresses allowed to bless/curse and holds the quorum logic for blessing a committed Merkle Root and cursing CCIP on a destination blockchain. Read the [Risk Management concepts](/ccip/concepts#risk-management-network) to learn more.
...
- The job waits for the [Risk Management](#risk-management-network) network to bless the message.
...
Separating commitment and execution permits the [Risk Management](#risk-management-network) network to have enough time to check the commitment of messages before executing them. The delay between commitment and execution also permits additional checks such as abnormal reorg depth, potential simulation, and slashing.
...
Read the [Risk Management concepts](/ccip/concepts#risk-management-network) page to learn more.
