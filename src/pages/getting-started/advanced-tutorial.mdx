...
<Aside type="note" title="Requirements">
  This guide requires basic knowledge about smart contracts. If you are new to smart contract development, read the
  [Consuming Data Feeds](/getting-started/consuming-data-feeds/) and [Random
  Numbers](/getting-started/intermediates-tutorial/) guides before you begin.
</Aside>
...
- [**Tasks**](/chainlink-nodes/oracle-jobs/task-types/all-tasks/) - These are tasks that come built-in to each node. (examples: http, ethabidecode, etc).
...
- [HTTP](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#http-task) calls the API. the `method` must be set to _GET_.
- [JSON Parse](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#json-parse-task) parses the JSON and extracts a value at a given keypath.
- [Multiply](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#multiply-task) multiplies the input by a multiplier. Used to remove the decimals.
- [ETH ABI Encode](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#eth-abi-encode-task) converts the data to a bytes payload according to ETH ABI encoding.
- [ETH Tx](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#eth-tx-task) submits the transaction to the chain, completing the cycle.
...
1. [HTTP](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#http-task) calls the API and returns the body of an HTTP GET result for [ETH/USD pair](https://min-api.cryptocompare.com/data/pricemultifull?fsyms=ETH&tsyms=USD). Example:
...
2. [JSON Parse](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#json-parse-task) walks a specified `path` (`"RAW,ETH,USD,VOLUME24HOUR"`) and returns the value found at that result. Example: `703946.0675653099`

3. [Multiply](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#multiply-task) parses the input into a float and multiplies it by the 10^18. Example: `703946067565309900000000`

4. [ETH ABI Encode](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#eth-abi-encode-task) formats the input into an integer and then converts it into Solidity's `uint256` format. Example: `0xc618a1e4`

5. [ETH Tx](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#eth-tx-task) takes the given input, places it into the data field of the transaction, signs a transaction, and broadcasts it to the network. Example: [transaction result](https://sepolia.etherscan.io/tx/0x10dd83b08ac2e2df1b1cc3c1615abb3287a4a9d29dad75f4c75f45a3a9dd819e)

**Note: Some tasks accept parameters to be passed to them to inform them how to run.** Example: [JSON Parse](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#json-parse-task) accepts a `path` parameter which informs the task where to find the data in the JSON object.
...
2. `requestVolumeData` functions: This builds and sends a request - which includes the fulfillment functions selector - to the oracle. Notice how it adds the `get`, `path` and `times` parameters. These are read by the Tasks in the job to perform correctly. `get` is used by [HTTP](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#http-task), `path` is used by [JSON Parse](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#json-parse-task) and `times` is used by [Multiply](/chainlink-nodes/oracle-jobs/task-types/all-tasks/#multiply-task).
