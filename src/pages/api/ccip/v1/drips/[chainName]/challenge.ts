...
export const GET: APIRoute = async ({ request, params }) => {
  ...
  try {
    ...
    // 3. Generate challenge using FaucetService
    const faucetService = new FaucetService()
    const ttlSec = parseInt(process.env.FAUCET_TTL_SECONDS || "300", 10) // 5 minutes
    const kid = process.env.FAUCET_CHALLENGE_SECRET_KID || "v1"
