...
<Aside type="caution" title="Disclaimer">
  This tutorial represents an example of using a Chainlink product or service. It is provided to help you understand how
  to interact with Chainlink's systems so that you can integrate them into your own. This template is provided "AS IS"
  without warranties of any kind, has not been audited, and may be missing key checks or error handling to make the
  usage of the product more clear. Take everything in this tutorial as an example and not something to be copy pasted
...
- Create an account with [Infura](https://www.infura.io/) or [Alchemy](https://www.alchemy.com/) if you do not already have one.

Your _deployer account_ is the main account you will use to deploy the lootbox contract, distribute reward tokens, pay for transaction fees, and pay for VRF costs.
...
- Ensure the deployer account has enough LINK to fund the [VRF subscription](/vrf/v2/subscription). If you don't already have a VRF subscription, the deployment script will create one for you.
  ...
  - The initial funding amount is set to 10 LINK. If needed, update the initial funding amount in [`network-config.ts`](https://github.com/smartcontractkit/quickstarts-lootbox/blob/main/network-config.ts).

In this tutorial, you will use a second account (the _receiving account_) to test opening the lootbox and receiving the reward tokens as a user.

- [Create a second account in your MetaMask wallet](https://support.metamask.io/hc/en-us/articles/360015289452-How-to-create-an-additional-account-in-your-wallet) if you do not already have one, and [configure the account](https://support.metamask.io/hc/en-us/articles/360015489031-How-to-display-tokens-in-MetaMask) to receive the reward tokens you want to distribute from the lootbox.
...
- The [RPC URL for your deployment network](https://hardhat.org/hardhat-runner/docs/config#json-rpc-based-networks), using an [RPC node provider] such as [Infura](https://www.infura.io/) or [Alchemy](https://www.alchemy.com/). Learn more about [Ethereum nodes as a service](https://ethereum.org/en/developers/docs/nodes-and-clients/nodes-as-a-service/).
