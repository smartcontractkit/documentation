...
- `source: fs.readFileSync(path.resolve(__dirname, "source.js")).toString()`: The source code must be a script object. This example uses `fs.readFileSync` to read `sourcejs` and calls `toString()` to get the content as a `string` object.
