...
The Chainlink Functions starter kit creates [gists](https://docs.github.com/en/get-started/writing-on-github/editing-and-sharing-content-with-gists/creating-gists) containing encrypted secrets on your behalf, then share the gist URL with the DON. This method comes with a security benefit:

- The encrypted secrets are never stored on-chain. The secrets are encrypted with the DON's public key so that only an oracle node in the DON can decrypt them using the DON's private key. After the DON fulfills a request, the starter kit deletes the gist. Which allows to revoking credentials once the DON has fulfilled the request.
...
1. Share your fine-grained personal access token with the starter kit. Run `npx env-enc` to add an encrypted `GITHUB_API_TOKEN` to your `.env.enc` file.

   ```shell
   npx env-enc set GITHUB_API_TOKEN "YOUR_GITHUB_API_TOKEN"
   ```
