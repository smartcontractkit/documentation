...
- Polygon Mumbai: Sign up for a [free Polygonscan API key](https://polygonscan.com/login) and include it in your `.env.enc` by running `npx env-enc set POLYGONSCAN_API_KEY "YOUR_POLYGONSCAN_API_KEY"`.
- Sepolia: Sign up for a [free Etherscan API key](https://etherscan.io/login) and include it in your `.env.enc` file by running `npx env-enc set ETHERSCAN_API_KEY "YOUR_ETHERSCAN_API_KEY"`.
