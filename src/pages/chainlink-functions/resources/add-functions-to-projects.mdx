...
1.  Use `curl` to get the example hardhat config file `hardhat.config.js`:
    ...
    Optionally, you can copy and paste this file from the **[smartcontractkit/smart-contract-examples](https://github.com/smartcontractkit/smart-contract-examples/blob/main/add-functions-to-project/hardhat.config.js)** repository.
...
1. Run the `curl` command to download the latest tarball from the starter kit `main` branch and then run `tar -xf` with the `--strip` flag to extract only the files you need. The following `curl` command combines both steps:
   ...
   Optionally, you can [download the tarball](https://api.github.com/repos/smartcontractkit/functions-hardhat-starter-kit/tarball/main) directly from GitHub and extract the dependencies manually. This tarball comes from the **[smartcontractkit/functions-hardhat-starter-kit](https://github.com/smartcontractkit/functions-hardhat-starter-kit)** repository, which is different from the **[smartcontractkit/smart-contract-examples](https://github.com/smartcontractkit/smart-contract-examples/)** repository even if it contains some similar files.
...
1. Use `curl` to get the example deployment script `deploy.js` and put it in the `./scripts` folder of your project:
   ...
   Optionally, you can copy and paste the script from the **[smartcontractkit/smart-contract-examples](https://github.com/smartcontractkit/smart-contract-examples/blob/main/add-functions-to-project/scripts/deploy.js)** repository.
...
1. Use `curl` to get the example subscription script `functions-sub.js` and put it in the `./scripts` folder of your project:
   ...
   Optionally, you can copy and paste this script from the **[smartcontractkit/smart-contract-examples](https://github.com/smartcontractkit/smart-contract-examples/blob/main/add-functions-to-project/scripts/functions-sub.js)** repository:
...
1. Use `curl` to get the example source code. This code runs on each node in the DON and returns a response to your consumer contract. For this example, use the source from the [Call an API](/chainlink-functions/tutorials/api-query-parameters) tutorial. The following `curl` request creates a file named `Functions-request-source.js` with the source code:
   ...
   Optionally, you can copy and paste this source code file from the **[smartcontractkit/smart-contract-examples](https://github.com/smartcontractkit/smart-contract-examples/blob/main/add-functions-to-project/add-functions-to-project/Functions-request-source.js)** repository.
...
1. Get the example script and put it in the `./scripts` directory:
   ...
   Optionally, you can copy and paste the script from the **[smartcontractkit/smart-contract-examples](https://github.com/smartcontractkit/smart-contract-examples/blob/main/add-functions-to-project/scripts/request.js)** repository.
