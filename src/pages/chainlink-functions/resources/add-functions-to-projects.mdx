...
1. Run the `curl` command to download the latest tarball from the starter kit `main` branch and then run `tar -xf` with the `--strip` flag to extract only the files you need. Optionally, you can [download the tarball](https://api.github.com/repos/smartcontractkit/functions-hardhat-starter-kit/tarball/main) directly from GitHub and extract the dependencies manually. The following command combines both steps:
