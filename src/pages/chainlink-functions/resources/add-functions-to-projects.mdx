...
You must have a wallet with testnet LINK and native gas coins to run this example. This example uses Polygon Mumbai, but you can use any of the [Supported Networks](/chainlink-functions/supported-networks). You can get testnet LINK at [faucets.chain.link](https://faucets.chain.link/mumbai) and testnet MATIC at the [Polygon Faucet](https://faucet.polygon.technology/). To learn how to get testnet funds for other networks, see the [LINK Token Contracts](/resources/link-token-contracts?parent=chainlinkFunctions) page.
