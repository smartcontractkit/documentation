...
Given a `values` array containing `[ 2, 5, 2, "foo", "foo" "bar", "foo", 2 ]`, the task returns the following JSON:
...
To encode the results array into the `ethabiencode` task, specify that the data is an array in the `abi` and point the data parameter to the results from your mode task:

{/* prettier-ignore */}
```toml
encode_mode_task [type="ethabiencode" abi="(bytes32 requestId, bytes32[] _data)" data="{\\"requestId\\": $(decode_log.requestId), \\"_data\\": $(my_mode_task.results)}"]
```
