...
The requirements for running a Chainlink node scale with the as the number of jobs that your node services. CPUs with the x86 architecture is recommended for production environments, but you can use Apple M1 ARM systems for development if you run the Chainlink node in Docker.
...
- Docker: Althought it is possible to [build Chainlink nodes from source](https://github.com/smartcontractkit/chainlink), the best practice is to use the [Chainlink Docker Images](https://hub.docker.com/r/smartcontract/chainlink/tags) without `-root`.
- [PostgreSQL](https://wiki.postgresql.org/wiki/Detailed_installation_guides) versions `>=11 <15` (Version 11, 12, 13, or 14. Not version 15 or later).
  - If you use a database as a service, your database host must provide access to logs.
  - If you run the database on a separate system, [secure the TCP/IP connection with SSL](https://www.postgresql.org/docs/14/ssl-tcp.html).
