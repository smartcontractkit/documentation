...
1. Create an `.api` file with your API credentials. The API email and password will be used to expose the API for the GUI interface, and will be used every time you log into your node.

   Create the file in the same directory as your TOML config files (for example, `/chainlink-sepolia/.api`) and list your API credentials:
   ...
   docker run \
   ...
   -config /chainlink-sepolia/config.toml \
   -secrets /chainlink-sepolia/secrets.toml \
   node start \
   -a /chainlink-sepolia/.api
   ```
...
1. Start the Chainlink Node by running the Docker image.
   ...
   <Tabs client:visible>
      ...
      <Fragment slot="panel.1">
      ```shell Sepolia
      cd ~/.chainlink-sepolia && docker run --platform linux/x86_64/v8 --name chainlink -v ~/.chainlink-sepolia:/chainlink -it -p 6688:6688 --add-host=host.docker.internal:host-gateway smartcontract/chainlink:2.0.0 node -config /chainlink-sepolia/config.toml -secrets /chainlink-sepolia/secrets.toml start -a /chainlink-sepolia/.api
      ```
      </Fragment>
      <Fragment slot="panel.2">
      ```shell Goerli
      cd ~/.chainlink-goerli && docker run --platform linux/x86_64/v8 --name chainlink -v ~/.chainlink-goerli:/chainlink -it -p 6688:6688 --add-host=host.docker.internal:host-gateway smartcontract/chainlink:2.0.0 node -config /chainlink-goerli/config.toml -secrets /chainlink-goerli/secrets.toml start -a /chainlink-goerli/.api
      ```
      </Fragment>
      <Fragment slot="panel.3">
      ```shell Sepolia
      cd ~/.chainlink-sepolia && docker run --platform linux/x86_64/v8 --name chainlink -v ~/.chainlink-sepolia:/chainlink -it -p 6688:6688 --add-host=host.docker.internal:host-gateway smartcontract/chainlink:1.13.1 -config /chainlink-sepolia/config.toml -secrets /chainlink-sepolia/secrets.toml node start -a /chainlink-sepolia/.api
      ```
      </Fragment>
      <Fragment slot="panel.4">
      ```shell Goerli
      cd ~/.chainlink-goerli && docker run platform linux/x86_64/v8 --name chainlink -v ~/.chainlink-goerli:/chainlink -it -p 6688:6688 --add-host=host.docker.internal:host-gateway smartcontract/chainlink:1.13.1 -config /chainlink-goerli/config.toml -secrets /chainlink-goerli/secrets.toml node start -a /chainlink-goerli/.api
      ```
      </Fragment>
   </Tabs>
