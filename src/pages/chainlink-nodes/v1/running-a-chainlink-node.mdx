...
1. Start the Chainlink Node by running the Docker image.
   ...
   Alternatively, you can pass the API credentials to Docker using a `.api` file. Create the file in the same directory as your TOML config files (for example, `/chainlink/.api`) and list your API credentials:
   ...
   At the end of the `docker run` command, include the `.api` file using the `-a` flag:

   ```
   ...
   -config /chainlink/.node.toml \
