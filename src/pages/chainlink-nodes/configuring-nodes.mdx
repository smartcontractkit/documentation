...
You can see the available config options on the [Node Config](/chainlink-nodes/v2/node-config) and [Node Secrets](/chainlink-nodes/v2/secrets-config) pages.

## Migrating from environment variables to TOML

1. Update your Chainlink Node to v1.13.0 and ensure that it operates as expected. This is the last version that supports environment variable configuration, and the best version to use for TOML migration before you update the node to v2.0.0.
...
1. Make a new directory on the host system to store your config files. Then, `cd` to that directory. This directory will be mounted to Docker when you run the node using the TOML config.

   ```shell
   mkdir ~/.chainlink-toml && cd ~/.chainlink-toml
   ```
...
1. Edit the `secrets.toml` file to include the secrets you need for your specific Chainlink node. See the [Node Secrets](/chainlink-nodes/v2/secrets-config) page for a full list of available config options.
...
1. Validate the configuration:
   ...
   ```shell
   docker run --name chainlink-config-validator -v ~/.chainlink-toml:/chainlink -it --rm smartcontract/chainlink:1.13.0 -config /chainlink/config.toml -secrets /chainlink/secrets.toml config validate
   ```

   You will likely see some invalid config errors. For example:

   ```shell
   Invalid configuration: invalid configuration: EVM.3.Nodes: missing: must have at least one node
   ```

1. Edit the `config.toml` file and manually correct any errors. See the [Node Config](/chainlink-nodes/v2/node-config) and [Node Secrets](/chainlink-nodes/v2/secrets-config) pages to learn which config settings are valid. For the error in this example, an EVM chain was configured with no nodes. Removing this from the config made the config valid:
...
1. Make a Postgres [database snapshot](https://www.postgresql.org/docs/current/backup-file.html) so you can restore your previous Chainlink node if necessary.
...
1. Start a new Chainlink node Docker container named using the new `config.toml` and `secrets.toml` files. This example uses `chainlink-toml` as the container name:
   ...
   ```shell
   docker run --platform linux/x86_64/v8 --name chainlink-toml  -v ~/.chainlink-toml:/chainlink -it -p 6688:6688 --add-host=host.docker.internal:host-gateway smartcontract/chainlink:1.13.0 local n -config /chainlink/config.toml -secrets /chainlink/secrets.toml
   ```

Test your node to verify that it works as intended. Open an SSH tunnel using `ssh -i $KEY $USER@$REMOTE-IP -L 6688:localhost:6688 -N` and connect to the Operator Interface in your browser at `localhost:6688`.
...
```shell
docker run --platform linux/x86_64/v8 --name chainlink-toml  -v ~/.chainlink-toml:/chainlink -it -p 6688:6688 --add-host=host.docker.internal:host-gateway smartcontract/chainlink:1.13.0 local n -config /chainlink/config.toml -config /chainlink/config2.toml -config /chainlink/config3.toml -secrets /chainlink/secrets.toml
```
