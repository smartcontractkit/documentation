...
/**
 * The canonical pool types that `normalizePoolType` is able to recognize and that
 * `POOL_MECHANISM_MAP` (config/data/ccip/utils.ts) keys on. Anything outside this set
 * is an unrecognized pool class returned verbatim by `normalizePoolType` — which would
 * produce an "Unsupported" mechanism unless a curated fallback is used.
 */
const KNOWN_POOL_TYPES = new Set(["usdc", "lockRelease", "burnMint"])

/**
 * Returns true when the given normalized pool type is one that `normalizePoolType`
 * successfully mapped (i.e. it can form a valid `POOL_MECHANISM_MAP` key).
 *
 * @example isKnownPoolType("lockRelease") → true
 * @example isKnownPoolType("SiloedWithUnsiloedXERC20GroupTokenPool") → false
 */
export function isKnownPoolType(poolType: string): boolean {
  return KNOWN_POOL_TYPES.has(poolType)
}
