...
export function cllAdmonitions() {
  return (tree) => {
    visit(tree, (node) => {
      if (
        ...
        if (!Object.keys(types).includes(node.name)) return;
        ...
        for (let i = 0; i < node.children.length; i++) {
          if (!!node.children[i]?.data?.directiveLabel) {
            node.children[i].data.hName = "div";
            node.children[i].data.hProperties = h("div", {
              class: "heading",
            }).properties;
            // // create the nodes for the icon
            const settings = types[node.name];
            const iconNodes = nodes(settings.svg);
            console.log({ iconNodes });
            const iconContainerNodes = [
              element("span", "admonition-icon", [iconNodes]),
            ];
            console.log({ iconContainerNodes });

            node.children[i].children = iconContainerNodes.concat(
