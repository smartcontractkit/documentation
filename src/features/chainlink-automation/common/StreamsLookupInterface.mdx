...
This function is simulated offchain to respond to [error codes](/chainlink-automation/guides/streams-lookup-error-handler#error-codes) before sending data onchain via `performUpkeep`. 

#### Parameters

| Variable Name | Type      | Description                                                    | Permissible Values                                                             |
| ------------- | --------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `errorCode`      | `uint` | Error code returned by StreamsLookup error handler | e.g. "800400" (`ErrCodeStreamsBadRequest`) |
| `extraData`   | `bytes`   | Bytes data sent as part of original MercuryLookup revert error | e.g. can send log data if you want to decode to use                            |
