...
1. **Claiming and Accepting the Admin Role**: This is a two-step process:
   1. You will call the [`RegistryModuleOwnerCustom`](/ccip/v1/evm/api-reference/v1.6.1/registry-module-owner-custom) contract's [`registerAdminViaOwner`](/ccip/v1/evm/api-reference/v1.6.1/registry-module-owner-custom#registeradminviaowner) function to register your EOA as the token admin. This role is required to enable your token in CCIP.

   1. Once claimed, you will call the [`TokenAdminRegistry`](/ccip/v1/evm/api-reference/v1.6.1/token-admin-registry) contract's [`acceptAdminRole`](/ccip/v1/evm/api-reference/v1.6.1/token-admin-registry#acceptadminrole) function to complete the registration process.

1. **Linking Tokens to Pools**: You will call the [`TokenAdminRegistry`](/ccip/v1/evm/api-reference/v1.6.1/token-admin-registry) contract's [`setPool`](/ccip/v1/evm/api-reference/v1.6.1/token-admin-registry#setpool) function to associate each token with its respective token pool.

1. **Configuring Token Pools**: You will call the [`applyChainUpdates`](/ccip/v1/evm/api-reference/v1.6.1/token-pool#applychainupdates) function on your token pools to configure each pool by setting cross-chain transfer parameters, such as token pool rate limits and enabled destination chains.

1. **Minting Tokens**: You will call the [`mint`](https://github.com/smartcontractkit/chainlink-evm/blob/contracts-solidity/1.5.0/contracts/src/v0.8/shared/token/ERC20/BurnMintERC20.sol#L128) function to mint tokens on Avalanche Fuji for your EOA. These tokens will later be used to test cross-chain transfers to Arbitrum Sepolia.
