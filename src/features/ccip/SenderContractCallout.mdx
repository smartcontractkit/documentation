...
- **Understand `allowOutOfOrderExecution` Usage**: This example sets `allowOutOfOrderExecution` to `true` (see [EVMExtraArgsV2](/ccip/api-reference/evm/v1.5.1/client#evmextraargsv2)). However, as documented in the [Best Practices: Setting `allowOutOfOrderExecution`](/ccip/concepts/best-practices/evm#setting-allowoutoforderexecution), this parameter is available only on lanes where the **Out of Order Execution** property is set to **Optional** or **Required**. Refer to the [CCIP Directory](/ccip/directory) to determine if your target lane supports this feature. For lanes where this parameter is absent, you must use [`extraArgsV1`](/ccip/api-reference/evm/v1.5.1/client#evmextraargsv1) instead.
