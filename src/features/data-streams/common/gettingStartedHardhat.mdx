...
This guide shows you how to read data from a Data Streams feed, verify the answer onchain, and store it. This example uses a [Chainlink Automation Log Trigger](/chainlink-automation/guides/log-trigger) to check for events that require data. The flow follows this sequence:

- A simple emitter contract emits a log that triggers the upkeep.
- Chainlink Automation then uses `StreamsLookup` to retrieve a signed report from the Data Streams Engine, returns the data in a callback, and runs the [`performUpkeep` function](/chainlink-automation/reference/automation-interfaces#performupkeep-function-for-log-triggers) on your registered upkeep contract.
