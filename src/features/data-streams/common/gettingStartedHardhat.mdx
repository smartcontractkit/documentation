...
Note: To learn how to use Data Streams with the REST API or WebSocket, see the [Fetch and decode reports via a REST API](/data-streams/tutorials/go-sdk-decode) guide or the [Stream and decode reports via WebSocket](/data-streams/tutorials/go-sdk-stream) guide.
...
- The decoded reports within the contract use `bytes32` types for feed IDs (see the [Report Schemas](/data-streams/reference/report-schema-v3) reference).
