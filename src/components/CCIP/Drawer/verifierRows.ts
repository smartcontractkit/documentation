...
export interface LaneVerifierRow {
  ...
  logo: string
  type: string | null
  /** Required = base CCV; Additional = only applies at/above threshold. */
  ...
  /** Human-readable lane threshold (raw smallest-unit amount) when threshold-based CCV is enabled. */
...
function resolveSide(set: VerifierSet | null, networkId: string, environment: Environment): ResolvedVerifier[] {
  ...
  for (const address of set.aboveThreshold) {
    ...
    resolved.push({
      ...
      type: meta?.type ?? null,
      isAdditional: !below.has(address.toLowerCase()),
    })
  }
  return resolved
}
...
export function buildLaneVerifierRows(
  ...
  laneThresholdAmount: string | null
): LaneVerifierRow[] {
  if (!verifiers) return []

  const enabledThreshold = laneThresholdAmount && laneThresholdAmount !== "0" ? laneThresholdAmount : null
  ...
  for (const v of resolveSide(verifiers.source, sourceNetworkId, environment)) {
    rows.set(v.joinKey, {
      ...
      logo: v.logo,
      type: v.type,
      category: v.isAdditional ? "Additional" : "Required",
      sourceAddress: v.address,
      destinationAddress: null,
      thresholdAmount: v.isAdditional ? enabledThreshold : null,
    })
  }
  ...
  for (const v of resolveSide(verifiers.destination, destNetworkId, environment)) {
    ...
    if (existing) {
      ...
      // If either side marks this verifier as additional, treat combined row as Additional.
      if (v.isAdditional) {
        existing.category = "Additional"
        existing.thresholdAmount = enabledThreshold
      }
    } else {
      rows.set(v.joinKey, {
        key: v.joinKey,
