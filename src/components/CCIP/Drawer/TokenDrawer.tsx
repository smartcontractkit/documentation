          ...
          <table className="ccip-table">
            ...
            <tbody>
              {laneRows
                ...
                .map(
                  ({ networkDetails, chainKey, destinationPoolType, rateLimits, verifiers, destinationDecimals }) => {
                    const allLimits = realtimeDataService.getAllRateLimitsForDirection(rateLimits, direction)
                    const tokenPaused = !!allLimits.standard?.isEnabled && allLimits.standard.capacity === "0"
                    ...
                    const verifierRows = buildLaneVerifierRows(
                      ...
                      environment,
                      tokenDirectory?.pool?.ccv?.thresholdAmount ?? null
                    )
