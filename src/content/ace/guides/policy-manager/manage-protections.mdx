...
1. A [target contract](/ace/guides/policy-manager/manage-targets) detected or registered under that engine.
1. A [policy instance](/ace/guides/policy-manager/manage-policies) created from a policy type and associated with the same engine.
...
<MethodTabs>
  ...
  <Fragment slot="api">
    ...
    | `policy_instance_id` | Yes | UUID of the policy to bind: the onchain policy instance ID, or the offchain (CADV) policy ID when `policy_kind` is `"offchain"` |
    | `policy_kind` | No | Kind of policy being protected: `"onchain"` (default) or `"offchain"` |
...
- [Managing Targets](/ace/guides/policy-manager/manage-targets) — detect or register contracts to protect
