...
<Aside type="note" title="Extractors: pre-built and custom">
  ...
  those contract types. A built-in CCIP-AdvancedPoolHooks type covers CCIP token pool hook functions. For other function
  signatures, you write and deploy your own extractor contract (implementing `IExtractor`) and register it via the
