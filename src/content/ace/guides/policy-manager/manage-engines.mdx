...
<MethodTabs>
  <Fragment slot="ui">
    ...
    1. Complete the three-step wizard:
       ...
       - **Contract types** — select **ERC-20**, **ERC-3643**, and/or **CCIP-AdvancedPoolHooks**. ACE attaches the pre-built extractors for the selected types automatically. For the CCIP-specific setup, see [Protect CCIP Token Pools with ACE](/ace/guides/policy-manager/ccip-token-pools).
  ...
  <Fragment slot="api">
    ...
    | `contract_type_ids` | No | Array of contract type UUIDs to register on the engine |
    ...
    <Aside type="tip" title="Attach extractors at creation time">
      Include the `extractor_ids` field when creating your engine so extractors are registered immediately. For ERC-20
      and ERC-3643 contracts, see the [Policy Manager Quick
      Start](/ace/getting-started/policy-manager#extractor-ids-for-api-creation) for the pre-built extractor IDs. For
      `CCIP-AdvancedPoolHooks`, see [Protect CCIP Token Pools with
      ACE](/ace/guides/policy-manager/ccip-token-pools). For a custom contract type, register your extractor first. See
      [Custom Contract Types & Extractors](/ace/guides/policy-manager/contracts/custom-contract-types).
    </Aside>
...
<MethodTabs>
  <Fragment slot="ui">
    ...
    1. Click on a policy engine to open it. The detail view has two tabs:
       ...
       - **Policy instances** — lists all deployed policy instances associated with this policy engine.

    1. To view the engine's on-chain details, click the **gear icon** to open the settings page. There you can see:
...
<Aside type="tip" title="Available extractors">
  See the [Policy Manager Quick Start](/ace/getting-started/policy-manager#extractor-ids-for-api-creation) for ERC-20
  and ERC-3643 extractor IDs. For `CCIP-AdvancedPoolHooks`, see [Protect CCIP Token Pools with
  ACE](/ace/guides/policy-manager/ccip-token-pools). You can retrieve every available extractor with `GET
...
- [Managing Targets](/ace/guides/policy-manager/manage-targets) — detect or register contracts to protect under an engine
- [Managing Policies](/ace/guides/policy-manager/manage-policies) — create and configure policy instances
- [Protect CCIP Token Pools with ACE](/ace/guides/policy-manager/ccip-token-pools) — configure a policy engine for CCIP AdvancedPoolHooks
