...
<Aside type="note" title="Extractors: pre-built and custom">
  The platform provides pre-built extractors for ERC-20, ERC-3643, and `CCIP-AdvancedPoolHooks` function signatures. See
  [Protect CCIP Token Pools with ACE](/ace/guides/policy-manager/ccip-token-pools) for the CCIP integration. For other
  contract types, write and deploy your own extractor contract (implementing `IExtractor`) and register it through the
  Coordinator API. See [Custom Contract Types & Extractors](/ace/guides/policy-manager/contracts/custom-contract-types)
  for details.
</Aside>
...
<Aside type="note" title="Custom mappers are not available">
  Custom Mappers are not available through the ACE Platform during Beta. The platform uses the default name-based
  parameter mapping for all policies. See [Beta
  Scope](/ace/beta-scope#custom-extractors-are-registered-by-you-custom-mappers-are-not-available) for details.
</Aside>
