...
1. Call the [`requestLinkFromTheFaucet`](/chainlink-local/api-reference/javascript/cciplocalsimulatorfork#requestlinkfromthefaucet) function to request LINK tokens for Alice.
...
1. CAll the [`getEvm2EvmMessage`](/chainlink-local/api-reference/javascript/cciplocalsimulatorfork#getevm2evmmessage) function to parse the transaction receipt and extract the `CCIPSendRequested` event and then decodes it to an object.
...
1. Call the [`routeMessage`](/chainlink-local/api-reference/javascript/cciplocalsimulatorfork#routemessage) function to route the message to the destination router.
