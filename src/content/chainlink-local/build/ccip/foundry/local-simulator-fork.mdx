...
date: Last Modified
title: "Using the Chainlink Local Simulator in your Foundry project with forked environments"
isIndex: false
---
...
This guide assumes that you are familiar with the guide [Using the Chainlink Local Simulator in your Foundry project](/chainlink-local/build/ccip/foundry/local-simulator).
...
1. Initialize the [fork Chainlink Local simulator](/chainlink-local/api-reference/v0.2.3/ccip-local-simulator-fork#cciplocalsimulatorfork).
...
1. Create an array [Client.EVMTokenAmount](/ccip/v1/evm/api-reference/v1.6.1/client#evmtokenamount)[] to specify the token transfer details. This array and the amount to send
...
1. Request 10 LINK tokens from the Chainlink Local simulator faucet for Alice (sender):
