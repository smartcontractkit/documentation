...
date: Last Modified
title: "Using CCIP Local Simulator in Your Foundry Project - Forked Environments"
isIndex: false
---

You can use Chainlink Local to run CCIP in forked environments within your Foundry project. To get started quickly, we will use the [CCIP Starter Kit - Foundry](https://github.com/smartcontractkit/ccip-starter-kit-foundry). This project includes the `Example01ForkTest` file located in the `./test/fork` directory, demonstrating how to set up and run token transfer tests between two accounts using CCIP in forked environments. Forked environments allow you to simulate real-world blockchain networks by forking the state of existing chains. In this example, we will fork _Arbitrum Sepolia_ and _Ethereum Sepolia_.

## Prerequisites

This guide assumes that you are familiar with the guide [Using CCIP Local Simulator in Your Foundry Project](/chainlink-local/build/ccip/foundry/local-simulator). If not, please get familiar with it and run all the prerequisites.

Additionally, set up an `.env` file with the following data:
...
- A test token contract (e.g., CCIP-BnM) on both source and destination chains
...
1. Initialize the [fork CCIP local simulator](/chainlink-local/api-reference/solidity/ccip/CCIPLocalSimulatorFork). **Note**: `vm.makePersistent` is used to make the `ccipLocalSimulatorFork` address persistent across forks:
...
1. Retrieve and set up the network details for the destination chain. **Note**: [`Register.NetworkDetails`](/chainlink-local/api-reference/solidity/ccip/Register#networkdetails) is a struct that stores network details (such as chain selector, router address, link address, wrapped native address, or CCIP test tokens). This struct is returned by [`getNetworkDetails`](/chainlink-local/api-reference/solidity/ccip/CCIPLocalSimulatorFork#getNetworkDetails) based on chain IDs:
...
1. Select the destination fork and record the initial token balance of Bob:
...
1. Select the source fork and record the initial token balance of Alice:
...
1. Request 10 LINK tokens from the CCIP local simulator faucet for Alice:

   ```solidity
   ccipLocalSimulatorFork.requestLinkFromFaucet(alice, 10 ether);
   ```

1. Construct the [Client.EVM2AnyMessage](/ccip/api-reference/client#evm2anymessage) structure with the receiver, token amounts, and other necessary details. We set data: `abi.encode("")` because we are not sending any arbitrary data (only tokens). Additionally, `extraArgs: Client._argsToBytes(Client.EVMExtraArgsV1({gasLimit: 0}))` is used because we are sending tokens to an EOA and do not expect any execution of receiver logic:
...
1. Record the final token balance of Alice and verify that it has decreased by the amount sent:
...
1. Call the [`switchChainAndRouteMessage`](/chainlink-local/api-reference/solidity/ccip/CCIPLocalSimulatorFork#switchChainAndRouteMessage) function to switch to the destination fork and route the message to complete the transfer:

   ```solidity
   ccipLocalSimulatorFork.switchChainAndRouteMessage(destinationFork);
   ```

1. Record the final token balance of Bob and verify that it has increased by the amount sent:
...
1. Select the destination fork and record the initial token balance of Bob. (This step is the same as in the previous test case.)
1. Select the source fork and record the initial token balance of Alice. (This step is the same as in the previous test case.)
1. Begin impersonating Alice and provide her with native gas:
...
1. Construct the [Client.EVM2AnyMessage](/ccip/api-reference/client#evm2anymessage) structure. This step is the same as in the previous test case. The main difference is that the `feeToken` is set with `address(0)` to indicate that the fees are paid in native gas:
...
1. Calculate the required fees for the transfer and send the CCIP transfer request along with the necessary gas:
...
1. Verify Alice (sender) balance. (This step is the same as in the previous test case.)
1. Call the [`switchChainAndRouteMessage`](/chainlink-local/api-reference/solidity/ccip/CCIPLocalSimulatorFork#switchChainAndRouteMessage) function to switch to the destination fork and route the message to complete the transfer. (This step is the same as in the previous test case.)
1. Verify Bob (receiver) balance. (This step is the same as in the previous test case.)
