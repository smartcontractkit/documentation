...
You can use Chainlink Local to run CCIP in a localhost environment within your Foundry project. To get started quickly, we will use the
...
1. Record the final token balances for Alice and Bob:
...
1. Stop impersonating Alice and verify the token balances for Alice (sender) and Bob (receiver). (This step is the same as in the previous test case.)
