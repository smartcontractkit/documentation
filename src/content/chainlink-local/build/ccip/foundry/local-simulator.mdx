...
You can use Chainlink Local to run CCIP in a localhost environment within your Foundry project. To get started quickly, we will use the [CCIP Starter Kit - Foundry](https://github.com/smartcontractkit/ccip-starter-kit-foundry). This project is a Foundry boilerplate that includes the Chainlink Local package and several CCIP examples.

## Prerequisites

1. In a terminal, clone the [CCIP Starter Kit - Foundry](https://github.com/smartcontractkit/ccip-starter-kit-foundry) repository and change directories.
...
- A test token contract (e.g., CCIP-BnM)
...
1. Initialize the [CCIP local simulator](/chainlink-local/api-reference/solidity/ccip/CCIPLocalSimulator) contract:
...
1. Request CCIP-BnM tokens for Alice:
...
Invoke the prepareScenario() function to set up the necessary variables:

    ```solidity
    (Client.EVMTokenAmount[] memory tokensToSendDetails, uint256 amountToSend) = prepareScenario();
    ```

1. Record the initial token balances of Alice and Bob:
...
1. Begin impersonating Alice to perform the subsequent actions:

   ```solidity
   vm.startPrank(alice);
   ```

1. Request 5 LINK tokens from the CCIP local simulator faucet for Alice:
...
1. Stop impersonating Alice:

   ```solidity
   vm.stopPrank();
   ```

1. Record the final token balances of Alice and Bob:
...
1. Record the initial token balances of Alice and Bob. (This step is the same as in the previous test case.)
1. Begin impersonating Alice and provide her with native gas to pay for the fees:
...
1. Stop impersonating Alice and verify Alice (sender) and Bob (receiver) token balances. (This step is the same as in the previous test case.)

## Next steps

For more advanced scenarios, please refer to other test files in the `./test/no-fork` directory. To learn how to use Chainlink local in forked environments, refer to the guide on [Using CCIP Local Simulator in Your Foundry Project - Forked Environments](/chainlink-local/build/ccip/foundry/local-simulator-fork) guide.
