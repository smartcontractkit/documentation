...
For this example request, 0.82 LINK is reserved from your subscription balance, but not yet deducted. Next, when the request is fulfilled, the exact request cost will be deducted. The estimated cost of a request is overestimated to allow for 99% of gas price fluctuation increases in between request and response time.
...
This example request would cost 0.2825 LINK when fulfilled. The subscription reservation for the 0.82 LINK is released, and the actual cost, 0.2825 LINK, is deducted from your subscription balance.
