...
The Subscription Manager provides the option to time out requests manually. You can time out requests that have been pending for longer than five minutes in order to unlock your subscription funds.

1. Open your subscription details and scroll to the **Pending** section. If a request has been pending longer than five minutes, its status displays as _Time out required_. Click the link that says **Time out request**:
...
Alternatively, if you have multiple requests that are eligible for timeout, you can time out an individual request. Open the **Actions** menu for that request, and click **Time out request**:
...
1. A MetaMask popup appears, and you are asked to confirm the transaction:
   ...
   <ClickToZoom
     src="/images/chainlink-functions/tutorials/request-timeout/2-approve-timout-transaction.png"
...
1. The timed out request appears in the **Failed fulfillments** tab of the **History** section:
   ...
   <ClickToZoom
     src="/images/chainlink-functions/tutorials/request-timeout/your-subscription-cancelled.jpg"
     alt="Chainlink Functions subscription canceled"
   />
