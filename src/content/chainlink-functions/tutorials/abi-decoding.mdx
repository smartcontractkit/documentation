...
// Encode first the metadata object
const metadataEncoded = ethers.utils.defaultAbiCoder.encode(
  ["string", "bool"],
  [complexData.metadata.description, complexData.metadata.awesome]
)
...
// Data to encode
const data = [complexData.id, complexData.name, metadataEncoded]
...
```javascript
const encodedBytes = ethers.getBytes(encodedData) // Converting the encoded data to a byte array
return encodedBytes // Return the Uint8Array to fulfill the Chainlink Functions API requirements
```
