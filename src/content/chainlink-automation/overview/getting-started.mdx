...
import { Address, Aside, ClickToZoom } from "@components"
...
<Tabs client:visible>
  ...
  <Fragment slot="panel.timeBased">
      **Increment a counter every 5 minutes using our example contract.**
      ...
      1. Specify the time schedule, for example every 5 minutes. Paste the cron expression `*/5 * * * *` under _Cron expression_ or select one of the example timers. Then click **Next**.

      1. To learn more about CRON expressions, click [here](/chainlink-automation/guides/job-scheduler#specifying-the-time-schedule).

      1. Enter an _Upkeep name_, your public key address under _Admin Address_, `500000` under _Gas limit_, and `0.1` under _Starting balance (LINK)_.
