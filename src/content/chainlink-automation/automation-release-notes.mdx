...
- **Verifiable compute**: The Automation DON now leverages a consensus mechanism, via Chainlink OCR3, to give you cryptographically verified compute. Save up to 90% of on-chain gas costs by off-loading compute intensive tasks to the Automation DON.

- **Log triggers**: Natively use log data in your smart contracts with [log triggers](/chainlink-automation/log-trigger). Unlock new connection possibilities.

- **StreamsLookup**: Seamlessly access and use Chainlink's Low Latency Data in upkeeps via [StreamsLookup](/chainlink-automation/automation-interfaces#streamslookupcompatibleinterface). Build like the best Derivative protocols.

- **Forwarder**: A unique `msg.Sender` for your `performUpkeep` function so you can lock down sensitive upkeeps. Read more about the [forwarder](/chainlink-automation/forwarder).
...
- **Programmatic control**: With automatic approval, you can now dynamically create, manage, and fund upkeeps from within your dApps and even have an upkeep fund itself. Learn more [here](/chainlink-automation/register-upkeep-in-contract).
...
Ask questions in the [#automation channel](https://discord.com/channels/592041321326182401/821350860302581771) in our [Discord server](https://discord.gg/qj9qarT), or [contact us](https://forms.gle/WadxnzzjHPtta5Zd9) for assistance with registration.
