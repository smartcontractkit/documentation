...
In this example, you will create two VRF subscriptions. One that is underfunded, and another to serve as the funding subscription. You will automate the process for funding the subscription balance. In a production environment, you would monitor multiple subscriptions and top them up with LINK as necessary. This makes it easier to fund multiple subscriptions because you fund one contract that distributes LINK to your VRF subscriptions as necessary.

Monitoring the balance of a single contract is easier than manually tracking multiple subscriptions, and you can fund multiple contracts with one LINK transfer to a single contract rather than paying the gas to distribute funds to multiple contracts yourself. Additionally, if multiple subscriptions require funds, your automated top-up contract can distribute LINK to those subscriptions in a single transaction to further save gas costs.
...
- [Deploy VRF-compatible contracts](#deploy-vrf-compatible-contracts)
