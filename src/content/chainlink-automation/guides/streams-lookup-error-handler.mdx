...
<ClickToZoom src="/images/automation/streamslookup-errorhandler-vertical.png" alt="Error handler flow diagram" style="max-width: 70%; max-height: 70%; display: block; margin-left: auto; margin-right: auto;" />
...
- Specifying an incorrect feedID to force error code [800400](#800400) (`ErrCodeStreamsBadRequest`)
- Specifying a future timestamp to force error code [808206](#808206) (where partial content is received) for both single feedID and bulk feedID requests
- Specifying old timestamps for reports not available anymore yields either error code [808504](#8085XX) (no response) or [808600](#808600) (bad response), depending on which service calls the timeout request

If your `revert streamsLookup` function is defined incorrectly in your smart contracts, the nodes will not be able to decode it.
...
<tbody>
  ...
  <tr>
    <td id="800401">ErrCodeStreamsUnauthorized: 808401</td>
    <td>No</td>
