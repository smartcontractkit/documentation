...
function checkErrorHandler(
  ...
  return (_upkeepNeeded, abi.encode(errorCode, extraData, success));
}
```
...
function performUpkeep(bytes calldata performData) external {
  ...
  if (reportSuccess) {
    ...
    // This is the alternative path if the reports were not pulled successfully
    // Define custom logic here to handle other error codes onchain
  }
}
```
