...
{/* prettier-ignore */}
<Tabs client:visible>
  <Fragment slot="tab.logtriggerUI">Automation App</Fragment>
  <Fragment slot="tab.logtriggerProgram">Programmatically</Fragment>
  <Fragment slot="panel.logtriggerUI">
    ...
    **Click the Register New Upkeep button.**
    ...
    1. **Provide the address of your [Automation-compatible contract](/chainlink-automation/guides/compatible-contracts)** that you want to automate. This contract must follow the format of the [ILogAutomation interface](https://github.com/smartcontractkit/chainlink/blob/develop/contracts/src/v0.8/automation/interfaces/ILogAutomation.sol). This ensures Automation nodes can interact with your contract as expected.
  ...
  <Fragment slot="panel.logtriggerProgram">
    ...
    1. Open Counter_Emit_Log.sol in Remix. Deploy and confirm transaction.
    1. Find the address of contract in Etherscan. Read Write contract in Etherscan
    1. Connect wallet to Etherscan.
    1. Emit log from wallet
    1. Should see in Events tab. May need to wait.
    1. can see log index_topic_1 = msg.sender it's what we're automation
    1. Counter_Log. Struct for log structure and interface for log automation. Two function: one for check Log and one for performUpkeep.
    1. Our contract is ILogInterface. Has event CountedBy. Show who counted. Counted variable will be incremented.
