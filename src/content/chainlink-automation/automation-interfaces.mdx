...
`checkLog` is the initial `view` function that will be simulated off-chain once a log that matches log specification has been emmitted. It should parse the log data and trigger an action. For example, it can trigger the retrieval of Data Streams reports. See the [Data Streams Getting Started](/data-streams/getting-started) guide to see an example.
