...
`checkLog` a `view` function that will be simulated off-chain once a log that matches your `LogTriggerConfig` has been emmitted. You should parse the log data and the check if something needs to happen on-chain. For example, it can trigger the retrieval of Data Streams reports. See the [Data Streams Getting Started](/data-streams/getting-started) guide to see an example.
