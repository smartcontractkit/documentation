...
- **`cre workflow list`**: New command that lists all workflows deployed for your organization. Use `--include-deleted` to include deleted workflows, or `--output json` for machine-readable output. See [`cre workflow list`](/cre/reference/cli/workflow#cre-workflow-list) for details.
