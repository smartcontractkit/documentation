...
{/* prettier-ignore */}
<Aside type="caution" title="Solana simulation always requires CRE_ETH_PRIVATE_KEY">
  For Solana workflows, `CRE_ETH_PRIVATE_KEY` is required even for dry-run simulation (without `--broadcast`). The CLI
  uses it for workflow owner derivation regardless of chain family. Set any valid EVM private key in your `.env` — it
  is not used for the Solana transaction itself. You also need `CRE_SOLANA_PRIVATE_KEY` for the transmitter account; see
  [Writing to Solana](/cre/guides/workflow/using-solana-client/onchain-write-ts) for setup details.
</Aside>
