...
If your private registry workflow uses an HTTP trigger, note that the gateway endpoint depends on the execution environment (DON) your workflow runs on, not on which registry it was deployed to. A workflow deployed to the private registry still runs on the public DON, so it uses the same public gateway as onchain deployments.

Update your `GATEWAY_URL` (or equivalent configuration) wherever you send trigger requests — in local testing, CI scripts, or backend integrations — to use the gateway that matches your workflow's execution environment. See [Gateway URLs](/cre/guides/workflow/using-triggers/http-trigger/triggering-deployed-workflows#gateway-urls) to pick the right endpoint.
