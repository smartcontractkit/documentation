...
<Aside type="note" title="Go">
  This page covers workflow verification for Go workflows. For TypeScript workflows, see [Verifying Workflows
  (TypeScript)](/cre/guides/operations/verifying-workflows-ts).
</Aside>
...
Go workflows are verifiable by default and do not require a template or Docker.
...
2. **Share your workflow source** with the verifier. Provide a zip archive or repository link that includes all workflow files, including `.env.public`. Exclude `.env` files that contain private keys or secrets.
