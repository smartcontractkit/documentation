...
import batchSecretsGo from "../snippets/using-secrets-batch-go.go?raw"
...
Use `runtime.GetSecrets()` to retrieve multiple secrets in a single call. Secrets are returned in the same order as the input requests.

<CodeHighlightBlock lang="go" code={batchSecretsGo} title="Fetching Multiple Secrets with Batch (Go)" />
...
You can also call `runtime.GetSecret()` once per secret. Because the WASM runtime is single-threaded, you must call `.Await()` on each promise before requesting the next secret.

{/* prettier-ignore */}
<Aside type="caution" title="Fetch secrets sequentially">
  The WASM host for the CRE runtime does not support parallel `runtime.GetSecret()` requests. Always call `.Await()` on each promise before calling `GetSecret()` again.
</Aside>

<CodeHighlightBlock lang="go" code={multipleSecretsGo} title="Fetching Multiple Secrets Sequentially (Go)" />
