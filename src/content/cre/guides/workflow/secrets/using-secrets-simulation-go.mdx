...
<Aside type="note" title="Batch fetch is atomic">
  `runtime.GetSecrets()` fetches all requested secrets in one call. If any secret in the batch cannot be retrieved, the promise resolves with an error that includes the failing secret ids and their error messages.
</Aside>
