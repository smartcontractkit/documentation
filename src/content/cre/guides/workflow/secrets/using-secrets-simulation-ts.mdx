...
You can fetch multiple secrets in a single batch call with `runtime.getSecrets()`. This is more efficient than calling `getSecret()` repeatedly, and it avoids the sequential-fetch requirement of the single-secret API.

{/* prettier-ignore */}
<Aside type="note" title="Batch fetch is atomic">
  `runtime.getSecrets()` fetches all requested secrets in one call. If any secret in the batch cannot be retrieved, `.result()` throws a `SecretsBatchError` that includes the failing secret ids and their error messages.
</Aside>
...
<CodeHighlightBlock lang="typescript" code={multipleSecretsTs} title="Fetching Multiple Secrets (TypeScript)" />
