...
metadata:
  description: "Use secrets in simulated workflows (TypeScript): learn to declare secrets in YAML and securely access them while simulating your workflows."
  datePublished: "2025-11-04"
  lastModified: "2025-11-04"
---

import { Aside, CodeHighlightBlock } from "@components"
import singleSecretTs from "../snippets/using-secrets-single-ts.ts?raw"
import batchSecretsTs from "../snippets/using-secrets-batch-ts.ts?raw"
import multipleSecretsTs from "../snippets/using-secrets-multiple-ts.ts?raw"
...
<CodeHighlightBlock lang="typescript" code={singleSecretTs} title="Fetching Single Secret (TypeScript)" />
...
First, update your `secrets.yaml` to declare both secrets:
...
### Using batch fetch (recommended)

Use `runtime.getSecrets()` to retrieve multiple secrets in a single call. The result is a map keyed by secret ID.

<CodeHighlightBlock lang="typescript" code={batchSecretsTs} title="Fetching Multiple Secrets with Batch (TypeScript)" />

{/* prettier-ignore */}
<Aside type="note" title="All-or-nothing error handling">
...
You can also call `runtime.getSecret()` once per secret. Because the WASM runtime does not support parallel calls, you must call `.result()` before requesting the next secret.

{/* prettier-ignore */}
<Aside type="caution" title="Fetch secrets sequentially">
  The WASM host for the CRE runtime does not support parallel `runtime.getSecret()` requests. Always call `.result()` on each secret before calling `getSecret()` again.
</Aside>

<CodeHighlightBlock
  lang="typescript"
  code={multipleSecretsTs}
  title="Fetching Multiple Secrets Sequentially (TypeScript)"
/>
