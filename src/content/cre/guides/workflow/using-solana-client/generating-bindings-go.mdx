...
<Aside type="note" title="What does the WriteReport helper do?">
  The `WriteReportFromUserData` method: 1. Borsh-encodes the `UserData` struct. 1. Hashes `remainingAccounts` via
  `CalculateAccountsHash`. 1. Encodes a forwarder report and calls `runtime.GenerateReport()` with the `solana` encoder.
  1. Calls the Solana client's `WriteReport` method to submit it. 1. Returns a `Promise` that resolves with the
