...
The following IDL is enough to generate a `WriteReportFromUserData()` helper. Save it as `contracts/solana/src/idl/cre_docs_receiver.json`.
...
After running `cre generate-bindings solana`, use the generated `cre_docs_receiver` package in your workflow:
...
result, err := ds.WriteReportFromUserData(runtime, cre_docs_receiver.UserData{
	Key:   "price",
	Value: "500000",
...
The generator creates a Go package for each IDL file in `contracts/solana/src/generated/<program_name>/` (for example, `cre_docs_receiver/`).
