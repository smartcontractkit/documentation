...
1. Run the dry-run simulation.
   ...
   Confirm the workflow compiles, the cron trigger runs, and your USER LOGs appear. The simulation uses a local
   Solana runtime, not the real Devnet, so the receiver's on-chain account state is not available. The write
   completes with a receiver CPI error such as `Custom:6002`. This is expected: the mock forwarder successfully
   verifies the report, encodes the payload, and submits the transaction, but the CPI into the receiver fails because
   the local simulation environment does not have the receiver's on-chain accounts. Seeing this error confirms the
   entire write path (bindings, Borsh encoding, report generation, forwarder submission) worked end-to-end. A
   successful onchain write requires deploying your workflow to the DON with the production Keystone Forwarder (see
   [Step 5](#step-5-deploy-the-workflow)).
   ...
   {/* prettier-ignore */}
   <Aside type="note" title="What does the simulation error mean?">
     The `Custom:6002` error occurs during the mock forwarder's CPI into the receiver. The local simulation runtime
     does not replicate the receiver's on-chain account state, so the CPI cannot complete. This does not indicate a
     problem with your workflow code — it confirms the write path (bindings, encoding, report generation, forwarder
     submission) executed successfully. On production, the Keystone Forwarder submits real transactions to Solana
     where the receiver's accounts exist, and the write succeeds. See
     [Step 5](#step-5-deploy-the-workflow) and the
     [confirmed example transaction](https://explorer.solana.com/tx/2tfcA2jKvaP3JrVaURQEPuWCLtjV8ZdzMCppCUa9a41LQoG5847MzJNJmwdepzjQHxrJak8Umr4fHebkhBMGTqFM?cluster=devnet).
   </Aside>
...
Use **your** receiver program ID and accounts — not the `DataStorage` example address unless that program is actually
deployed and ready to accept writes. Update the IDL address, regenerate bindings, and set `receiverProgramId` to the
same value (Go bindings use the program ID baked into the generated package for the write receiver; config is used to
derive the forwarder authority PDA).
