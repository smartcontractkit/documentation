...
1. Run the dry-run simulation.
   ...
   completes with a receiver CPI error such as `Custom:6002` (Anchor's `AccountOwnedByWrongProgram`). This is
   expected: the mock forwarder successfully verifies the report, encodes the payload, and submits the transaction,
   but the CPI into the receiver fails because the `DataAccount` PDA does not exist in the local simulation
   environment. Seeing this error confirms the entire write path (bindings, Borsh encoding, report generation,
   forwarder submission) worked end-to-end. A successful onchain write requires deploying your workflow to the DON
   with the production Keystone Forwarder (see [Step 5](#step-5-deploy-the-workflow)).
   ...
   {/* prettier-ignore */}
   <Aside type="note" title="What does Custom:6002 mean?">
     `Custom:6002` is Anchor's `AccountOwnedByWrongProgram` error. The local simulation runtime does not have the
     receiver's on-chain account state, so when the mock forwarder CPIs into `on_report`, Anchor cannot find the
     `DataAccount` PDA owned by the receiver program. This does not indicate a problem with your workflow code. On
     production, the Keystone Forwarder submits real transactions to Solana where the account exists — the write
     succeeds. See [Step 5](#step-5-deploy-the-workflow).
   </Aside>
