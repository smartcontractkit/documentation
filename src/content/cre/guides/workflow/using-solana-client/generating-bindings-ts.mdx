...
<Aside type="note" title="CLI version requirement">
  TypeScript Solana binding generation and Solana write simulation require a current CRE CLI. Run `cre update` to get
  the latest version.
</Aside>
...
<Aside type="note" title="What does writeReportFrom do?">
  The generated method: 1. Borsh-encodes the struct with `@solana/codecs`. 1. Hashes `remainingAccounts` via
  `calculateAccountsHash`. 1. Encodes a forwarder report and calls `runtime.report()` with `prepareSolanaReportRequest`.
  1. Calls `SolanaClient.writeReport()` to submit it. 1. Returns the transaction result synchronously (after consensus).
</Aside>
