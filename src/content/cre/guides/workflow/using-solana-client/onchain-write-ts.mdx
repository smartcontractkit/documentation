...
- A current CRE CLI (`cre update`) with TypeScript Solana binding generation.
...
1. Open (or create) `config.staging.json` and add the Solana Devnet dry-run parameters.
   ...
   `["forwarder", forwarderState, receiverProgram]` under `forwarderProgramId`. When you move to Keystone in Step 5,
   keep that derivation and only swap the forwarder program/state in config.
...
1. Create `config.production.json` with Devnet Keystone forwarder addresses (same receiver and report state your program
   ...
   `["forwarder", forwarderState, receiverProgram]` under the Keystone `forwarderProgramId`.
