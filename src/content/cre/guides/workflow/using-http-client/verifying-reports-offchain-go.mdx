...
<Aside type="note" title="Using the private registry?">
  This guide uses the **Capability Registry** (DON signers), not the **workflow registry** where you deploy. [`cre.ParseReport()`](/cre/reference/sdk/core-go#creparsereport) works the same way regardless of which registry you deployed with. For HTTP-triggered receivers, the gateway URL depends on the workflow's [execution environment](/cre/guides/workflow/using-triggers/http-trigger/triggering-deployed-workflows#gateway-urls), not the registry. Local simulation may need an `ethereum-mainnet` RPC in `project.yaml` for registry reads.
</Aside>
