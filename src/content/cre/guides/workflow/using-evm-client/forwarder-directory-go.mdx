...
Solana uses a different forwarder model than EVM. Instead of a single contract address, each Solana forwarder deployment has a **program ID** and a **state account**. Your workflow config needs both values. Learn more: [Writing to Solana](/cre/guides/workflow/using-solana-client/onchain-write-go).
