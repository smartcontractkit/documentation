...
**`<ContractName>.ts`** — The main binding file, containing:

- A typed ABI constant (`<ContractName>ABI`) for use with viem utilities.
- A contract class (`<ContractName>`) with:
  - **Read methods** for each `view`/`pure` function, returning decoded values directly.
  - **`writeReportFrom<FunctionName>()`** methods for each non-view function, handling the full report-and-submit lifecycle.
  - **`logTrigger<EventName>()`** methods for each event, returning a typed trigger object.
  - **`decode<EventName>()`** methods for decoding raw `EVMLog` objects into `DecodedLog<EventDecoded>`.
- **Event types** (`<EventName>Topics` and `<EventName>Decoded`) for each event.

**`<ContractName>_mock.ts`** — A mock for testing, containing:

- A `<ContractName>Mock` type with optional overrideable methods for each view function.
- A `new<ContractName>Mock(address, evmMock)` factory function for use in workflow tests.

**`index.ts`** — A barrel file that re-exports everything from all generated bindings in the directory, so you can import from a single path:
