...
abstract contract ReceiverTemplate is IReceiver, Ownable {
    ...
    bytes10 private s_expectedWorkflowName; // Disabled by default - see setExpectedWorkflowName()
    ...
    function onReport(bytes calldata metadata, bytes calldata report) external override {
        ...
        // Security Checks 2-3: Verify workflow identity - ID and/or owner (if any are configured)
        if (s_expectedWorkflowId != bytes32(0) || s_expectedAuthor != address(0)) {
            ...
            // ====================================================================================
            // WORKFLOW NAME VALIDATION - DISABLED BY DEFAULT
            // ====================================================================================
            // Workflow name validation is disabled by default for security reasons: the workflow
            // name uses only 40-bit truncation (bytes10), making collision attacks computationally
            // feasible. Additionally, workflow names are not guaranteed to be
            // unique. Only enable this check if you fully understand these limitations and use
            // it in combination with workflowId or author validation.
            //
            // To enable this validation:
            // 1. Update the _decodeMetadata call above to extract workflowName:
            //    (bytes32 workflowId, bytes10 workflowName, address workflowOwner) = _decodeMetadata(metadata);
            // 2. Uncomment the validation check below:
    ...
    /// @dev WARNING: This validation is DISABLED BY DEFAULT in onReport().
    ///      The workflow name uses only 40-bit truncation, making collision attacks feasible.
    ///      Only enable this check if you fully understand these limitations and use it in
    ///      combination with setExpectedWorkflowId() and/or setExpectedAuthor().
...
// Example: Enable workflow name check (not recommended as sole validation)
...
- Stores the workflow name (if `expectedWorkflowName` is configured, but validation is disabled by default)
...
{/* prettier-ignore */}
<Aside type="caution" title="Workflow name validation is disabled by default">
  The workflow name check is **not enabled by default** in `ReceiverTemplate.onReport()` due to security limitations (40-bit truncation collision risk and no uniqueness guarantee). Setting the expected workflow name stores the value but does not activate the check unless you explicitly enable it in your implementation. See [Security Considerations](#7-security-considerations) for details.
</Aside>

**Usage:**

```solidity
// Set the expected workflow name (does not enable validation by default)
myConsumer.setExpectedWorkflowName("my_workflow");

// To disable/clear the stored value
...
- **`expectedWorkflowName`** (Not recommended): Disabled by default due to collision risks and non-uniqueness. See [Workflow name validation risks](#workflow-name-validation-risks) below.

### Workflow name validation risks

{/* prettier-ignore */}
<Aside type="caution" title="Workflow name validation disabled by default">
  The `expectedWorkflowName` check is **disabled by default** in `ReceiverTemplate.onReport()` because:
  
  - **Collision Risk**: Workflow names use only 40-bit truncation (bytes10), making collision attacks computationally feasible
  - **No Uniqueness Guarantee**: Workflow names are not guaranteed to be unique across the platform
  
  If you need to enable this check in your implementation, you **must** use it in combination with `expectedWorkflowId` and/or `expectedAuthor` validation. Never rely on workflow name alone.
</Aside>
...
1. **Never rely on workflow name alone** - If you enable this check, combine it with workflow ID or author validation
