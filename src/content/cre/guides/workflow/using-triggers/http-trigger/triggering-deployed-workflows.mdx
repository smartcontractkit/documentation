...
The gateway you send trigger requests to depends on the **execution environment (DON)** your workflow runs on:

| Execution environment | Gateway URL                                            |
| --------------------- | ------------------------------------------------------ |
| Public DON            | `https://01.gateway.zone-a.cre.chain.link`             |
| Enterprise DON        | `https://01.enterprise-gateway.zone-a.cre.chain.link/` |

To determine which gateway to use:

- **Public DON**: The default execution environment available to all CRE users. Whether you deploy your workflow
  onchain or to the private registry, it runs on the public DON and is triggered through the public gateway.
- **Enterprise DON**: A separate, dedicated execution environment provisioned for organizations with an Enterprise
  agreement with Chainlink. Only workflows deployed to the Enterprise DON are triggered through the enterprise
  gateway.
...
Here's what a complete curl request looks like (using the public DON gateway — see [Gateway URLs](#gateway-urls) to pick the right endpoint for your workflow's execution environment):
