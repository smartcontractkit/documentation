...
Find your production HTTP trigger endpoint in the UI under [Endpoints](https://app.chain.link/organization?otab=configuration). To see your environment, run `cre whoami` in the CLI.

Here are the trigger endpoints per environment:

| Environment    | Trigger endpoint                                       |
| -------------- | ------------------------------------------------------ |
| CRE Public     | `https://01.gateway.zone-a.cre.chain.link`             |
| CRE Enterprise | `https://01.enterprise-gateway.zone-a.cre.chain.link/` |
