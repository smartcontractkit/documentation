...
You create an HTTP trigger by calling the [`http.Trigger`](/cre/reference/sdk/triggers/overview#httptrigger) function. Its configuration ([`http.Config`](/cre/reference/sdk/triggers/overview#httpconfig)) requires a set of authorized public keys to validate incoming request signatures.
