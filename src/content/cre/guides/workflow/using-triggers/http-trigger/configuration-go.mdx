...
You create an HTTP trigger by calling the [`http.Trigger`](/cre/reference/sdk/triggers/http-trigger-go#httptrigger) function. Its configuration ([`http.Config`](/cre/reference/sdk/triggers/http-trigger-go#httpconfig)) requires a set of authorized public keys to validate incoming request signatures.
...
The HTTP trigger passes a [`*http.Payload`](/cre/reference/sdk/triggers/http-trigger-go#httppayload) to your callback. This object contains the request body (`Input`) and the signing key (`Key`) from the incoming HTTP request.
