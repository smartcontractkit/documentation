...
{/* prettier-ignore */}
<Aside type="note" title="Gateway URL depends on your execution environment">
  The `GATEWAY_URL` depends on the execution environment (DON) your workflow runs on, not on which registry it was
  deployed to. A workflow deployed to the private registry still runs on the public DON, so it uses the public gateway.
  See [Gateway URLs](/cre/guides/workflow/using-triggers/http-trigger/triggering-deployed-workflows#gateway-urls) to
  pick the right endpoint for your workflow.
</Aside>
