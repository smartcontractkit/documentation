...
<Aside type="note" title="One HTTP trigger per workflow">
  Each workflow supports only one HTTP trigger handler. There is currently no mechanism to route requests to different
  HTTP trigger handlers within the same workflow. If you need to handle different use cases or execution paths, include
  routing logic in your request payload and branch within a single handler function.
</Aside>
...
| **Endpoint**       | Local simulator                 | CRE gateway (see [Gateway URLs](/cre/guides/workflow/using-triggers/http-trigger/triggering-deployed-workflows#gateway-urls)) |
