...
- **Regulatory compliance:** Some industries require that sensitive data not be exposed to multiple parties.
- **Credential security:** High-risk API credentials could cause damage if compromised or exposed in node memory.
- **Response privacy:** API responses may contain sensitive fields that are not intended to be visible to the decentralized network.
...
1. **Enclave execution:** Secrets are threshold decrypted using Vault DON. Decryption shares are injected to a secure enclave such that only the secure enclave can combine them to extract plaintext secrets to use in the HTTP request execution—credentials are designed not to exist in node-accessible memory outside the enclave.
...
- **Credential isolation:** Secrets are encrypted at rest and in transit, and are intended to be decrypted only inside the enclave, not in node memory.
...
| :----------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Secrets** (API keys, tokens) | Designed to remain confidential. Stored in the Vault DON, decrypted only inside the enclave.                                                                                                 |
| **Request body**               | Template-based injection: secrets referenced in the request body (e.g., `{{.myApiKey}}`) are resolved inside the enclave, so sensitive values are designed not to appear in workflow memory. |
...
A SaaS service fetches user data from an e-commerce provider API. The same credential could also modify user data if misused. Confidential HTTP is designed so the credential is decrypted only inside the enclave and not accessible in node memory.
...
A workflow processes card transactions where the request contains card details and the API key provides processor access. Confidential HTTP is designed to keep both the credentials and card details private through template-based injection, returning only the encrypted response to the workflow.

### Single execution by design
...
Confidential HTTP takes a different approach: by design, only one request is made from the enclave after quorum is reached on the request parameters, intended to provide single execution.

## When to use Confidential HTTP vs. regular HTTP

### Use Confidential HTTP when:

- The API response contains sensitive data that is not intended to be visible to the network
- API credentials must not be accessible in node memory (enclave-only access)
...
<Aside type="note" title="Need to protect more than a single request?">
  Confidential HTTP does not support complex data transformations within the enclave. [Confidential
  Workflows](/cre/concepts/confidential-workflows) (beta) let you run your handler's full callback logic inside a secure
  enclave, not just a single request.
</Aside>
...
- **[Confidential Workflows in CRE](/cre/concepts/confidential-workflows)**: Run your full handler logic inside a secure enclave (beta).
