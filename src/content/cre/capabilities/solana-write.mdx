...
<Aside type="note" title="CLI and simulation">
  Solana write requires **CRE CLI v1.24.0 or later**. Local `cre workflow simulate` dry-runs the write path against the
  Devnet forwarder built into the CLI. Deployed workflows use the live Keystone Forwarder on your target network.
</Aside>
