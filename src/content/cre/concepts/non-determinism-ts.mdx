...
| `Math.random()` (outside CRE runtime) | `Math.random()` inside the CRE WASM runtime  |
...
JavaScript's `Math.random()` generates different random sequences on each node, making it impossible to reach consensus on values that depend on randomness.

**The problem:** Each node generates different random values, breaking consensus.

**The solution:** Use `Math.random()` inside the CRE WASM runtime, which provides consensus-safe random number generation. The CRE runtime overrides `Math.random()` with a seeded generator so all nodes produce the same sequence, enabling consensus. See [Using Randomness in Workflows](/cre/guides/workflow/using-randomness-ts) for details.
