...
`Date.now()` reads the local system clock, which differs slightly on each node. This breaks consensus—nodes may execute different code paths and fail to agree on the workflow result. Think of it like using `Math.random()`: it introduces non-determinism that's incompatible with CRE's consensus model.
