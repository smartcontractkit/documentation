...
- **`getSecret(...)`**: Access to a single workflow secret
- **`getSecrets(...)`**: Access to multiple workflow secrets in a single batch call
...
Fetches multiple secrets from the Vault DON in a single batch call. This is more efficient than calling `getSecret()` repeatedly, and it avoids the sequential-fetch requirement of the single-secret API.

**Signature:**

```typescript
runtime.getSecrets(
  requests: Array<SecretRequest | SecretRequestJson>
): { result: () => Record<string, Secret> }
```

The result is a record keyed by each secret's `id`. Each value is a [`Secret`](#secret).

**Behavior:**

- **Throws on any failure**: If any secret in the batch cannot be retrieved, `.result()` throws a `SecretsBatchError` that includes the failing secret ids and their error messages.
- **Duplicate ids rejected**: Requesting the same `id` more than once in a single call throws a `SecretsBatchError` (`duplicate secret id requested: <id>`), because the result is keyed by id.
- **Empty input**: Passing an empty array returns an empty record.
...
Retrieves multiple secrets in a single batch call. Use this instead of calling `getSecret()` in a loop when you need two or more secrets.
...
| Field      | Type                                        | Required | Description                                               |
| ---------- | ------------------------------------------- | -------- | --------------------------------------------------------- |
| `requests` | `Array<SecretRequest \| SecretRequestJson>` | Yes      | Array of secrets to fetch. Duplicate IDs are not allowed. |

Each entry in `requests` follows the same [`SecretRequest`](#secretrequest--secretrequestjson) shape as `getSecret()`.

**Returns:**

A `Record<string, Secret>` (after calling `.result()`) where each key is the secret's `id` and the value is the corresponding [`Secret`](#secret) object.

**Error handling:**

If any secret in the batch fails, `getSecrets()` throws a `SecretsBatchError` containing all error messages aggregated from the failed secrets.
...
const onTrigger = (runtime: Runtime<Config>): string => {
  const secretsToFetch = [{ id: "api-key" }, { id: "webhook-url" }]

  const secrets = runtime.getSecrets(secretsToFetch).result()

  const apiKey = secrets["api-key"].value
  const webhookUrl = secrets["webhook-url"].value

  runtime.log(`Fetched ${Object.keys(secrets).length} secrets`)
  return "done"
}
```
