...
`BaseRuntime<C>` provides `config`, `now()`, `log()`, and `emitMetric()`, same as on a regular `Runtime<C>`. `SecretsProvider` provides `getSecret()`.

<Aside type="note" title="No batch getSecrets in TypeScript">
  Unlike the Go SDK (which has both `GetSecret` and a batch `GetSecrets`), the TypeScript `SecretsProvider` currently
  exposes only `getSecret()`. Fetch secrets one at a time.
</Aside>
