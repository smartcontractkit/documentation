...
- **The Trigger**: An instance of a trigger capability (e.g., `cron.Trigger(...)`). This defines the event that will start your workflow. See the [Triggers reference](/cre/reference/sdk/triggers/overview-go) for details.
...
<Aside type="note" title="Callback Function Signatures">
  Each trigger type requires a specific callback signature. See the [Triggers
  reference](/cre/reference/sdk/triggers/overview-go) for the exact signature required for each trigger type.
</Aside>
...
type SecretsProvider interface {
    ...
    GetSecrets(reqs []*SecretRequest) Promise[[]*Secret]
}
```
...
Retrieves multiple secrets in a single batch call. Use this instead of calling `GetSecret()` multiple times when you need two or more secrets.

**Signature:**

```go
runtime.GetSecrets(reqs []*SecretRequest) Promise[[]*Secret]
```
...
| Field  | Type               | Required | Description                                                                              |
| ------ | ------------------ | -------- | ---------------------------------------------------------------------------------------- |
| `reqs` | `[]*SecretRequest` | Yes      | Slice of secret requests to fetch. Each entry requires an `Id` and optional `Namespace`. |

Each entry follows the same [`SecretRequest`](#secretrequest) shape as `GetSecret()`.

**Returns:**

A `Promise[[]*Secret]` that resolves to a slice of `*Secret` values in the **same order as the input requests**. Use the index to map results back to the original requests.

**Error handling:**

If any secret in the batch fails, `GetSecrets()` returns an error containing all error messages from the failed secrets. Either all secrets are returned successfully or an error is returned.
...
func onTrigger(config *Config, runtime cre.Runtime, ...) (string, error) {
    ...
    webhookUrl := secrets[1].Value

    logger := runtime.Logger()
    logger.Info("Secrets retrieved", "endpoint", webhookUrl)
    _ = apiKey
    return "done", nil
}
```
...
<Aside type="note" title="Non-determinism">
