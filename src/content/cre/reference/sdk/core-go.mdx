...
The `SecretsProvider` interface is embedded in `Runtime`, which means `GetSecret` and `GetSecrets` are available directly on the runtime object. It is also passed as a parameter to [`InitWorkflow`](#initworkflow), allowing you to access secrets during workflow initialization.
...
The promise resolves to a slice of [`Secret`](#secret) values, in the same order as the requests.

**Behavior:**

- **Returns an error on any failure**: If any secret in the batch cannot be retrieved, the promise resolves with an error that includes the failing secret ids and their error messages.
- **Empty input**: Passing `nil` or an empty slice returns a promise that resolves to an empty slice.
