...
| [`client.SendRequest`](#clientsendrequest)                          | Complex scenarios requiring fine control  | [GET](/cre/guides/workflow/using-http-client/get-request-go#2-the-creruninnodemode-pattern-low-level)                   |
...
| `MultiHeaders`                         | `map[string]*HeaderValues`            | Optional HTTP headers (recommended). Supports multiple values per header name. See [`HeaderValues`](#http-headervalues) below. |
| `Headers`                              | `map[string]string`                   | **Deprecated.** Use `MultiHeaders` instead. Single-value headers only.                                                         |
...
| `MultiHeaders` | `map[string]*HeaderValues` | The HTTP response headers, with multiple values per name where applicable. |
| `Headers`      | `map[string]string`        | **Deprecated.** Use `MultiHeaders` instead.                                |
...
Holds one or more values for a single HTTP header name. Used as the value type in `MultiHeaders` on both requests and responses.

| Field    | Type       | Description                                    |
| -------- | ---------- | ---------------------------------------------- |
| `Values` | `[]string` | One or more string values for the header name. |
...
req := &http.Request{
    ...
    MultiHeaders: map[string]*http.HeaderValues{
        ...
        "X-Custom-Key": {Values: []string{"value1", "value2"}},
    },
}
```
...
- [GET Requests](/cre/guides/workflow/using-http-client/get-request-go#1-the-httpsendrequest-pattern-recommended)
- [POST Requests](/cre/guides/workflow/using-http-client/post-request-go#1-the-httpsendrequest-pattern-recommended)
...
- [Using `cre.RunInNodeMode`](/cre/guides/workflow/using-http-client/get-request-go#2-the-creruninnodemode-pattern-low-level)
