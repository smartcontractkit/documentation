...
date: Last Modified
metadata:
  description: "Configure your Go project: learn to manage project.yaml, workflow.yaml, and targets for different environments."
...
| **Workflow**                          | A sub-folder that contains everything needed to run _one_ workflow (code, build artifacts, and the optional `workflow.yaml`).                                                                      |
...
| **ABI**                               | Application Binary Interface - a JSON file that describes a smart contract's functions, events, and data structures.                                                                               |
| **Bindings**                          | Type-safe Go packages automatically generated from contract ABIs, enabling easy interaction with smart contracts from workflows.                                                                   |
...
<Aside type="note" title="Available chain names">
  ...
  Selectors](/cre/reference/sdk/evm-client-go#chain-selectors).
</Aside>
...
If you created your project with an older CLI version, update the SDK in your `go.mod`:

```bash
go get github.com/smartcontractkit/cre-sdk-go@v1.1.4
```
...
- **SDK version:** Check `go.mod` for the `cre-sdk-go` dependency version
