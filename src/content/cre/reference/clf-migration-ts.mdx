...
Important: **DON-hosted CLF secrets cannot be exported.** When migrating, you must re-supply API keys and tokens through the CRE Vault. Fetch secrets sequentially (one `await` at a time) rather than in parallel (e.g., using `Promise.all()`). This ensures the DON can reach consensus on each secret retrieval reliably within the WASM runtime.
