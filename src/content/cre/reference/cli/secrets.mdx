...
1. The CLI starts a local callback server and opens your browser to a Vault authorization URL.
1. You authenticate in the browser. When complete, the browser redirects back to the local callback.
1. The CLI exchanges the authorization code for a short-lived vault JWT and submits your encrypted secrets directly to the Vault DON gateway (CLI v1.8.2+).
