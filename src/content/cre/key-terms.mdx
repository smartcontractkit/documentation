...
A hardware-isolated secure computing environment intended to prevent code and data from being inspected by the host machine or its operator during execution. CRE's confidential features run sensitive logic inside a TEE so that it is intended not to be visible to node operators while it executes.

### Enclave

A single running TEE instance that executes confidential workflow logic or capability requests. "TEE" describes the technology; "enclave" refers to a specific running instance of one.

### Vault DON

The [DON](#decentralized-oracle-network-don) responsible for storing threshold-encrypted secrets, designed to release them only to an attested [enclave](#enclave) for confidential execution—secrets are intended not to be disclosed in plaintext to a node operator or to Workflow DON node memory.

### Confidential workflow or confidential execution

Execution that uses a confidential runtime, [TEE](#tee-trusted-execution-environment), [enclave](#enclave), or confidential capability to protect data during execution. This is separate from registry selection. Confidential execution does not require the private registry, and a workflow can use confidential execution while still being managed through the public onchain registry.

- **[Confidential HTTP](/cre/capabilities/confidential-http)**: Executes a single outbound HTTP request inside an enclave, with secrets injected via templates and an optional encrypted response. Available in Go and TypeScript.
- **[Confidential Workflows](/cre/concepts/confidential-workflows)** (beta): A workflow type that runs a handler's full callback logic inside an enclave, registered with `cre.HandlerInTee`/`handlerInTee` and given a `cre.TeeRuntime`/`TeeRuntime`. Secrets are fetched dynamically inside the enclave with `runtime.GetSecret()`/`runtime.getSecret()`, and `runtime.UsingTheDons()`/`runtime.usingTheDons()` crosses back to a regular DON runtime for anything that needs Workflow DON consensus. The workflow's own source code and deployed binary are not protected by this—only the marked computation, secrets, and sensitive inputs it processes inside the enclave are.
...
- **[About CRE](/cre/overview)**: Learn more about the vision and high-level architecture of CRE.
