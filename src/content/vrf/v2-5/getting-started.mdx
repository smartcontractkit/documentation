...
<p style="text-align:center">VRF v2.5 - Developer Walkthrough</p>
...
- [`IVRFCoordinatorV2Plus`](https://github.com/smartcontractkit/chainlink/blob/develop/contracts/src/v0.8/vrf/dev/interfaces/IVRFCoordinatorV2Plus.sol) that must be imported to communicate with the VRF coordinator.
...
- `IVRFCoordinatorV2Plus COORDINATOR`: The address of the Chainlink VRF Coordinator contract that this contract will use. Initialized in the `constructor`.
...
The coordinator and subscription id must be initialized in the `constructor` of the contract. To use `VRFConsumerBaseV2Plus` properly, you must also pass the VRF coordinator address into its constructor.
