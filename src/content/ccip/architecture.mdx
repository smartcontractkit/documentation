...
{/* prettier-ignore */}
<Tabs client:visible>
  <Fragment slot="tab.1">With CCT enabled</Fragment>
  <Fragment slot="tab.2">Without CCT</Fragment>
...
The `TokenAdminRegistry` contract is responsible for managing the configuration of token pools for all cross chain tokens. It allows projects to autonomously register and administer their tokens without direct intervention from Chainlink Labs. The contract enables projects to configure their token pools for cross-chain transactions.
