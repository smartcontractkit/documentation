...
{/* prettier-ignore */}
<Tabs client:visible>
  <Fragment slot="tab.1">Testnet</Fragment>
...
- Interacts with the [TokenAdminRegistry](#tokenadminregistry) to retrieve the token pool linked to a token if the message includes a token transfer. **Note**: This contract is part of the new CCIP architecture, which is currently only available on testnet. The new CCIP architecture has not yet been deployed on mainnet
...
The `TokenAdminRegistry` contract is responsible for managing the configuration of token pools for all CCIP enabled tokens. It enables autonomous token management, allowing projects to autonomously register and administer their tokens without direct intervention from the CCIP owner. The contract enables projects to configure their token pools for cross-chain transactions.

**Key Features:**

- **Autonomous Tokens Registration:** Projects can register tokens and configure token pools independently without CCIP owner intervention.
- **Token Pool Management:** Projects can set or update the token pool for any registered token. This enables the token to be used in CCIP cross-chain transactions. A token pool can be configured, replaced, or delisted.
