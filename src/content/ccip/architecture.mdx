...
Chainlink CCIP enforces two rate limits. Both limits have a _refill rate_, the speed at which the maximum capacity is restored. The rate limits are enforced at the source and destination for security reasons.
If these rate limits are reached, specific errors are generated. CCIP users can manage these errors within their dApps to enhance the experience for their end-users. A comprehensive list of errors and their descriptions is available on the [errors API reference](/ccip/api-reference/errors#ratelimiter) page.

### Token pool rate limit

For each supported token, each individual [lane](/ccip/concepts#lane) has a limit on the total number of tokens that it can transfer. This limit is independent of the USD value of the tokens.

For example, let's say 100 LINK is the capacity of the LINK token pool on the Ethereum Sepolia => Optimism Goerli Lane, and the refill rate is 1 LINK per second. If 100 LINK is transferred on that lane, the whole capacity is used up. After that, if a user wants to send 10 LINK, they must wait 10 seconds.

### Aggregate rate limit

Each [lane](/ccip/concepts#lane) also has an overall USD value limit that can be transferred across all supported tokens. The total USD value of tokens is calculated using [Chainlink Data Feeds](/data-feeds). To improve security, the aggregate rate limit for any given lane is always lower than all token pools' combined rate limits.

For example, suppose the Ethereum Sepolia => Optimism Goerli Lane has a capacity of 100,000 USD and a refill rate of 100 USD per second, and several token transfers totaling 60,000 USD have been executed. In that case, the remaining capacity is 40,000 USD. So, if a user intends to transfer tokens equating to 50,000 USD, they would have to wait an additional 100 seconds.
