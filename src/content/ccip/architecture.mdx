...
{/* prettier-ignore */}
<Tabs client:visible>
  <Fragment slot="tab.1">With CCIP-Enabled tokens</Fragment>
  <Fragment slot="tab.2">Without CCIP-Enabled tokens</Fragment>
...
The `TokenAdminRegistry` contract is responsible for managing the configuration of token pools for all CCIP enabled tokens. It allows projects to autonomously register and administer their tokens without direct intervention from Chainlink Labs. The contract enables projects to configure their token pools for cross-chain transactions.

**Key Features:**

- **Autonomous Tokens Registration:** Projects can register tokens and configure token pools independently without Chainlink Labs intervention.
...
- **Admin Role Transfer:** Administrators can transfer their role to another address via a two-step process. First, the current administrator initiates the transfer by specifying the new address. Then, the new administrator must accept the admin role to finalize the transfer.
