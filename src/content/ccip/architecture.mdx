...
<ClickToZoom src="/images/ccip/ccip-diagram-04_v04.webp" alt="Chainlink CCIP Detailed Architecture" />
...
For example, the maximum capacity of the snxUSD token pool on the [Mainnet Ethereum → Optimism lane](/ccip/supported-networks#optimism-mainnet--ethereum-mainnet-lane) is 100,000 snxUSD. The refill rate is 167 snxUSD per second. If 100,000 snxUSD is transferred on that lane, the entire capacity is consumed. After that, if a user wants to send 1670 snxUSD, they must wait at least 10 seconds as the capacity refills. The maximum throughput for this token on the lane is 100,000 snxUSD every 10 minutes.
