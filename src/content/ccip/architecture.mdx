...
Chainlink CCIP has been purposely designed to take a security-first approach to minimize the risk of block reorgs on the source blockchain. The end-to-end transaction time of a CCIP cross-chain transaction largely depends on the time it takes for the transaction on the source chain to reach finality (the assurance that past transactions included on-chain are extremely difficult, if not impossible, to revert). The time to reach finality varies by blockchain. For example, on Ethereum, it takes about 15 minutes for a block to be 'finalized'. See the [etereum.org finality](https://ethereum.org/en/developers/docs/consensus-mechanisms/pos/#finality) documentation to learn more about finality on Ethereum. When cross-chain transactions are initiated from a chain with faster finality, such as Avalanche, which has a time-to-finality of around one second, the end-to-end transaction time is faster.
