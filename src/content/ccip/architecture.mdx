...
Execution delays in excess of one hour should be rare as a result of _Smart Execution_. The smart execution timeframe parameter represents the waiting time before manual execution is enabled. If the DON fails to execute within the duration of smart execution timeframe due to extreme network conditions, you can manually execute the message through the [CCIP Explorer](https://ccip.chain.link/). Read the [manual execution](/ccip/concepts/manual-execution) conceptual guide to learn more.
