...
**Finality** is the assurance that past transactions included on-chain are extremely difficult, if not impossible, to revert. If the parameters for finality are properly set, the likelihood of irreversibility is extremely low. For CCIP, source chain finality is the main factor that determines the end-to-end elapsed time for CCIP to send a message from one chain to another.
