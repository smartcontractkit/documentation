...
section: ccip
date: Last Modified
title: "Faster-Than-Finality (FTF) - Token Issuers"
metadata:
  description: "Learn how token issuers enable and manage Faster-Than-Finality (FTF) on CCIP 2.0token pools, including confirmation floors, rate limits, and fees."
  excerpt: "FTF token issuer allowedFinality rate limits FTF fees V2 token pool"
...
Because FTF transfers carry a different risk profile than standard finality transfers, CCIP 2.0token pools maintain dedicated rate limit buckets for each:
...
| :----------------- | :----------------------------------------------------------------------- | :----------------------------------------- |
| FTF transfers      | `fastFinalityTransferFeeBps`                                             | `fastFinalityFeeUSDCents`                  |
...
**To reiterate:** none of these mechanisms activate unless the token issuer explicitly enables FTF on a V2 pool. Existing V1 pools and new V2 pools with default settings both enforce full finality.
