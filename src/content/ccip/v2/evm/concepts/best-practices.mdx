...
metadata:
  description: "Security and reliability guide for CCIP 2.0on EVM chains: message verification, extraArgs configuration, gas optimization, CCT administration, lockbox liquidity management, and multi-signature protection for cross-chain applications."
  excerpt: "CCIP 2.0evm best practices security message verification extraargs gas limit token administration lockbox liquidity multi-signature ccip receiver defensive programming"
...
CCIP 2.0accepts multiple `extraArgs` formats on EVM lanes. New integrations should use `ExtraArgsV3`. Existing v1.x-style applications can continue to send legacy formats: the OnRamp normalizes them internally before building the message.
