...
section: ccip
date: Last Modified
title: "Configure a sender allowlist with AdvancedPoolHooks using Hardhat"
metadata:
  description: "Deploy and attach AdvancedPoolHooks to a CCIP 2.0 token pool to enforce a sender allowlist using Hardhat. Send a baseline transfer, attach hooks, remove your address from the allowlist to force an expected failure, then detach hooks and send again."
...
whatsnext:
  "Enforce ACE policies on CCIP token transfers using Hardhat": "/ccip/evm/tutorials/cross-chain-tokens/enforce-ace-policies-hardhat"
