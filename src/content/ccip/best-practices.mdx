...
metadata:
  description: "Learn essential Chainlink CCIP best practices for secure cross-chain dApp development. Covers verification, gas limits, token admin, liquidity, and multisig."
...
Before calling the router's `ccipSend` [function](/ccip/api-reference/evm/v1.5.1/i-router-client#ccipsend), ensure that your code allows users to send CCIP messages to trusted destination chains.
...
When implementing the `ccipReceive` [method](/ccip/api-reference/evm/v1.5.1/ccip-receiver#ccipreceive) in a contract residing on the destination chain, ensure to verify the source chain of the incoming CCIP message. This verification ensures that CCIP messages can only be received from trusted source chains.
...
When implementing the `ccipReceive` [method](/ccip/api-reference/evm/v1.5.1/ccip-receiver#ccipreceive) in a contract residing on the destination chain, it's important to validate the sender of the incoming CCIP message. This check ensures that CCIP messages are received only from trusted sender addresses.
...
When you implement the `ccipReceive` [method](/ccip/api-reference/evm/v1.5.1/ccip-receiver#ccipreceive) in the contract residing on the destination chain, validate that the `msg.sender` is the correct router address. This verification ensures that only the router contract can call the `ccipReceive` function on the receiver contract and is for developers that want to restrict which accounts are allowed to call `ccipReceive`.
...
The purpose of [`extraArgs`](/ccip/api-reference/evm/v1.5.1/client#evmextraargsv2) is to allow compatibility with future CCIP upgrades. To get this benefit, make sure that `extraArgs` is mutable in production deployments. This allows you to build it offchain and pass it in a call to a function or store it in a variable that you can update on-demand.
...
The `allowOutOfOrderExecution` parameter enables you to control the execution order of your messages on the destination blockchain. This parameter is part of [`EVMExtraArgsV2`](/ccip/api-reference/evm/v1.5.1/client#evmextraargsv2) and is available only on lanes where the **Out of Order Execution** property is set to **Optional** or **Required**. Refer to the [CCIP Directory](/ccip/directory) to determine if your target lane supports this feature.
...
In pools like the **LockReleaseTokenPool**, liquidity providers can add and remove liquidity using the [`provideLiquidity`](/ccip/api-reference/evm/v1.5.1/lock-release-token-pool#provideliquidity) and [`withdrawLiquidity`](/ccip/api-reference/evm/v1.5.1/lock-release-token-pool#withdrawliquidity) functions.
