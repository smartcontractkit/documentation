...
## Best Practices for Cross-Chain Tokens (CCT) Administration
...
The **token admin** role configures token pools across blockchains and enables cross-chain operations. This role allows the token admin to set the token pool for a token on each supported CCIP blockchain and manage cross-chain transfers. To ensure security, follow these guidelines:
...
When a **LockReleaseTokenPool** operates in **reception mode** (on the destination blockchain), it releases the tokens locked in the token pool. The pool must have sufficient liquidity to ensure that tokens can be released to the receiver. If the pool lacks liquidity, the release operation will fail.
...
In pools like the **LockReleaseTokenPool**, liquidity providers can add and remove liquidity using the [`provideLiquidity`](/ccip/api-reference/lock-release-token-pool#provideliquidity) and [`withdrawLiquidity`](/ccip/api-reference/lock-release-token-pool#withdrawliquidity) functions.
...
Using a **Lock and Unlock** mechanism across multiple issuing blockchains can lead to fragmented liquidity, making it harder to maintain sufficient liquidity in each pool.
...
- **Best Practice**: Set up automated alerts to monitor the token pool’s balance and ensure it remains above a threshold that supports the expected volume of cross-chain transfers.
