...
<Aside type="caution" title="WE HIGHLY RECOMMEND NOT USING PLAINTEXT PRIVATE KEYS">
  It is not recommended to store your sensitive keys in plaintext in `.env` files.
  In this tutorial we use in-built keystores for both Hardhat and Foundry for their
  respective projects.
...
<Accordion title="Read data" number={3}>
