...
<Aside type="note" title="LINK token Interface">
  1. You will notice that we create a minimal interface for the `LINK` token as part of our script. 2. Another option
  could have been to generate a compiled artifact out of an empty contract that extends the `LinkTokenInterface` from
  the `@chainlink/contracts` package. 3. Which approach do you prefer?
</Aside>
