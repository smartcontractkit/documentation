...
1. **Deploying Token Pools**: Once your tokens are deployed, you will deploy [`BurnMintTokenPool`](/ccip/v1/evm/api-reference/v1.6.1/burn-mint-token-pool) token pools on Ethereum Sepolia and BASE Sepolia. The Safe account will own each token pool.
...
1. **Linking Tokens to Pools**: You will use the Safe account to call the [`TokenAdminRegistry`](/ccip/v1/evm/api-reference/v1.6.1/token-admin-registry) contract's [`setPool`](/ccip/v1/evm/api-reference/v1.6.1/token-admin-registry#setpool) function to associate each token with its respective token pool.
...
1.  **Clone the repository and navigate to the project directory:**
    ...
    cd smart-contract-examples/ccip/v1/cct/hardhat
    ```
...
<Aside type="note" title="Using Different Networks">
  ...
  Tutorials](/ccip/v1/evm/tutorials/cross-chain-tokens/configure-additional-networks-hardhat).
</Aside>
