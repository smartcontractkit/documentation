...
metadata:
  description: "Use CCIP Token Manager to deploy Cross‑Chain Tokens (CCT), enable existing ERC‑20 tokens, configure pools, register admins, update limits, and request verification."
  image: "/images/ccip/token-manager/new/start-0.png"
  excerpt: "Token Manager, CCT, ERC‑20, ERC20, deploy, enable, token pools, admin, verification, CCIP, wallet, MetaMask, Safe Wallet, WalletConnect"
...
If you prefer to manage your deployments and configurations programmatically, refer to the [Cross-Chain Tokens](/ccip/v1/evm/tutorials/cross-chain-tokens) guides available for Remix, Hardhat and Foundry.
...
1. On the **Pools** page, you're prompted to select the token pool mechanism that's used to transfer value between networks &mdash; _Burn / Mint_ or _Lock / Release_. Depending on your token contract, you may have more than one option for _Burn / Mint_:
   ...
   Before selecting a token pool type, be sure to review [CCIP token handling mechanisms](/ccip/v1/concepts/cross-chain-token/overview#token-handling-mechanisms).
...
1. The **Deploy** page displays the steps that you need to complete.
   ...
   <Aside type="caution" title="Manually granting Burn/Mint privileges">
     ...
     Review the [CCT requirements](/ccip/v1/evm/concepts/cross-chain-token/tokens) to understand the requirements your
     token must fulfill.
   </Aside>
