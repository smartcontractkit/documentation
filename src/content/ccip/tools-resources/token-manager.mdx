...
The CCIP Token Manager is a web interface designed to simplify the deployment, configuration, and management of Cross-Chain Tokens (CCTs) for use with Chainlink CCIP.
...
<Aside type="note">
  While the Token Manager simplifies many tasks, certain advanced configurations (like deploying Lock/Unlock pools or
  custom pools) or upgrades might still require manual processes or programmatic interaction. Refer to the [Using the
  Token Manager tutorial](/ccip/tutorials/evm/token-manager) for detailed steps and current limitations.
</Aside>
