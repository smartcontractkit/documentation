...
<Aside title="Encoding a TON Address">
  Parse the TON address with `Address.parse()` from `@ton/core`, then encode it into the 36-byte format expected by the
  EVM CCIP Router:
...
function encodeTONAddress(addr: Address): Uint8Array {
