...
If you need to use different networks, you can find more faucets on the [LINK Token Contracts](/resources/link-token-contracts) page.
...
1. **Sending and Building a CCIP Message:** You'll initiate and send a message from the _message tracker_ contract on Ethereum Sepolia to the _acknowledger_ contract on Polygon Mumbai. The _message tracker_ contract constructs a CCIP message that encapsulates the string text and establishes a tracking status for this message before sending it off.
...
Deploy the `MessageTracker.sol` contract on _Ethereum Sepolia_.
...
1. Deploy the contract on _Ethereum Sepolia_:
   ...
   1. Open MetaMask and send <CopyText text="3" code/> LINK to the contract address you copied. Your contract will pay CCIP fees in LINK.
...
Deploy the _acknowledger_ contract on _Polygon Mumbai_. You will use this contract to receive data from and send receipt acknowledgment to the _message tracker_ contract you deployed on _Ethereum Sepolia_.
...
1. Deploy the contract on _Polygon Mumbai_:
   ...
   1. Under the **Deploy** section, fill in the constructor arguments:
      ...
      - For _Polygon Mumbai_, the Router address is <CopyText text="0x1035cabc275068e0f4b745a29cedf38e13af41b1" code/>. You can find the address for each network on the [Supported Networks](/ccip/supported-networks) page.
      - The LINK token address is <CopyText text="0x326C977E6efc84E512bB9C30f76E30c160eD06FB" code/>. You can find the address for each network on the [LINK Token Contracts](/resources/link-token-contracts) page.
      - The _message tracker_ chain selector, deployed on _Ethereum Sepolia_, is <CopyText text="16015286601757825753" code/>. You can find each network's chain selector on the [Supported Networks](/ccip/supported-networks) page.
...
The `acknowledgePayLINK` function acts as a custom [`sendMessagePayLINK`](https://docs.chain.link/ccip/tutorials/send-arbitrary-data#sending-data-and-pay-in-link) function nested within the `ccipReceive` function of the _acknowledger_ contract and sends an acknowledgment (a CCIP message) back to the _message tracker_ contract upon reception of the initial message. This acknowledgment message contains the initial message ID.
...
Upon reception of the acknowledgment message, the _message tracker_ `ccipReceive` function:

    - Updates the initial message status to `2`, which corresponds to the `Confirmed` status as defined by the `MessageStatus` `enum`
    - Stores the acknowledged message ID in the `s_lastReceivedAcknowledgedMsgId` variable
    - Emits a `MessageConfirmed` event

1. On the **Deploy & Run Transactions** tab in Remix, expand your _message tracker_ contract in the **Deployed Contracts** section.

1. Expand the **messagesStatus** getter function and paste the initial message ID. Click **call** to read the message status. It returns `2`, which corresponds to the `Confirmed` status.
...
1. Click the **getLastReceivedMessageDetails** getter function. You see the acknowledgment message ID (`s_lastReceivedMessageId`) and the acknowledged message ID (`s_lastReceivedAcknowledgedMsgId`).

   <ClickToZoom
     src="/images/ccip/tutorials/message-tracker-getLastReceivedMessageDetails.webp"
     alt="Chainlink CCIP - Message Tracker Get Last Received Message Details"
     style="max-width: 70%;"
   />
...
1. Copy your _message tracker_ contract address from Remix. Open the [_Ethereum Sepolia_ explorer](https://sepolia.etherscan.io/) and search for your deployed _message tracker_ contract. Then, click on the **Events** tab.
   ...
   The `MessageConfirmed` event is emitted with the acknowledged message ID `0x1a09b4ce57fe52913b08867fc18cd9c263cb2c332a92fb19ccd44b90125e38b1`.
