...
1. Send a `Hello World!` string from your _message tracker_ contract on _Ethereum Sepolia_ to your _acknowledger_ contract deployed on _Polygon Mumbai_. You will track the status of this message during this tutorial.
   ...
   1. Click **transact** to call the function. MetaMask prompts you to confirm the transaction.
      ...
      <Aside type="note">
        During gas price spikes, your transaction might fail, requiring more than _2 LINK_ to proceed. If your
...
- **Status tracking**:
  ...
  - Upon receiving an acknowledgment message from the _acknowledger_ contract, the _message tracker_ contract updates the message status from `Sent` (status `1`) to `Confirmed` (status `2`). This update indicates that the cross-chain communication cycle is complete, and the receiver successfully received and acknowledged the message.
...
- **Message receipt**: Upon receiving a message via CCIP, the `ccipReceive` function decodes it and calls the `acknowledgePayLINK` internal function nested within the [`ccipReceive`](/ccip/tutorials/send-arbitrary-data#receiving-data) function.

- **Acknowledgment sending**: `acknowledgePayLINK` constructs an acknowledgment message, sends it to the _message tracker_ contract, and emits an `AcknowledgmentSent` event. This acknowledgment message signals that the initial message was received and acknowledged.
...
- **Verifying message authenticity**: The _message tracker_ contract ensures that a message status is updated to `Confirmed` only for a message it has sent.

## Final note

In this example, the _message tracker_ contract emits an event when it receives the acknowledgment message confirming the initial message reception on the counterpart chain. However, you could think of any other logic to execute upon acknowledgment receipt. Also, this tutorial demonstrates this pattern for sending arbitrary data, but you can apply the same pattern to programmable token transfers.
