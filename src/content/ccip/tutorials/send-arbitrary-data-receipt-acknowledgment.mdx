...
1. Copy the **initial message ID** from the CCIP explorer (transaction from _Ethereum Sepolia_ to _Polygon Mumbai_) and paste it as the argument in the **messagesInfo** getter function. Click **messagesInfo** to read the message status. It returns `status 2` and the acknowledger message ID that confirms this status.
