...
When the _message tracker_ receives the acknowledgment message, the `ccipReceive` function updates the initial message status to `2`, which corresponds to the `ProcessedOnDest` status as defined by the `MessageStatus` `enum`. The function emits a `MessageProcessedOnDest` event.
...
1. Copy your _message tracker_ contract address from Remix. Open the [_Ethereum Sepolia_ explorer](https://sepolia.etherscan.io/) and search for your deployed _message tracker_ contract. Then, click on the **Events** tab.
   ...
   The `MessageProcessedOnDest` event is emitted with the acknowledged message ID `0x8773e27ea01eb0e7671da8d63e14a647713cb1274b7110141d249d3b8fce116b` as indexed `topic2`.
...
- **Status tracking**:
  ...
  - Upon receiving an acknowledgment message from the _acknowledger_ contract, the _message tracker_ contract updates the message status from `Sent` (status `1`) to `ProcessedOnDest` (status `2`). This update indicates that the cross-chain communication cycle is complete, and the receiver successfully received and acknowledged the message.

    ```solidity
    // Update the message status to `ProcessedOnDest`
    messagesInfo[messageId].status = MessageStatus.ProcessedOnDest;
    ```
...
- **Ensuring the initial message authenticity**: The _message tracker_ contract first checks that the message awaiting acknowledgment was sent from the contract itself and is currently marked as `Sent`. Once confirmed, the message status is updated to `ProcessedOnDest`.
