...
```bash
# Save Phase 1 variables to file for later use
cat > ~/.phase1_vars << 'EOF'
export ETH_TOKEN_ADDRESS=0xd3d2f66dd4Eead2cF2cD4C6219Eb1e1E80e6109d
export ETH_POOL_ADDRESS=0x1a65BA94731a6541efDbeb1E72A7b1E9771B64b7
EOF

echo "Phase 1 variables saved to ~/.phase1_vars"
```
...
export SOL_TOKEN_MINT=9qfyXQhZt1ZUvEX1kKephoTqpiXnh1FBskRFB6udRwQG
...
export SOL_POOL_SIGNER_PDA=7WTnVd9Q6de3GgPCpDfHX5S2T5XzGDeMDZRev3FVbZo5
```
...
spl-token create-multisig 1 $SOL_POOL_SIGNER_PDA $ADMIN_WALLET
```
...
export SOL_MULTISIG_ADDRESS=EFpxujcKwYiLhNZyt9NWUsDqFDMUGbsRBdqqvxrfuvGj
```
...
echo "Loaded variables: ETH_TOKEN_ADDRESS=$ETH_TOKEN_ADDRESS, SOL_TOKEN_MINT=$SOL_TOKEN_MINT"
```
...
export SOL_POOL_CONFIG_PDA=B6yVeSSESLtmyaqwnzWDAxg8ZrSz58Z9WezYaK1SK9jc
```
...
```bash
export SOL_ALT_ADDRESS=9TG3CoF2XyLAH8usfCm7ADJQNxopEAu4CyiWmYCEVuUw
```
...
Load all variables for testing:

```bash
source ~/.ccip_complete_vars
echo "Environment loaded: SOL_TOKEN_MINT=$SOL_TOKEN_MINT, SOL_ALT_ADDRESS=$SOL_ALT_ADDRESS"
```
...
yarn svm:token-transfer --token-mint $SOL_TOKEN_MINT
```
...
yarn svm:token-transfer --token-mint $SOL_TOKEN_MINT --token-amount 1000000 --receiver 0x9d087fC03ae39b088326b67fA3C788236645b717
```
