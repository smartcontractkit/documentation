...
CCIP messages from SVM are built using the [`SVM2AnyMessage` struct](https://github.com/smartcontractkit/chainlink-ccip/blob/solana-ccip-release/0.1.0/chains/svm/contracts/ccip-router/src/messages.rs#L92-L99) from the CCIP Router program. The `SVM2AnyMessage` struct is defined as follows:
...
- For EVM destinations: This is the hex-encoded address of the contract or wallet that will receive the message
- Must be properly formatted as a 20-byte Ethereum address
- Can target smart contracts that implement CCIP-compatible functionality or user wallets for token-only transfers
...
- Encoding consideration: The EVM receiver must be able to correctly decode this data
...
<Aside type="tip" title="Token Compatibility">
  Only tokens with established token pools can be transferred. Verify token compatibility before implementation.
</Aside>
...
<Aside type="tip" title="Fee Token Compatibility">
  Check the [CCIP Directory](/ccip/directory) for the list of supported fees tokens.
</Aside>
...
<Aside title="Setting Gas Limits">
...
- Only tokens with established token pools can be transferred
...
- The receiver must be a contract that implements CCIP receive functionality
...
4. **Token Indices**: For token transfers, indices marking where each token's accounts begin
...
<Fragment slot="tab.2">Example: String Encoding</Fragment>

  <Fragment slot="panel.1">

    When sending messages across chains, ensure the data payload uses an encoding format that's compatible with the destination chain's receiver contract. For EVM-based destinations, implement Ethereum ABI encoding to guarantee proper data interpretation upon receipt.
...
1. Add a flags byte for execution order (0x00 for strict order, 0x01 for allowing out of order)
