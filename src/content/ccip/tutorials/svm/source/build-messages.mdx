...
CCIP messages from SVM are built using the [`SVM2AnyMessage`](https://github.com/smartcontractkit/chainlink-ccip/blob/contracts-ccip-release/1.6.0/chains/solana/contracts/programs/ccip-router/src/messages.rs#L51) struct from the CCIP Router program. The `SVM2AnyMessage` struct is defined as follows:
