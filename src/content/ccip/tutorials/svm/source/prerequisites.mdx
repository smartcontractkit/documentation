...
- **Solana Wallet with Private Key**: You'll need a Solana keypair file. If you don't have one, create it with:
  ...
  If you want to use your existing account from [Phantom Wallet](https://phantom.app/), you can [export your account's secret recovery phrase](https://help.phantom.com/hc/en-us/articles/25334064171795-How-to-view-or-export-your-recovery-phrase-and-private-key-in-Phantom#h_01K3726WHTCPM62551RDQ0C31D) and recover it with the following command:
  ```bash
  solana-keygen recover 'prompt:?key=0/0'
  ```
  <br></br>
  You may need to use the `--force` flag if you have already created a new account using the `solana-keygen new` command above. Paste your secret recovery phrase when prompted and press ENTER.
