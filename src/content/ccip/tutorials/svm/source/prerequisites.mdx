...
isIndex: false
metadata:
  description: "Set up Solana CCIP development. Install Anchor and Solana CLI, configure wallets and token accounts, and prepare for Solana→EVM tutorials."
  image: "/images/ccip/concepts/architecture/ccip-svm-source-chain.jpg"
  excerpt: "prerequisites, Solana, Anchor, CLI, wallets, tokens, setup"
...
- **Solana Wallet with Private Key**: You'll need a Solana keypair file. If you don't have one, create it with:
  ...
  <Aside type="note" title="Note">
    For a browser-based extension wallet (similar to MetaMask for EVM-based chains), you can install [Phantom
    Wallet](https://phantom.app/) for Solana. You can [import your account using your private
    key](https://help.phantom.com/hc/en-us/articles/15079894392851-How-to-import-an-existing-wallet-into-Phantom#h_01JVS7DHK9V610PR1Y6XG35CZV)
    in **64-byte array** format, which you can obtain from the `id.json` file generated using the above command. To get
    the value, simply execute the `cat ~/.config/solana/id.json` command and copy the **64-byte array** value (like
    `[191,198,244,...,179,114,73]`) of the private key to import your created account into Phantom Wallet.
  </Aside>
