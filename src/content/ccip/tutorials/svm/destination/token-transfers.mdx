...
1. **Edit the script directly**: Modify the `tokenReceiver`
   value in the code by changing `"EPUjBP3Xf76K1VKsDSc6GupBWE8uykNksCLJgXZn87CB"` to your own Solana wallet address.
...
**Option B: Run with your wallet address as a parameter** (recommended for quick testing):

```bash
npx ts-node ccip-scripts/evm/router/1_token-transfer.ts --token-receiver YOUR_SOLANA_WALLET_ADDRESS
```

> **Important:** Replace `YOUR_SOLANA_WALLET_ADDRESS` with your actual Solana wallet address to receive tokens in your own wallet.

This script will:

- Configure the CCIP message with the default parameters
- Calculate the CCIP fees
- Approve the router to spend your CCIP-BnM tokens
- Approve the router to spend your LINK tokens
- Call the CCIP router contract to execute the cross-chain transfer
- Return the transaction details and message ID for tracking
...
You can customize the token transfer by providing command-line arguments to the script. These parameters will override any values in the script.

**Specify your own Solana wallet:** ✨ (Recommended for quick testing)

```bash
npx ts-node ccip-scripts/evm/router/1_token-transfer.ts --token-receiver YOUR_SOLANA_WALLET_ADDRESS
```

Replace `YOUR_SOLANA_WALLET_ADDRESS` with your Solana wallet address (e.g., DvSgNMRxVSMBpT8YGgEjrThS9NSGxDCwiyfNxeU6hwyA).
...
```bash
npx ts-node ccip-scripts/evm/router/1_token-transfer.ts --token-receiver YOUR_SOLANA_WALLET_ADDRESS --amount 2000000000000000 --fee-token native
```
