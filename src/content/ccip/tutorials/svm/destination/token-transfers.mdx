...
<Aside type="important" title="Critical Settings">
...
Let's examine the key components of the token transfer script:

1. **Setup and Configuration**

```typescript
// Import required libraries
import { setupClientContext, FeeTokenType } from "@solana-starter-kit/ccip-sdk"
import { PublicKey } from "@solana/web3.js"
import { getTokenDetails } from "./utils"

// Configure environment and defaults
const config = {
  tokenAddress: "0x316496C5dA67D052235B9952bc42db498d6c520b", // CCIP-BnM on Sepolia
  destinationChain: "svm", // Solana Virtual Machine
  destinationChainSelector: 16423721717087811551n, // Solana Devnet selector
  solanaRecipient: "EPUjBP3Xf76K1VKsDSc6GupBWE8uykNksCLJgXZn87CB", // Your Solana wallet
}
```

2. **Message Configuration and Preparation**

```typescript
// Define the CCIP message configuration
const MESSAGE_CONFIG = {
  tokenAmounts: [
    {
  ...
  data: "0x", // Empty for token transfers
  extraArgs: {
    computeUnits: 0, // MUST be 0 for token-only transfers
    allowOutOfOrderExecution: true,
    accountIsWritableBitmap: BigInt(0),
    tokenReceiver: config.solanaRecipient,
    accounts: [],
  },
  receiver: PublicKey.default.toString(),
}

// Set up client and prepare the message
const { client, wallet } = await setupClientContext()
const { tokenInfo } = await getTokenDetails(client, MESSAGE_CONFIG)
const ccipMessageRequest = await createCCIPMessageRequest(client, config.destinationChainSelector, MESSAGE_CONFIG)
```

3. **Executing the Transfer and Handling Results**

```typescript
// Send the CCIP message and capture the result
const result = await client.sendCCIPMessage(ccipMessageRequest)

// Display transaction details for tracking
...
This structured approach ensures that your token transfer:

- Uses the correct parameters for Solana destinations
- Includes all required settings for token-only transfers
- Properly tracks the message across chains

## Running the Token Transfer

Now that you understand the configuration, let's execute the token transfer using the script from the starter kit:
...
- Configure the token transfer message
- Calculate the CCIP fees
- Execute the transfer to your Solana wallet
