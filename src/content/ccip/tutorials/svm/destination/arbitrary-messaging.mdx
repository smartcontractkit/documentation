...
To interact with a SVM programs, you need to understand Program Derived Addresses (PDAs):
...
<Tabs client:visible>
  ...
  <Fragment slot="panel.1">
    **Binary representation of writable accounts**: Position 0 (statePda): Not writable → Bit 0 = 0 Position 1
    (messagesStoragePda): Writable → Bit 1 = 1 Binary: 10 = Decimal 2
  </Fragment>
  <Fragment slot="panel.2">
    **Each position has a decimal value**: Position 0: 2^0 = 1 (not used, as this account is not writable) Position 1:
    2^1 = 2 Total: 0 + 2 = 2
  </Fragment>
  <Fragment slot="panel.3">
    **Simplified position values**: | Account Position | Decimal Value If Writable |
    |------------------|---------------------------| | 0 | 1 | | 1 | 2 | | 2 | 4 | | 3 | 8 | | 4 | 16 | | 5 | 32 | In
    our case: Only Position 1 is writable (2)
  </Fragment>
</Tabs>
