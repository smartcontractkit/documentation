...
## Understanding Arbitrary Messaging to Solana

Before diving into the code, let's explore how arbitrary messaging differs from token transfers:
...
<Tabs client:visible>
  ...
  <Fragment slot="panel.2">
    ...
    **Arbitrary Messaging**:
    - `computeUnits` must be sufficient (200,000+)
...
This tutorial uses the CCIP Basic Receiver program deployed on Solana Devnet as the destination program. This sample program:

- Implements the CCIP receiver interface
- Receives and stores cross-chain messages
- Maintains a history of received messages
- Allows retrieval of message data
...
<Aside type="note" title="Scope Clarification">
  ...
  focuses on sending messages to an existing program. The principles you'll learn can be applied to any Solana program
...
- They provide on-chain storage for Solana programs
- Each PDA serves a specific purpose (e.g., state storage, message storage)
- When sending messages to a program, you must specify all required PDAs

For the CCIP Basic Receiver program, we need two main PDAs:

1. **State PDA**: Stores the program's configuration
2. **Messages Storage PDA**: Stores received messages

## Implementing Arbitrary Messaging

Let's implement arbitrary messaging from Ethereum Sepolia to the CCIP Basic Receiver program on Solana Devnet.

### 1. Deriving Program Derived Addresses

First, we need to derive the PDAs required by the receiver program:
...
<Aside type="important" title="PDA Derivation">
  PDAs must be derived using the exact same seeds that the program uses internally. Incorrect seeds will result in
  different addresses, causing transaction failures.
</Aside>

### 2. Configuring the Message

Next, let's configure the CCIP message for arbitrary messaging:
...
const MESSAGE_CONFIG = {
  ...
  extraArgs: {
    ...
    // Binary 11 (decimal 3) means both accounts are writable
...
<Aside type="tip" title="Message Data Encoding">
  When sending string data, you need to encode it as hex: - "Hello World" in hex is "0x48656c6c6f20576f726c64" - You can
  use online tools or console commands to convert: ```bash echo -n "Hello World" | xxd -p ```
</Aside>

### 3. Understanding AccountIsWritableBitmap

The `accountIsWritableBitmap` value is critical for arbitrary messaging. It indicates which accounts in the `accounts` array should be marked as writable.

For our example:

- We're providing 2 accounts: `[statePda, messagesStoragePda]`
- Both accounts need to be writable for the program to update them
- The bitmap is calculated as follows:
...
### 4. Creating and Sending the CCIP Message

Now, let's create and send the CCIP message:
...
Let's run the script to send an arbitrary message:
...
[ccip-messenger] [INFO] ✅ Verified on-chain allowance for LINK
...
You can customize your arbitrary message:
...
- Configure a CCIP message for arbitrary data messaging to Solana
- Understand PDAs and their role in Solana program interactions
- Calculate the correct `accountIsWritableBitmap` for program execution
- Send cross-chain messages that trigger program execution
- Verify the message was received and processed by the program

Arbitrary messaging opens up powerful possibilities for cross-chain applications, allowing for complex interactions between Ethereum and Solana ecosystems.
