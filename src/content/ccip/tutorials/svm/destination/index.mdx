...
<Tabs client:visible>
  <Fragment slot="tab.1">Description</Fragment>
  ...
  <Fragment slot="panel.2">
    If your accounts array has 7 accounts and accounts at positions 1 and 2 need to be writable:
    ```
    ...
    bitmap |= (1n << 2n); // Set bit for account at index 2
    // Result: bitmap = 6n
    ```
  </Fragment>
</Tabs>
...
<Aside type="caution" title="Required Setting">
  **MUST** be set to `true` for Solana as a destination chain due to how the Solana CCIP implementation processes
  messages.
</Aside>
...
<Tabs client:visible>
  ...
  <Fragment slot="panel.1">
    For transfers to users: - Set to the user's Solana wallet address - Direct transfer with no intermediate handling
  </Fragment>
  <Fragment slot="panel.2">
    For transfers to programs: - Set to a Program Derived Address (PDA) that the program has authority over - Prevents
    tokens from being locked in inaccessible accounts - Program should forward tokens to final recipient
  </Fragment>
</Tabs>
