...
1. Call the `_buildCCIPMessage` private function to construct a CCIP-compatible message using the `EVM2AnyMessage` [struct](/ccip/api-reference/evm/v1.5.1/client#any2evmmessage):
   ...
   - The `tokenAmounts` is an empty `EVMTokenAmount` [struct](/ccip/api-reference/evm/v1.5.1/client#evmtokenamount) array as no tokens are transferred.
...
On the destination blockchain, the router invokes the `ccipReceive` [function](/ccip/api-reference/evm/v1.5.1/ccip-receiver#ccipreceive) which expects an `Any2EVMMessage` [struct](/ccip/api-reference/evm/v1.5.1/client#any2evmmessage) that contains:
