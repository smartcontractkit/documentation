...
whatsnext:
  {
    "Learn how to manually execute a failed CCIP transaction": "/ccip/evm/tutorials/manual-execution",
    "Learn how to handle errors gracefully when making CCIP transactions": "/ccip/evm/tutorials/programmable-token-transfers-defensive",
    "Transfer Tokens Between EOAs": "/ccip/evm/tutorials/transfer-tokens-from-eoa",
...
- Sender part:
  ...
  - Calls the router's `getFee` [function](/ccip/tools-resources/api-reference/evm/v1.5.1/i-router-client#getfee) to estimate the CCIP fees.
  - Calls the router's `ccipSend` [function](/ccip/tools-resources/api-reference/evm/v1.5.1/i-router-client#ccipsend) to send CCIP messages.

- Receiver part:
  - The contract inherits from [CCIPReceiver](/ccip/tools-resources/api-reference/evm/v1.5.1/ccip-receiver), which serves as a base contract for receiver contracts. This contract requires that child contracts implement the `_ccipReceive` [function](/ccip/tools-resources/api-reference/evm/v1.5.1/ccip-receiver#_ccipreceive). `_ccipReceive` is called by the `ccipReceive` [function](/ccip/tools-resources/api-reference/evm/v1.5.1/ccip-receiver#ccipreceive), which ensures that only the router can deliver CCIP messages to the receiver contract.

### Transferring tokens and data and pay in LINK

The `sendMessagePayLINK` function undertakes six primary operations:

1. Call the `_buildCCIPMessage` private function to construct a CCIP-compatible message using the `EVM2AnyMessage` [struct](/ccip/tools-resources/api-reference/evm/v1.5.1/client#any2evmmessage):
   ...
   - The `tokenAmounts` is an array, with each element comprising an `EVMTokenAmount` [struct](/ccip/tools-resources/api-reference/evm/v1.5.1/client#evmtokenamount) containing the token address and amount. The array contains one element where the `_token` (token address) and `_amount` (token amount) are passed by the user when calling the `sendMessagePayLINK` function.
...
1. Computes the fees by invoking the router's `getFee` [function](/ccip/tools-resources/api-reference/evm/v1.5.1/i-router-client#getfee).
...
1. Dispatches the CCIP message to the destination chain by executing the router's `ccipSend` [function](/ccip/tools-resources/api-reference/evm/v1.5.1/i-router-client#ccipsend).
...
1. Grants the router contract permission to deduct the amount from the contract's _CCIP-BnM_ balance.
1. Dispatches the CCIP message to the destination chain by executing the router's `ccipSend` [function](/ccip/tools-resources/api-reference/evm/v1.5.1/i-router-client#ccipsend). **Note**: `msg.value` is set because you pay in native gas.
...
On the destination blockchain, the router invokes the `_ccipReceive` [function](/ccip/tools-resources/api-reference/evm/v1.5.1/ccip-receiver#_ccipreceive) which expects a `Any2EVMMessage` [struct](/ccip/tools-resources/api-reference/evm/v1.5.1/client#any2evmmessage) that contains:
...
- `_ccipReceive` is called by the `ccipReceive` [function](/ccip/tools-resources/api-reference/evm/v1.5.1/ccip-receiver#ccipreceive), which ensures that only the router can deliver CCIP messages to the receiver contract. See the `onlyRouter` [modifier](/ccip/tools-resources/api-reference/evm/v1.5.1/ccip-receiver#onlyrouter) for more information.
