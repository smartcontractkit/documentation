...
If you prefer to manage your deployments and configurations programmatically, refer to the [Cross-Chain Tokens](/ccip/tutorials/evm/cross-chain-tokens) guides available for Remix, Hardhat and Foundry.
...
- **Deployment of token pools that use the [Lock and Unlock mechanism](/ccip/concepts/cross-chain-token/overview#lock-and-unlock)**. In this use case, tokens are locked on the source blockchain, and an equivalent amount of tokens are released on the destination blockchain.
...
1. On the **Pools** page, you're prompted to select the token pool mechanism that's used to transfer value between networks &mdash; _Burn / Mint_ or _Lock / Release_. Depending on your token contract, you may have more than one option for _Burn / Mint_:
   ...
   Before selecting a token pool type, be sure to review [CCIP token handling mechanisms](/ccip/concepts/cross-chain-token/evm/token-pools#token-handling-mechanisms).
