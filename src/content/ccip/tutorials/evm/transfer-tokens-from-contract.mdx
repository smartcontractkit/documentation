...
1. Call the `_buildCCIPMessage` private function to construct a CCIP-compatible message using the `EVM2AnyMessage` [struct](/ccip/tools-resources/api-reference/evm/v1.5.1/client#any2evmmessage):
   ...
   - The `tokenAmounts` is an array, with each element comprising a [`EVMTokenAmount` struct](/ccip/tools-resources/api-reference/evm/v1.5.1/client#evmtokenamount) that contains the token address and amount. The array contains one element where the `_token` (token address) and `_amount` (token amount) are passed by the user when calling the `transferTokensPayLINK` function.
