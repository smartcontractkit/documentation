...
1. Initiate a CCIP transaction and note the CCIP source transaction hash (i.e., the transaction hash on the source blockchain). You can obtain the same by running any of the previous CCIP tutorials.
...
<Aside type="note" title="Note">
  The `--page` option is used to limit the number of blocks to search for the message. The default value is `10000` which could result in an error due to RPC limitations. Setting it to `500` is a good practice to avoid such issues.
</Aside>
