...
1.  For higher security, you can use the [@chainlink/env-enc](https://www.npmjs.com/package/@chainlink/env-enc) to encrypt your environment variables at rest.
...
- `source`: Source network, chain ID, or name.
- `router`: Router contract address on the source network.
- `dest`: Destination network, chain ID, or name.
...
You can find details such as the router contract address, LINK token address, and wrapped native gas token (like WETH) address in the [CCIP Directory](https://docs.chain.link/ccip/directory) by searching for the corresponding network.
...
1.  Send 0.001 CCIP-BnM from your EOA on _Avalanche Fuji_ to another account on _Ethereum Sepolia_:
    ...
    | <CopyText text="--transfer-tokens" code/>                          | This specifies the `transfer-tokens` flag followed by the token address to be transferred.                                                                                                                                     |
    ...
    | <CopyText text="0.001" code/>                                      | This is the amount of CCIP-BnM tokens to be transferred. In this example, 0.001 CCIP-BnM are transferred.                                                                                                                      |
...
1.  Once you execute the command, you should see the following logs:
        ...
        Approving 1000000000000000n 0xD21341536c5cF5EB1bcb58f6723cE26e8D8E90e4 for 0xF694E193200268f9a4868e4Aa017A0118C9a8177 = 0x406619cd74dc3e5996f52e52f9cc84f23284c26f1df219e0d51ad273aa4e05e4
        Approving 40903083926519498n 0x0b9d5D9136855f6FEc3c0993feE6E9CE8a297846 for 0xF694E193200268f9a4868e4Aa017A0118C9a8177 = 0xd7802b3b12420645fb4e57dca016a5c2a34caf199586f6e23cd073b3272e7954
        Sending message to 0x27d7A69C878F9c8f51f4e53703abCE9bAcd2D9bf @ ethereum-testnet-sepolia , tx_hash = 0x980dacf245f9c6919678219e97d6ad20e0c1964795ec3801e688315f1f18defd
        Lane:
        ┌────────────────┬──────────────────────────────────────────────┬────────────────────────────┐
        │ (index)        │ source                                       │ dest                       │
        ├────────────────┼──────────────────────────────────────────────┼────────────────────────────┤
        ...
        │ timestamp       │ '2025-05-26 16:34:14 (7s ago)'                                       │
        │ finalized       │ true                                                                 │
...
1.  After several minutes (the waiting time depends on the [finality of the source blockchain](/ccip/ccip-execution-latency)), the transaction should be finalized, and you will see the status as `SUCCESS` in the CCIP explorer.
...
1.  The data field is empty because only tokens are transferred. The gas limit is set to `200000` because that's the default value in the ramp config. However, you can override this value to `0` by passing it in the `--gas-limit` flag, since the transaction is directed to an Externally Owned Account (EOA). With an empty data field, no function calls on a smart contract are expected on the destination chain.
...
1.  After you execute the command, you should see the following logs:
        ...
        Approving 1000000000000000n 0xD21341536c5cF5EB1bcb58f6723cE26e8D8E90e4 for 0xF694E193200268f9a4868e4Aa017A0118C9a8177 = 0xae95da9b6b603513c6588730c7b984561ab58ef7abbd34f7cc768ecc7362faba
        Sending message to 0x27d7A69C878F9c8f51f4e53703abCE9bAcd2D9bf @ ethereum-testnet-sepolia , tx_hash = 0x589b0b00ee1f86bff15dd51e7160ab01b09f987be78835a8d21697493f1fee3d
        Lane:
        ┌────────────────┬──────────────────────────────────────────────┬────────────────────────────┐
        │ (index)        │ source                                       │ dest                       │
        ├────────────────┼──────────────────────────────────────────────┼────────────────────────────┤
        ...
        │ (index)         │ Values                                                               │
        ├─────────────────┼──────────────────────────────────────────────────────────────────────┤
        │ messageId       │ '0xd23961ad66647c99c25bf260fab009fd706e3256d30bc32bcf1cd1f5ee79ac06' │
        │ origin          │ '0x8C244f0B2164E6A3BED74ab429B0ebd661Bb14CA'                         │
        │ sender          │ '0x8C244f0B2164E6A3BED74ab429B0ebd661Bb14CA'                         │
        │ receiver        │ '0x27d7A69C878F9c8f51f4e53703abCE9bAcd2D9bf'                         │
        │ sequenceNumber  │ 3837                                                                 │
        │ nonce           │ 6                                                                    │
        │ gasLimit        │ 200000                                                               │
        │ transactionHash │ '0x589b0b00ee1f86bff15dd51e7160ab01b09f987be78835a8d21697493f1fee3d' │
        │ logIndex        │ 22                                                                   │
        │ blockNumber     │ 40956468                                                             │
        │ timestamp       │ '2025-05-26 18:16:18 (7s ago)'                                       │
        │ finalized       │ true                                                                 │
        │ fee             │ '0.011191023362609395 WAVAX'                                         │
        │ tokens          │ '0.001 CCIP-BnM'                                                     │
        │ data            │ '0x'                                                                 │
        └─────────────────┴──────────────────────────────────────────────────────────────────────┘
        ```
