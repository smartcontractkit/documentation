...
Complete the [Canton as Source prerequisites](/ccip/tutorials/canton/source/prerequisites) before starting this tutorial. Your Canton party needs **Amulet** holdings to pay CCIP fees.
...
- **Fee payment**: CCIP fees are paid on Canton in **Amulet**.
...
2. **Argument parsing**: Reads `--dataString`, `--evmReceiver`, `--gasLimit` (default `200000`), and `--no-exec`.
