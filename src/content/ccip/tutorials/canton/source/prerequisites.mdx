...
`--finality` is an `any2canton`-only flag (Canton as destination). See the [Canton as Destination prerequisites](/ccip/tutorials/canton/destination/prerequisites#optional-script-flags) for its details — it is not accepted by `canton2any:*` send scripts.
