...
- **Finality**: The TEST ↔ LINK token pool only supports default (`finalized`) source finality. Unlike data-only sends, you cannot pass `--finality` to `any2canton:token`.
...
The TEST ↔ LINK token pool only supports default (`finalized`) source finality. Do **not** pass `--finality` to `any2canton:token` — it is not supported by the pool. The `--finality` flag is only valid for data-only sends (`any2canton:data`). Wait for Sepolia finality before running `any2canton:manual-exec`.
