...
| <nobr>`--finality`</nobr>       | <nobr>`finalized`</nobr> | **Data-only sends** — requested source finality (`finalized` or block depth, e.g. `32`). Not supported for token / data-and-token sends on the TEST ↔ LINK token pool, which only supports default finality. |
