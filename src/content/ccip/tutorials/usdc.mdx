...
**Note**: You are not limited to programmable token transfers but can use CCIP to transfer USDC tokens without data. Check the [Mainnets](/ccip/supported-networks/mainnet) and [Testnets](/ccip/supported-networks/testnet) configuration pages to learn on which blockchains CCIP supports USDC transfers.

## Architecture

Fundamentally the [architecture](/ccip/architecture#ccip-components) of CCIP and [API](/ccip/api-reference/i-router-client) are unchanged:

- The sender has to interact with the CCIP router to initiate a cross-chain transaction, similar to the process for any other token transfers (Read the [Transfer Tokens](/ccip/tutorials/cross-chain-tokens) guide to learn more).
- The same onchain components (Router, OnRamp, Commit store, OffRamp and Token Pool) are used.
- The same offchain components (Committing DON, Executing DON and Risk Management Network) are used.
...
The diagram below shows that the USDC token pools and Executing DON handle the integration with the Circle’s contracts and offchain CCTP Attestation API. As with any other supported ERC-20 token, USDC has a linked token pool on each supported blockchain to facilitate OnRamp and OffRamp operations (To learn more, read the [architecture page](/ccip/architecture#ccip-components)).
...
1. On the source blockchain:
   1. When the sender initiates a transfer of USDC, the USDC token pool interacts with CCTP's contract and specifies the USDC token pool address on the destination chain.
   1. CCTP burns the specified USDC tokens and emits a burn event.
1. Offchain:
   1. Circle attestation service listens to CCTP burn events and generates attestations. An attestation is a signed authorization to mint the specified amount of USDC for a given receiver address on the destination blockchain.
   1. The [Executing DON](/ccip/architecture#executing-don) listens for the CCTP burn event and then calls the Circle attestation service to fetch the attestation.
1. On the destination blockchain:
   ...
   1. The OffRamp contract calls the USDC token pool with the USDC amount to be minted and the Circle attestation. If there is data in the CCIP message, then the OffRamp contract transmits the CCIP message to the [Router](/chip/architecture#router) contract.
   1. The USDC token pool calls the CCTP contract. The CCTP contract verifies the attestation signature, the USDC amount, and the USDC token pool address before minting the specified USDC amount for the USDC token pool.
   1. The USDC token pool transfers the specified USDC amount to the CCIP receiver.
...
1. Check the [Supported Networks page](/ccip/supported-networks) to confirm that USDC are supported for your lane. In this example, you will transfer tokens from _Avalanche Fuji_ to _Polygon Mumbai_ so check the list of supported tokens [here](/ccip/supported-networks/testnet#avalanche-fuji-polygon-mumbai).
...
1. At this point, you have one _sender_ contract on _Avalanche Fuji_ and one _receiver_ contract on _Polygon Mumbai_. As security measures, you enabled the sender contract to send CCIP messages to _Polygon Mumbai_ and the receiver contract to receive CCIP messages from the sender and _Avalanche Fuji_.
