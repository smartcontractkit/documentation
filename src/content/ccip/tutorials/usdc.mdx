...
1. On the destination blockchain:
   ...
   1. The OffRamp contract calls the USDC token pool with the USDC amount to be minted and the Circle attestation.
   1. The USDC token pool calls the CCTP contract. The CCTP contract verifies the attestation signature before minting the specified USDC amount into the CCIP receiver.
   1. The USDC token pool transfers the specified USDC amount to the CCIP receiver.
   1. If there is data in the CCIP message, then the OffRamp contract transmits the CCIP message to the [Router](/ccip/architecture#router) contract.
