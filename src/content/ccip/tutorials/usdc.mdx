...
The process will involve the following steps:
      ...
      1.  **Transfer USDC and Data:** You'll initiate a transfer of USDC tokens and associated data from the Sender contract on Avalanche Fuji. The data will include the function signature of the stake function of the Staker contract along with the necessary arguments.
      1.  **Receive and Stake:** The Receiver contract on Ethereum Sepolia will receive the tokens and data. It will then directly use this data to make a low-level call to the Staker contract, executing the stake function to stake the USDC on behalf of a beneficiary.
      1.  **Redeem Staked Tokens:** The beneficiary will later be able to redeem the staked tokens for USDC.

The purpose of including the function signature and arguments in the data is to demonstrate how arbitrary data can support a variety of scenarios and use cases. By sending specific instructions within the data, you can define various interactions between smart contracts across different blockchain networks, making your decentralized application more flexible and powerful.
...
<ClickToZoom src="/images/ccip/tutorials/usdc-tutorial.jpg" alt="Chainlink CCIP usdc tutorial" />
      ...
      After you followed the tutorial:
         ...
         - Explore the [CCIP USDC
         masterclass](https://cll-devrel.gitbook.io/ccip-masterclass-4) to learn about other examples of cross-chain transfers with
         USDC.
...
1. Deploy, fund your sender contract on _Avalanche Fuji_ and enable sending messages to _Ethereum Sepolia_:
   ...
   1. Fill in your blockchain's router, LINK, and USDC contract addresses. The router and USDC addresses can be found on the [supported networks page](/ccip/supported-networks) and the LINK contract address on the [LINK token contracts page](/resources/link-token-contracts). For Avalanche Fuji, the addresses are:
...
1. Deploy the Staker contract:
   ...
   1. Fill in the usdc contract address. The usdc contract address can be found on the [supported networks page](/ccip/supported-networks). For _Ethereum Sepolia_, the usdc contract address is:
...
1. Deploy the Receiver contract:
   ...
   1. Fill in your blockchain's router, LINK, and Staker contract addresses. The router and usdc addresses can be found on the [supported networks page](/ccip/supported-networks) and the Staker contract address from the previous step. For _Ethereum Sepolia_, the addresses are:
...
1. Configure the Receiver contract to receive CCIP messages from the Sender contract:
   ...
   1. Fill in the arguments of the _**setSenderForSourceChain**_ function:
      ...
      | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
      | \_sourceChainSelector | <CopyText text="14767482510784806043" code/> <br /> The chain selector of _Avalanche Fuji_. You can find it on the [supported networks page](/ccip/supported-networks). |
