...
This tutorial will guide you through the process of enabling your own tokens in CCIP using [Foundry](https://book.getfoundry.sh/). You will learn how to deploy tokens, a _Lock & Release_ token pool on one network, and a _Burn & Mint_ token pool on another network. After that, you will register them in CCIP and configure them without needing direct intervention from Chainlink Labs.
