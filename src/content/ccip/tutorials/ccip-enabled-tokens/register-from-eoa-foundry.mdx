...
The script reads the `config.json` file to get the token name, symbol, decimals, initial supply, and whether to include CCIP admin functionality. Based on these configurations, it deploys the appropriate token contract — either with or without CCIP admin features — and optionally sets the CCIP admin. It then deploys the token contract and grants mint and burn roles to your EOA.
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------ | -------- |
| `tokenAddress`    | The address of the token to mint.                                                                                        | Yes      |
| `amount`          | The amount of tokens to mint (in wei), read from `config.json` under `tokenAmountToMint`.                                | Yes      |
| `receiverAddress` | The address of the receiver of the minted tokens; defaults to your EOA (`msg.sender`).                                   | No       |
| `network`         | The blockchain network on which the minting process will be executed; determined by the RPC URL when running the script. | Yes      |
...
In this step, you will use the `TransferTokens.s.sol` script to transfer tokens from Avalanche Fuji to Ethereum Sepolia using CCIP. You have two options for paying CCIP fees: using LINK tokens or native gas tokens. The [`config.json` file](/ccip/tutorials/ccip-enabled-tokens/register-from-eoa-foundry#optional-configure-your-configjson-file) specifies the fee type you will use in the `feeType` field.
...
| Parameter          | Description                                                                                                              | Required |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------ | -------- |
| `tokenAddress`     | The address of the token being transferred.                                                                              | Yes      |
| `amount`           | The amount of tokens to transfer.                                                                                        | Yes      |
| `destinationChain` | The blockchain to which the tokens will be transferred; determined from `config.json` via the `remoteChains` mapping.    | Yes      |
| `receiverAddress`  | The address of the receiver on the destination blockchain; defaults to your EOA (`msg.sender`).                          | Yes      |
| `feeType`          | The type of fee used for the transfer, either `"link"` or `"native"`; read from `config.json` under `feeType`.           | Yes      |
| `network`          | The blockchain network on which the token transfer will be initiated; determined by the RPC URL when running the script. | Yes      |
