...
1. Compute the fees by invoking the router's `getFee` [function](/ccip/api-reference/i-router-client#getfee).
...
This example is simplified for learning purposes. For production code, use the following best practices:
    ...
    is to make sure `extraArgs` is mutable. For example, you can build `extraArgs` offchain and pass it in your functions call
