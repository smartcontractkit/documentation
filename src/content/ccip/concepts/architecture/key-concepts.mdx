...
metadata:
  description: "Explore key concepts for Chainlink CCIP architecture: blockchain families, cross-chain dApps, finality, lanes, DONs, RMN, and Smart Execution."
---
...
CCIP implements a built-in gas escalation mechanism called Smart Execution to ensure messages are reliably delivered to the destination blockchain—even if gas prices fluctuate. When the RMN blesses a batch:

- If the initial fee paid on the source blockchain is sufficient, the DON attempts immediate execution on the destination chain.
- If gas prices increase unexpectedly, CCIP increments the gas price in steps to avoid indefinite transaction stalling.
- Although this might cause additional latency in extreme cases, it reduces the chance of messages remaining stuck in transit.
