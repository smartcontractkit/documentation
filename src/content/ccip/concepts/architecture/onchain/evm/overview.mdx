...
| Token Pool           | External (Token Issuer) | A contract that implements token handling mechanisms, such as Lock/Burn/Release, typically set up by the token Issuer. There is one token pool per token on a given chain.                                                                               |
| Token Contract       | External (Token Issuer) | An ERC20 token contract owned by the token issuer                                                                                                                                                                                                        |
| RMNRemote            | CCIP                    | Verifies Risk Management Network (RMN) signatures and handles cursing.                                                                                                                                                                                   |
...
- The OffRamp emits a final event, `ExecutionStateChanged`, containing the execution state. This state can be one of the following: 0) "UNTOUCHED", 1) "IN_PROGRESS", 2) "SUCCESS", or 3) "FAILURE".
