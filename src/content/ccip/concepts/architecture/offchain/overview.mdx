...
<ClickToZoom src="/images/ccip/concepts/architecture/ccip-offchain-architecture.jpg" alt="CCIP Offchain Architecture" />
...
- **Configuration and Role Validation**: Node roles are configured on the Home Chain (i.e., Ethereum) through specific contracts. The node software reads from the **CCIP Home** and **RMN Home** contracts to verify that each node is authorized for its role—whether for making observations or providing RMN blessings.
...
- **Pending Execution Check**
  ...
  - **Validation and Optimization**: The DON coordinates to validate the corresponding source chain events for these pending executions.
  ...
  - **Execution**: The message (or batch of messages) is executed on the destination chain.
