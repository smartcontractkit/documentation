...
1. **Observing Merkle Roots**
   ...
   - If the node's locally reconstructed merkle root matches the merke root posted on the destination chain, it signs an RMN observation stating that the commit is valid.
