...
For a consolidated view of all CCT security requirements, see [CCT Operational Security](/ccip/concepts/cross-chain-token/operational-security).
...
<Aside type="caution" title="Strongly recommended">
  It is strongly recommended to secure token pool and admin roles with multi-signature smart accounts (threshold of at
  least 3 signers) and a timelock with propose, review, and cancel steps before critical actions execute. See [CCT
  Operational Security](/ccip/concepts/cross-chain-token/operational-security#secure-signing-and-opsec).
</Aside>

**Timelock and risk grading:**

- Use a timelock to delay execution and allow distributed review or veto of proposed actions.
- Prefer Chainlink MCMS or multi-signature smart accounts (3/5+ threshold) over EOAs.
- Never assign mint/burn rights to a single EOA.
- Configure admin roles through the [Token Manager](/ccip/tools-resources/token-manager) where applicable.

**Signer hygiene:**

- Remove deprecated or compromised signers promptly.
- Audit signer access and contract settings quarterly.
- Distribute signers across time zones for rapid incident response when needed.
