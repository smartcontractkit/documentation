...
Before changing rate limits, call **`typeAndVersion()`** on the pool contract to confirm which contract release is deployed (for example, `"BurnMintTokenPool 1.6.3"`). Rate limit validation rules vary by version — see [Contract Version Behavior (EVM)](/ccip/concepts/rate-limit-management/contract-version-behavior#identify-your-version-with-typeandversion).
