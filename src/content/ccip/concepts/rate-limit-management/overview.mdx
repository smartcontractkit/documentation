...
<Aside type="note" title="Contract version matters (EVM)">
  TokenPool rate limit validation and consume behavior differ between [`@chainlink/contracts-ccip`
  releases](https://www.npmjs.com/package/@chainlink/contracts-ccip). Call `typeAndVersion()` on your pool to read the
  deployed release (for example, `"BurnMintTokenPool 1.6.3"`), then see [Contract Version Behavior
  (EVM)](/ccip/concepts/rate-limit-management/contract-version-behavior). When feasible, upgrade to the latest available
  pool release — see [Upgradability (EVM)](/ccip/concepts/cross-chain-token/evm/upgradability).
</Aside>

<Aside type="note" title="Recommendation for CCTs">
  It is recommended to configure Token Pool Rate Limits (TPRLs) on all lanes for CCT deployments. Unset rate limits
  reduce a defensive layer against unmitigated cross-chain flows. See [CCT Operational
  Security](/ccip/concepts/cross-chain-token/operational-security#unset-token-pool-rate-limits).
</Aside>
...
- **EVM contract version differences** ([Contract Version Behavior](/ccip/concepts/rate-limit-management/contract-version-behavior))
...
For the consolidated CCT security reference, see [CCT Operational Security](/ccip/concepts/cross-chain-token/operational-security).
