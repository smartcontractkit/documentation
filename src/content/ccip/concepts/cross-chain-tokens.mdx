...
The CCT architecture is designed to provide a self-service, streamlined approach to enabling cross-chain operations. If you have already understood the core concepts, ERC20-compatible token requirements, and token pool requirements, you are ready to proceed with enabling Cross Chain Tokens (CCT) for your project.

### Contracts

To successfully register your token for **Cross Chain Tokens (CCT)**, two main components are involved: the **TokenAdminRegistry** and the **RegistryModuleOwnerCustom**. These components work together to ensure that your token and its administrator are properly registered within the CCT system.

#### TokenAdminRegistry

The [`TokenAdminRegistry`](/ccip/api-reference/token-admin-registry) contract is responsible for storing the tokens, their administrators and token pools for all Cross Chain Tokens (CCT). This contract operates on a self-service basis, allowing tokens to be registered directly without requiring intervention from the CCIP contract owner. This ensures an efficient registration process that can accommodate a large number of tokens.

Key features of the **TokenAdminRegistry** include:

- Facilitates self-service token registration.
- Tokens for which the Token Administrator cannot be determined automatically can be registered manually by the CCIP contract owner.
- Manages the transfer of administrator roles for tokens.
- Non-upgradable: Designed as a customer-facing contract, making it immutable and stable.
- Handles admin role changes through a two-step process (transfer request and acceptance).

#### RegistryModuleOwnerCustom

The [`RegistryModuleOwnerCustom`](/ccip/api-reference/registry-module-owner-custom) contract acts as the interface through which the admin of a token is registered. It works alongside the **TokenAdminRegistry** to ensure that only the correct admin can perform the registration. The contract uses different methods to determine and register the token's administrator:

- **getCCIPAdmin**: Registers the administrator based on the `getCCIPAdmin()` method in the token contract.
- **owner**: Registers the administrator based on the token’s `owner()` method.
...
#### Sequence Diagram: Accepting the Administrator Role
