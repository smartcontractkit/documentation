...
- **Recommended Permissionless Token Administrator address registration methods**: A token can use any of the following supported function signatures for permissionless registration:
  ...
  - `hasRole(bytes32 role, address account)`: For tokens using the [OpenZeppelin's AccessControl](https://docs.openzeppelin.com/contracts/5.x/access-control) pattern, the token administrator must have the `DEFAULT_ADMIN_ROLE`.
...
If the token contract includes any of the necessary functions (`getCCIPAdmin()`, `owner()`, or [OpenZeppelin's AccessControl](https://docs.openzeppelin.com/contracts/5.x/access-control)), the registration can be done autonomously by the token administrator:
...
1. **Admin Initiates Registration**: The token administrator begins the process by calling one of these functions on the **RegistryModuleOwnerCustom** contract:
   ...
   - [`registerAccessControlDefaultAdmin()`](/ccip/api-reference/v1.5.1/registry-module-owner-custom#registeraccesscontroldefaultadmin) for tokens using [OpenZeppelin's AccessControl](https://docs.openzeppelin.com/contracts/5.x/access-control) pattern
...
1. **Determine Administrator**: The **RegistryModuleOwnerCustom** contract retrieves the administrator from the token contract using the appropriate method:
   ...
   - Via [`hasRole(DEFAULT_ADMIN_ROLE, msg.sender)`](https://docs.openzeppelin.com/contracts/5.x/access-control#using-access-control) for AccessControl tokens
