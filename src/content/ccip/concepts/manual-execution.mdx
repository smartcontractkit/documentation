...
In general, messages are successfully delivered and processed via CCIP, as detailed in the [Architecture](/ccip/architecture) page. CCIP has an inbuilt gas bumping mechanism called _Smart Execution_ to ensure users’ messages are reliably delivered to the destination blockchain (Read the [CCIP Execution latency](/ccip/architecture#ccip-execution-latency) section to learn more).
However, some exceptional conditions may require users to manually execute the transaction on the destination blockchain:

     - The receiver contract on the destination blockchain reverted due to an unhandled exception (for example, a logical error).
     - The receiver contract on the destination blockchain reverted due to the gas limit being insufficient to execute the triggered function (Note: The gas limit value is set in the [extraArgs](/ccip/api-reference/client#evmextraargsv1) param of the message).
     - The message couldn’t be delivered to the destination chain by CCIP’s smart execution within the smart execution time frame, currently set to 8 hours. This could happen, for example, during extreme network congestion and resulting gas spikes.

The flowchart presented below provides a visual guide through the process of a cross-chain transaction, detailing the steps involved in the manual execution:
...
1. Once the committed Merkle root is blessed, the [CCIP Executing DON](/ccip/architecture#executing-don) proceeds with the execution on the destination blockchain:
   1. During the smart execution timeframe, the Executing DON assesses and, if necessary, adjusts the gas price (a process known as gas bumping) to facilitate successful transaction execution on the destination blockchain as part of the smart execution process.
   1. If the smart execution timeframe has passed and the CCIP message has still not yet been executed, then Manual Execution is enabled (Read [below](/ccip/concepts/manual-execution#manual-execution)).
1. The execution on the destination blockchain works as follows:
   ...
   1. If the message involves token transfers, the tokens are first transferred to the receiver.
      ...
      1. If the receiver is an EOA, then this transaction is considered complete, with no further processing.
      1. If the receiver is a smart contract, the ccipReceive function is invoked after the token transfer. The ccipReceive function processes the CCIP message and any user-specified logic in the receiver contract. The execution of the CCIP message is atomic (all/none). If the ccipReceive function successfully executes, then all aspects of the transaction are complete, and there is no revert. If, however, there is an issue in the receiver execution due to insufficient gas limit or unhandled exceptions, the attempted token transfer will also revert. The transaction then becomes eligible for manual execution.

   1. If the message does not involve token transfers and only arbitrary messaging, then if the receiver execution fails due to gas limit/ unhandled exceptions, the transaction becomes eligible for manual execution.
...
When a CCIP message is eligible for manual execution, the [CCIP explorer](https://ccip.chain.link/) shows this clearly with a “Trigger Manual Execution” button.
...
<Aside type="note">
  Manual execution is expected to be available. If it is unavailable, please contact Chainlink Labs Support for
  assistance.
</Aside>

Depending on the situation, you can take one of the following steps:

- Insufficient gas limit: The executor (after connecting their wallet) can override the gas limit parameter (i.e., increase it for this particular execution) and trigger a manual execution. If this new gas limit override is sufficient, the transaction will go through successfully. Note that this only applies to this transaction. If subsequent messages involve the same gas limit, they all may fail. So, ensure you update the gas limit on future messages for this application to avoid more manual execution situations.
...
- If the issue was due to extreme gas spikes/ network conditions, and CCIP was not able to successfully transfer the message despite gas bumping for the entire duration of the Smart Execution timeframe (currently 8 hrs), then you can try manual execution.
...
It depends. If a message goes into manual execution mode due to receiver errors (unhandled exceptions or gas limit issues), subsequent messages don’t get stuck. However, suppose a message goes into manual execution mode after the smart execution time frame (currently 8 hrs). In that case, subsequent messages must wait for the first message to be processed to maintain the default sequence.

1. If the maximum gas limit is 2M (2,000,000) on mainnet, but it turns out that the destination blockchain requires more than that, will an override of > 2M work even though the max gas limit is 2M?
...
1. Will Chainlink Labs reimburse us for manual execution fees?
   Since most manual execution situations are due to insufficient gas limit or an unhandled exception in the receiver contract, Chainlink Labs does not reimburse these fees. If you are a dApp developer, please ensure you test thoroughly to avoid manual execs to the extent possible.
...
1. My transaction meets one of the conditions to trigger manual execution, but I do not see this option on the CCIP explorer. How am I supposed to execute this manually?
   This should not happen, but in the unlikely scenario that this does happen, please reach out to Chainlink Labs.
