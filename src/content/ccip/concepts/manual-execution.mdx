...
In general, messages are successfully delivered and processed via CCIP, as detailed in the [Architecture](/ccip/architecture) page. As described in the [CCIP Execution latency](/ccip/architecture#ccip-execution-latency) section, CCIP has an inbuilt gas bumping mechanism called _Smart Execution_ to ensure users’ messages are reliably delivered to the destination blockchain.
