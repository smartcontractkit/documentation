...
When deploying a token across multiple blockchains, you choose how many decimals the token uses on each chain. These choices directly determine how token amounts behave during cross-chain transfers.
...
If you choose different decimals across chains, you are also choosing where precision is lost during transfers.

<Aside type="caution" title="Impact on Token Supply">
  When transferring from higher to lower precision chains, rounding creates small differences per transfer. Depending on
  how your pool is configured, these differences either remain in the source pool or are not represented on the
  destination chain.
</Aside>

Decimal differences affect both token amounts and how rate limits are interpreted in earlier versions.

#### v1.5.1

Token pools in **v1.5.1** introduce support for tokens with different decimal places across blockchains. However, Token Pool Rate Limits (TPRLs) are not properly enforced when decimals differ between chains.

If you use mixed-decimal tokens in this version, you must manually account for how decimals affect TPRL configuration—and even then, enforcement may be significantly higher or lower than intended.
