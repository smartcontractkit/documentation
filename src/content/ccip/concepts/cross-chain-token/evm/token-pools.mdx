...
#### destGasOverhead and transfer fees

Some token transfers require more gas on the destination chain than the default allows—for example, when the receiver is a new account that must be created, or when custom pool logic exceeds the default execution budget.

**Best practices:**

- Test that token transfers **execute automatically** on every configured lane before going to production.
- Ensure `destGasOverhead` (configured via the FeeQuoter) provides sufficient buffer for first-time receivers.
- Use consistent `destGasOverhead` values across lanes pointing to the same destination unless you have a documented reason for differences.
- Review [network-specific guides](/ccip/tools-resources/network-specific) for chains with non-default gas behavior.

If transfers fail automatic execution, [contact Chainlink Labs](https://chain.link/ccip-contact?v=Tokens:%20Gas%20limit%20update) to review `destGasOverhead` configuration.

See [destGasOverhead Configuration](/ccip/concepts/cross-chain-token/operational-security#destgasoverhead-configuration) for the full security context.
...
For the full decimal symmetry security requirements, see [CCT Operational Security](/ccip/concepts/cross-chain-token/operational-security#decimal-symmetry).
