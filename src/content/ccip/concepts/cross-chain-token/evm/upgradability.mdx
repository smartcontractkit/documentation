...
metadata:
  ...
  excerpt: "Token pool upgrades EVM upgradability Zero-downtime upgrades CCIP v1.5.1 Pool migration In-flight messages Remote pool addresses Upgrade paths Contract upgrades Version migration"
...
It is recommended to keep token pools on the **latest available** release from [`@chainlink/contracts-ccip`](https://www.npmjs.com/package/@chainlink/contracts-ccip?activeTab=versions) when feasible. Use [`typeAndVersion()`](/ccip/concepts/rate-limit-management/contract-version-behavior#identify-your-version-with-typeandversion) on each deployed pool to confirm what you are running today.
...
Before upgrading, minimize new token transfers by setting near-zero rate limits on the existing pools. Use `setChainRateLimiterConfig` on both existing pools. On **1.6.1+** TokenPool releases, a minimal pause pattern is `capacity: 1`, `rate: 1`, `isEnabled: true`; on **1.4.0 – 1.6.0**, use at least `capacity: 2`, `rate: 1`. See [Contract Version Behavior (EVM)](/ccip/concepts/rate-limit-management/contract-version-behavior).
...
<Aside type="caution" title="Stale remote pool risk">
  Leaving old remote pool addresses configured after migration creates a security risk—compromised or deprecated pools
  may continue to process in-flight messages. Remove stale addresses as soon as all in-flight transactions settle. See
  [Stale Remote Pool Addresses](/ccip/concepts/cross-chain-token/operational-security#stale-remote-pool-addresses).
</Aside>
