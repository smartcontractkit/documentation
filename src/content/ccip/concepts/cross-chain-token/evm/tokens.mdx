...
metadata:
  ...
  excerpt: "ERC20 tokens Token requirements EVM tokens Burn and mint Lock and release Token compatibility Registration functions Transfer functions Token handling CCIP integration"
...
## Liquidity architecture complexity

Choose the simplest token handling mechanism that meets your requirements. Higher complexity increases misconfiguration and operational error risk.

| Risk tier | Pattern                                | When to use                                                            |
| --------- | -------------------------------------- | ---------------------------------------------------------------------- |
| Low       | Burn and Mint on all chains            | Default recommendation; simplest cross-chain accounting                |
| Medium    | Lock and Mint                          | Issuing chain holds canonical supply; monitor locked vs minted balance |
| High      | Lock and Unlock across multiple chains | Avoid unless required; fragmented liquidity                            |

For supply monitoring requirements on Lock and Mint setups, see [Supply Imbalance](/ccip/concepts/cross-chain-token/operational-security#supply-imbalance) and [Liquidity Management](/ccip/concepts/best-practices/evm#best-practices-for-liquidity-management).

### Burn and mint role hygiene

Grant mint and burn permissions only to:

- The active token pool on each chain
- Owner-like roles (`owner()`, `default_admin`) needed for administration

Revoke mint/burn rights from any other addresses. See [Burn/Mint Rights](/ccip/concepts/cross-chain-token/operational-security#burn-mint-rights).
