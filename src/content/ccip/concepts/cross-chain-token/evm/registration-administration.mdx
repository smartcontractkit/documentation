...
metadata:
  ...
  excerpt: "CCT registration Token administration EVM registration Administrator roles TokenAdminRegistry Token pool configuration Two-step process Role transfer Self-service registration Pool management"
...
Before enabling cross-chain transfers on a new lane, verify the following on **both** sides of the lane:

1. **Chain selector is valid**: Confirm the remote chain selector appears on the [CCIP Directory](/ccip/directory).
2. **Remote pool address matches**: Source `remotePoolAddresses` must point to the correct destination token pool.
3. **Remote token address matches**: Source `remoteTokenAddress` must match the token returned by `getToken()` on the destination pool.
4. **TokenAdminRegistry is set**: Call `setPool` on each chain so the TokenAdminRegistry maps the token to its active pool.
5. **Rate limits are enabled**: Both inbound and outbound TPRLs must be enabled on every lane. See [Configuration Checklist](/ccip/concepts/rate-limit-management/configuration-checklist).
6. **Symmetric configuration**: Apply matching `applyChainUpdates` on both pools with inbound/outbound values swapped appropriately.

<Aside type="caution" title="Cross-silo topology">
  If your token uses siloed liquidity lanes, do not connect siloed lanes to networks outside their designated hub chain.
  See [Cross-Silo Isolation](/ccip/concepts/cross-chain-token/operational-security#cross-silo-isolation).
</Aside>

### Canceling a pending administrator

If a token administrator role transfer was initiated in error, the current administrator can cancel it:

```solidity
tokenAdminRegistry.transferAdminRole(localToken, address(0));
```

This clears the pending administrator without changing the active administrator.

### Canceling a pending owner

For contracts using Ownable2Step (token, token pool, hooks, lock box), cancel an erroneous ownership transfer:
