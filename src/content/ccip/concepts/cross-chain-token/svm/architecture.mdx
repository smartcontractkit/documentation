...
1. **Token Administration**
   ...
   - **Administrator Methods:** The token administrator can register or update cross-chain support for their mint through a sequence of instructions:
     - `owner_propose_administrator`: Initiated by the mint authority to designate a token administrator.
     - `accept_admin_role_token_admin_registry`: Confirmation by the proposed administrator to accept the role.
     - `set_pool`: Links the token to a specific token pool configuration via the address lookup table.
...
To learn more , read the [Tokens compatibility requirements](/ccip/concepts/cross-chain-token/svm/tokens#compatiblity).

## Token Pool

A Token Pool is a self-contained program responsible for executing cross-chain token transfers for a specific CCT. Depending on your chosen bridging model, it manages token operations on both source and destination chains through specialized lock/burn and release/mint mechanisms.
...
- Token Pool Operations
  - When acting as a source chain pool (called by the Router in ccip_send):
    ...
    - Lock Release Token Pool: Locks tokens by transferring them from the sender ATA to the pool's ATA.
...
Either way, the Token Pool uses PDAs (program-derived addresses) as authorized signers to perform token operations. These PDAs are derived using specific seeds (like the `ccip_tokenpool_signer` seed and the mint address) to create deterministic signers that either have mint authority (for burn/mint pools) or sign transfers of locked tokens (for lock/release pools).
Token pools also implement rate limiting controls for both inbound and outbound transfers and maintain configuration state in PDAs that track remote chain information and pool settings.
For an in-depth overview of available pool programs, see the Token Pools page.
