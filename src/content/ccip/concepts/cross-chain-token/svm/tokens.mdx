...
Once the registration is complete, the CCIP Router recognizes your token and lets its administrator configure a token pool (burn-and-mint or lock-and-release).
...
**Note**: If `mint_authority` is no longer accessible, use this [contact form](https://chain.link/ccip-contact?v=Tokens:%20Token%20admin%20registration) to request assistance with registration.

## Mint Authority for Burn and Mint

In the “Burn and Mint” token handling mechanism, the same token pool type (BurnMint) is used on both source and destination blockchains. For this process to function correctly, the token pool program must hold the mint authority, which is required to mint tokens on the destination blockchain.

On SVM-based blockchains, only a single public key can serve as the mint authority. There are two approaches for managing mint authority in the BurnMint setup, with the multisig approach being strongly recommended for production environments.
...
1. Enable minting via multisig
   - The Token Pool Program will execute a CPI to the multisig, authorizing mint operations through the included PDA signer

**Note**: If your token already uses a multisig mint authority and includes the Token Pool PDA as one of the signers, you can still complete onboarding by adding a proxy message to the Token Pool Program that validates the signer logic during registration.
...
- Future modifications (such as supporting additional minters) require coordination and possibly redeployment
- Changes to mint authority may be constrained by the token pool program's capabilities

## Token Handling Mechanisms

To facilitate cross-chain token transfers, you need to choose the appropriate token handling mechanism and deploy the correct combination of token pools for the source and destination blockchains. The table below summarizes the different token handling mechanisms and the [recommended token pools](https://github.com/smartcontractkit/chainlink-ccip/tree/main/chains/solana/contracts/programs) to deploy for each scenario, ensuring a seamless token transfer process.
...
In this token handling mechanism, the same token pool type (BurnMint) is used on both source and destination blockchains, so these requirements are similar.

1. **Set the Token Pool as Mint Authority**

   - The token pool program must hold the mint authority to enable minting tokens.
   - For further details on mint authority configurations, refer to the "Mint Authority for Burn and Mint" section.
