...
Cross-Chain tokens need to adhere to specific requirements to ensure interoperability across different blockchain ecosystems. The specific requirements for each blockchain family (EVM, SVM..Etc) can be found under the Token pages (e.g., You can read [EVM token requirements](/ccip/concepts/cross-chain-token/evm/tokens)).
