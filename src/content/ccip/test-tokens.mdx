...
section: ccip
date: Last Modified
title: "Acquire CCIP Test Tokens - CCIP-BnM and CCIP-LnM Faucets"
metadata:
  description: "Learn to mint CCIP test tokens (CCIP-BnM and CCIP-LnM) for cross-chain testing. Step-by-step guide for faucet access via documentation or block explorer."
  excerpt: "CCIP test tokens, CCIP-BnM, CCIP-LnM, test token faucet, testnet tokens, burn and mint tokens, lock and mint tokens, MetaMask integration, token minting"
...
import { TestTokenInterface } from "@features/ccip/components/TestTokenInterface"
import { SolanaWalletProvider } from "@features/ccip/solana/WalletProvider"
...
You can use these interfaces to connect your wallet, select a testnet, and mint tokens to your wallet address. Alternatively, you can call these same functions in the block explorer (Read the [Mint tokens in a block explorer](#mint-tokens-in-a-block-explorer) section).

#### EVM Test Tokens

Use this interface to connect your MetaMask wallet and mint tokens on EVM chains:

<TestTokenInterface family="evm" client:visible />

#### Solana Test Tokens

Use this interface to connect your Solana wallet and mint tokens on Solana Devnet:

<SolanaWalletProvider client:only="react">
  <TestTokenInterface family="svm" client:visible />
