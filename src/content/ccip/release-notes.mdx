...
- We've simplified the message sequencing process in our CCIP message handling by removing the `strict` sequencing flag. This change affects how messages are processed when using the [extraArgs](/ccip/api-reference/client#evmextraargsv1) field in [CCIP messages](/ccip/api-reference/client#evm2anymessage).Key Changes:
