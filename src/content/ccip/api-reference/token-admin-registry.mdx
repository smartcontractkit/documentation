...
section: ccip
date: Last Modified
title: "TokenAdminRegistry API Reference"
---
...
<Aside type="note" title="Testnet Only">
  The **TokenAdminRegistry** contract is currently available only on **testnet**.
</Aside>

The [`TokenAdminRegistry`](https://github.com/smartcontractkit/ccip/blob/ccip-develop/contracts/src/v0.8/ccip/tokenAdminRegistry/TokenAdminRegistry.sol) contract stores the token pool configuration for all CCIP-enabled tokens. It operates on a self-serve basis, where tokens can be registered without intervention from the CCIP owner.
