...
Where:
    ...
    - `fee`: The total fee for processing a [CCIP message](/ccip/api-reference/evm/v1.5.1/client#evm2anymessage). **Note:** Users can call the [getFee](/ccip/api-reference/evm/v1.5.1/i-router-client#getfee) function to estimate the fee.
...
Where:
    ...
    - `gas usage`:
        ...
        - `gas limit`: This specifies the maximum amount of gas CCIP can consume to execute [ccipReceive()](/ccip/api-reference/evm/v1.5.1/ccip-receiver#ccipreceive) on the receiver contract located on the destination blockchain. Users set the gas limit in the [extra argument field](/ccip/api-reference/evm/v1.5.1/client#evmextraargsv1) of the CCIP message. **Note:** Remember to [carefully estimate the `gasLimit` that you set](/ccip/best-practices#setting-gaslimit) for your destination contract so CCIP can have enough gas to execute `ccipReceive()`. Any unspent gas from this user-set limit is not refunded.
        ...
        - `destination gas per payload`: This variable gas depends on the length of the data field in the [CCIP message](/ccip/api-reference/evm/v1.5.1/client#evm2anymessage). If there is no payload (CCIP only transfers tokens), the value is `0`.
...
For token transfers or programmable token transfers (token + data), the network fee varies based on the [token handling mechanism](/ccip/concepts/cross-chain-token/evm/token-pools#token-handling-mechanisms) and the lanes:
