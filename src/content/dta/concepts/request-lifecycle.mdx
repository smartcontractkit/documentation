...
Every subscription and redemption request submitted to the DTA Open Marketplace moves through a predictable lifecycle controlled by a state machine. However, the exact flow depends on a critical parameter set for each fund: the Net Asset Value (NAV) Time-to-Live, or NAV TTL.
...
- **Trigger:** The Fund Administrator begins processing a Pending request.
- **What it means:** The request is now locked. The DTA Open Marketplace has validated it, calculated the final share/amount based on the current NAV, and forwarded the settlement instructions to the Fund Administrator's DTA Wallet.
- **Possible Next Steps:**
...
- **Trigger:** The DTA Wallet completes the settlement and sends a success confirmation back to the DTA Open Marketplace.
...
- **Trigger:** An error occurred during the Processing state (e.g., a failed compliance check). The DTA Wallet sends a failure confirmation back to the DTA Open Marketplace.
...
1. The DTA Open Marketplace immediately checks if the last NAV update occurred within the NAV TTL window.
