...
- Deploying and owning the DTA Wallet for their fund.
- Registering their fund tokens on the DTA Open Marketplace, defining all operational rules like the payment method, processing model (navTTL), rate limits, and timezones.
...
- Registering their address on the DTA Open Marketplace.
...
- Granting the necessary permissions to the Fund Administrator's DTA Wallet, authorizing it to mint and burn tokens in response to processed requests.
- Ensuring the DTA Wallet is authorized to hold tokens within the token's compliance framework.
...
- **Accountants / NAV Calculators:** This external role is responsible for calculating the fund's Net Asset Value (NAV). This value is then brought onchain by a Chainlink Data Feed, which the DTA Open Marketplace is configured to use for all price calculations.
