...
description: "Some smart contract applications require you to maintain a certain Ethereum balance to ensure optimal operation. You can automate this process using Chainlink Automation. Use these smart contracts to monitor and top-up native token balances on your smart contracts."
...
1. The constructor requires a `keeperRegistryAddress` value and a `minWaitPeriodSeconds` value. Specify these in the **Deploy** fields in Remix. You can find these registry addresses on the Automation [Supported Networks](/chainlink-automation/supported-networks) page. For this example using Polygon Mumbai, enter the following values:
