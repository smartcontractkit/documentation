...
Before deploying the `RandomSVG` contract on a public network, an ID of a prefunded VRF subscription must be set in `./packages/hardhat/helper-hardhat-config.ts`
