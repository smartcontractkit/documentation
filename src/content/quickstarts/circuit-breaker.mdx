...
1. Deploy the `ExampleImplementation.sol` contract and enter the following input to the constructor:
   - The proxy contract address of the price feed. For example, the address for the BTC/USD price feed on Fuji is `0x31CF013A08c6Ac228C94551d535d5BAfE19c602a`. Find other price feed addresses [here](/data-feeds/price-feeds/addresses).
   - The `maxBalance` in sats. For example, `2650000000000` for a max balance of $26,500.
