...
Your _deployer account_ is the main account that you will use to deploy the lootbox contract, distribute reward tokens, pay for transaction fees, and pay for VRF costs. This is also the owner account.
...
1. [Create an allowlist for private mints](#create-an-allowlist-for-private-mints)
...
### Create an allowlist for private mints
...
## Open the lootbox and claim rewards
...
1. The rewards are ready to claim when the randomness result from VRF is ready. To check this, call the `canClaimRewards` function and pass in the opener address.
1. When the rewards are ready, call the `claimRewards` function to claim the reward, and pass in the opener address.
