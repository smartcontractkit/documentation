...
- `POLYGONSCAN_API_KEY` - API key for Polygonscan. Not required, it can be used to verify and read contracts.
...
1. Optionally, you can verify the contract. This allows you to decode the bytecode on Polygonscan.
...
1. Under your account, go to "API Keys".
1. Add a new key.
1. Copy your token and save it as the `ETHERSCAN_API_KEY` environment variable.
1. Verify the contract with `npx hardhat verify --constructor-args arguments.js $NEXT_PUBLIC_CONTRACT_ADDRESS`. (Replace `$NEXT_PUBLIC_CONTRACT_ADDRESS` with your contract address if you don't have the address in your shell environment).
