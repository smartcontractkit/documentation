---
title: "Automated Data Relay with Data Streams Push Engine"
description: "Set up and manage a self-hosted, automated relay to push offchain Data Streams to your smart contracts using Push Engine."
...
<Aside type="caution" title="Disclaimer">
  The Data Streams Push Engine and the reference consumer contract are community-built tools. The Push Engine is
...
The [Data Streams Push Engine][GITHUB_REPO], a community-led solution, provides a foundational framework for the DevOps teams and Infrastructure Managers tasked with building these systems. Push Engine is a configurable service that handles the core logic of monitoring offchain Data Streams and pushing values to your smart contracts based on predefined conditions, such as time intervals or price deviations. It also includes a web interface to manage your feeds and monitor their status.

This guide walks you through deploying a `DataStreamsFeed` consumer contract, setting up the Data Streams Push Engine, and configuring it to monitor a specific Data Stream and relay it to your onchain contract.
...
First, deploy a [`DataStreamsFeed.sol` contract](https://github.com/woogieboogie-jl/push-engine-contract/blob/main/src/feed/DataStreamsFeed.sol), which receives, verifies, and stores the data relayed by the Push Engine.
...
1.  Configure `script/DeployDataStreamsFeed.s.sol`.
    ...
    This script deploys the `DataStreamsFeed` contract, which will receive data from the Push Engine.
...
1.  Deploy the contract via the command line.
    ...
    After deployment, copy the **Deployed Contract Address** and **ABI** from the output. You will need them to configure the Push Engine.
    ...
    Before we can use the Push Engine, we need to fund this contract with `LINK`.
...
With a consumer contract onchain, you can now set up the Push Engine service to relay data to it.
...
1.  Open `config.yml` in a text editor. Update the `targetChains` section to point to the contract you deployed on **Avalanche Fuji**.
    ...
    - Each target block must keep `skipVerify: false`, `functionName: verifyAndUpdateReport`, and `functionArgs: ["rawReport", "parameterPayload"]` to ensure the Push Engine correctly prepares the transaction for onchain verification.
...
1.  Run the following command to start the Push Engine and its services in the background:
...
<ClickToZoom src="/images/quickstarts/transmitter/transmitter_home.png" alt="The Push Engine home page" />

The optional sections below provide a detailed overview of the Push Engine UI, explaining each section and its purpose.
...
- **`Gas estimation too high` error**
  ...
  - For the push engine, you can increase the `gasCap` value in your `config.yml` file. You can also update it in the Push Engine UI under the **Gas Cap** section.
...
Congratulations! 🎉 You have successfully deployed a `DataStreamsFeed` contract, set up the Data Streams Push Engine, and linked them together.
