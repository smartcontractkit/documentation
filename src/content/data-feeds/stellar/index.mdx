...
Data Feeds on Stellar use a single proxy contract that serves every feed. Consumers read feeds from the proxy by passing a 32-byte `data_id`:

- **Proxy contract**: A single proxy contract serves every feed. Consumers read feed data from the proxy by passing the `data_id` for the feed they need.
...
| `get_min_decimals(data_id)`              | The minimum decimal places the feed can be served at             |
...
| `timestamp` | The observation timestamp in Unix seconds when the answer was produced |
...
A round is available for a retention period of `3,110,400` ledgers. After this period, a round can no longer be read from the proxy.
...
All feeds are stored at `18` decimal places and are typically returned at `18` decimal places. The `decimals` parameter is required when you call `latest_round` or `get_round` on the proxy. You should pass `18` unless a feed is explicitly served at a reduced precision.

In some instances, Chainlink approves the serving of a reduced precision for certain feeds. In this case, you can pass that reduced precision as the `decimals` parameter when requesting a feed from the proxy.
