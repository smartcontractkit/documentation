...
You can read Chainlink Data Feeds on Stellar either onchain or offchain. Both methods read the same data from the cache contract, differing only in where the read happens.

### Onchain

Deploy a [Soroban contract in Rust](/data-feeds/stellar/using-data-feeds-on-chain) that calls the cache and reads a price. This method is for applications that need to read feed data directly within a smart contract.

To complete the onchain guide, you'll need:
...
- **Rust toolchain**: Install [Rust](https://www.rust-lang.org/tools/install). Run <CopyText text="cargo --version" code /> to verify your installation.
- **Testnet XLM**: Fund a Stellar Testnet account using the friendbot faucet. Testnet XLM has no real value.

### Offchain

Read a feed via [Soroban RPC simulation](/data-feeds/stellar/using-data-feeds-off-chain) using `stellar-cli` and the [Stellar JavaScript SDK](https://github.com/stellar/js-stellar-sdk). This method is for applications that need to read feed data without deploying a contract.

To complete the offchain guide, you'll need:
