...
The proxy contract exposes a `latest_round` function that takes a `data_id` and a `decimals` parameter, and returns the most recent round for that feed. You pass the `data_id` as raw hex (without the `0x` prefix), and `18` for the `decimals` parameter unless the feed is served at a reduced precision.
...
1. Run the following command, replacing `<SOURCE_ACCOUNT>` with any Stellar account public key. The proxy contract on Stellar Testnet is `CBUF6IADAWPWIDWRTHJNINKXHG2UTIQ6TU2F2HRAXWAI7OT3KJPKK6O4`, and the BTC/USD `data_id` is `01a0b4d920000332000000000000000000000000000000000000000000000000`:
   ...
   ```bash
   {"answer":"84421629086034400000000","round_id":96,"timestamp":1790274329}
   ```

   Where:
   - `answer` is the latest BTC/USD price with 18 decimal places (`84421629086034400000000` = 84421.63).
...
1. Create a `read-feed.js` file with the following contents. This script queries the proxy contract's `latest_round` function for a given `data_id`:
   ...
   The `data_id` argument is a 32-byte value, passed as a `Buffer` object. The `decimals` argument is required and requests the precision at which the answer is returned; pass `18` unless the feed is served at a reduced precision. The `latest_round` function returns a `Round` object wrapped in an `Ok` value, so you access it with `result.result.value`.
...
1. Run the script:
   ...
   Latest round: {
     round_id: '96',
     answer: '84421629086034400000000',
     timestamp: '1790274329'
   }
   ```

   Where `answer` is the latest BTC/USD price with 18 decimal places (`84421629086034400000000` = 84421.63).
