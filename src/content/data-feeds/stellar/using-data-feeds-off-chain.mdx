...
You can also read a Chainlink Data Feed offchain using the [Stellar JavaScript SDK](https://github.com/stellar/js-stellar-sdk) with a Soroban RPC query. This example uses the `queryContract` method available in SDK v17 and later.
...
1. Create a `read-feed.js` file with the following contents. This script queries the cache contract's `latest_round` function for a given `data_id`:
   ...
   async function readLatestRound() {
     const dataIdBytes = xdr.ScVal.scvVec([xdr.ScVal.scvBytes(Buffer.from(dataId, "hex"))])
     const result = await server.queryContract(cache, "latest_round", { data_ids: dataIdBytes })
     console.log("Latest round:", JSON.stringify(result, null, 2))
   }

   readLatestRound().catch(console.error)
   ```

   {/* TODO: Confirm the exact queryContract args serialization for the Stellar JS SDK v17 before publishing. The CLI method above is verified working; the SDK API is version-specific. */}
