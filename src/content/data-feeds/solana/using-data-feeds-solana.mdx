...
The Chainlink Solana SDK v2 is now the recommended approach for reading Data Feeds onchain. SDK v2 offers improved performance with lower compute unit usage through direct account reads instead of Cross-Program Invocation (CPI). This guide demonstrates the v2 SDK pattern. If you're using the v1 SDK, see the [migration guide](#migrating-from-v1-to-v2) to upgrade.
...
1. Choose the code sample that matches your project setup:
   - **Rust (vanilla)**: Use this if you're building a native Solana program using `solana-program` primitives directly
   - **Rust with Anchor**: Use this if your project uses the Anchor framework (look for `#[program]` macros and `Cargo.toml` with `anchor-lang` dependency)

   Both samples demonstrate SDK v2, which reads data directly from the feed account without requiring a Cross-Program Invocation. The code samples have the following components:
...
<Aside type="note" title="Starter kit v2 update">

The [Solana Starter Kit](https://github.com/smartcontractkit/solana-starter-kit) is being updated to use SDK v2. Once the update is merged, the `client.js` file will no longer require passing the `chainlinkProgram` account in the transaction, as v2 uses direct account reads instead of Cross-Program Invocation.
...
<Aside type="note" title="v1 Deprecation">

v1 SDK is deprecated but remains functional. All new projects should use v2. Migrate existing projects to benefit from improved performance and lower transaction costs.
