...
<Aside type="tip" title="SVR Now Available on Base, Arbitrum, and BNB Chain">
  <p>
...
<Aside type="note" title="Talk to an expert">
  [Contact us](https://chain.link/contact?ref_id=pricefeeds) to talk to an expert about integrating Chainlink SVR with
  your applications.
</Aside>
...
Chainlink SVR is activated by default for all Chainlink Data Feeds live on certain blockchain networks. Where SVR is active by default, new and existing lending market deployments recapture oracle-related MEV (OEV) without any additional integration work.

Integrators should expect zero service disruption during activation, and no action is needed on their part.

On these networks, SVR feeds are listed as **SVR Proxy 2** on the [Feed Addresses](/data-feeds/price-feeds/addresses) page and on [data.chain.link](https://data.chain.link/). SVR Proxy and SVR Proxy 2 are functionally identical.
