...
You can decode the new price from the `forward` `callData` that calls:
...
This is an example ABI that can be used to expedite the decoding of these functions:

<CodeSample src="samples/DataFeeds/SVR/decoding-abi.json" />

Below are code samples that decode the `forward` and `transmitSecondary` functions:
...
### 4. Detect SVR-enabled feed for Aave

Check if the `to` address from the `forward` call is one of the following:

| Feed name | Address                                      |
| --------- | -------------------------------------------- |
| AAVE/USD  | `0xbd7F896e60B650C01caf2d7279a1148189A68884` |
| BTC/USD   | `0x85355da30ee4b35F4B30759Bd49a1EBE3fc41Bdb` |
| LINK/USD  | `0x76F8C9E423C228E83DCB11d17F0Bd8aEB0Ca01bb` |
