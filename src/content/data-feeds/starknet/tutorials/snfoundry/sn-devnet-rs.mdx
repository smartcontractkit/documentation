...
1. Declare and deploy the mock aggregator consumer contract to the devnet using the `deploy_aggregator_consumer` [LINK_TO_UPDATE] script. Run the following command:
...
1. Retrieve and store the latest round data from the aggregator contract using the `set_answer` [LINK_TO_UPDATE] script. Run the following command:
...
1. Read the stored `answer` value using the `read_answer` [LINK_TO_UPDATE] script. Run the following command:
