...
- **Starknet Foundry**: Install Starknet Foundry **v0.20.1**. You can check your current version by running <CopyText text="snforge --version" code/> or <CopyText text="sncast --version" code/> in your terminal and [install the required version](https://github.com/foundry-rs/starknet-foundry#installation) if necessary.

- **Scarb**: Install Scarb **v2.5.4**. You can check your current version by running <CopyText text="scarb --version" code/> in your terminal and [install the required version](https://docs.swmansion.com/scarb/download.html#install-via-installation-script) if necessary.
...
1. Declare and deploy a mock aggregator contract to the devnet using the `deploy_mock_aggregator` [LINK_TO_UPDATE] script. Run the following command:
...
1. You can set the latest round data using the `set_latest_round` [LINK_TO_UPDATE] script. Run the following command:
...
1. Read the latest round data again using the <CopyText text="make agg-read-latest-round NETWORK=devnet" code/> command to verify the updated values:
...
1. Declare and deploy both a mock aggregator and a consumer contracts to the devnet using the following command:
   ...
   This command runs both `ma-deploy` [LINK_TO_UPDATE] and `ac-deploy` [LINK_TO_UPDATE] scripts.
...
Once the contracts are deployed, experiment and interact with both contracts and the available scripts [LINK_TO_UPDATE] as needed for your projects.

You can, for instance, set the latest round data on the aggregator contract, retrieve and set the `answer` on the consumer contract, and read the `answer` from the consumer contract:
