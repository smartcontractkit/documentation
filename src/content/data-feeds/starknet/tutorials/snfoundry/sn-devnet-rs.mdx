...
This guide employs [Starknet Devnet RS](https://github.com/0xSpaceShard/starknet-devnet-rs), a local Docker-based testnet environment for Starknet. It provides code examples and scripts that enable you to compile, declare, and deploy your own aggregator and consumer contracts. You can use a set of prefunded accounts to interact with Chainlink Data Feeds on Starknet without deploying to a live network.
...
1. The Starknet devnet container includes a set of prefunded accounts. To run the scripts from this guide, add one of these accounts to a local `accounts.json` file using the following command:
...
1. Declare and deploy a mock aggregator contract to the devnet by using the [`deploy_mock_aggregator`](https://github.com/smartcontractkit/chainlink-starknet/blob/develop/examples/contracts/aggregator_consumer/scripts/src/mock_aggregator/deploy_mock_aggregator.cairo) script. Run the following command:
   ...
   In this example, the mock aggregator address is `1708487128334545444076626254389547159540422020354796381590079145465639129383` (decimal representation), equivalent to `0x3c6f82da5dbfa89ec9dbe414f33d23d1720d15568e4a880afcc9b0c3d98d127` (hexadecimal representation). You can use a decimal-to-hexadecimal [converter](https://www.rapidtables.com/convert/number/decimal-to-hex.html) to convert the address. Save this address for the next step.

1. In `/scripts/src/aggregator/`, open the [`read_latest_round`](https://github.com/smartcontractkit/chainlink-starknet/blob/develop/examples/contracts/aggregator_consumer/scripts/src/aggregator/read_latest_round.cairo) script and update the `aggregator_address` variable with your deployed mock aggregator address.

1. Read the latest round data by using the [`read_latest_round`](https://github.com/smartcontractkit/chainlink-starknet/blob/develop/examples/contracts/aggregator_consumer/scripts/src/aggregator/read_latest_round.cairo) script. Run the following command:
   ...
   By default, the data on your mock aggregator contract are all set to zeros.

1. In `/scripts/src/mock_aggregator/`, open the [`set_latest_round`](https://github.com/smartcontractkit/chainlink-starknet/blob/develop/examples/contracts/aggregator_consumer/scripts/src/mock_aggregator/set_latest_round.cairo) script and update the `mock_aggregator_address` variable with your deployed mock aggregator address.

1. Set the latest round of data to some values defined in the [`set_latest_round`](https://github.com/smartcontractkit/chainlink-starknet/blob/develop/examples/contracts/aggregator_consumer/scripts/src/mock_aggregator/set_latest_round.cairo) script. Run the following command:
...
1. Add a prefunded account to the local `accounts.json` file using the following command:

   ```bash
   make add-account
   ```

1. Declare and deploy both a mock aggregator and a consumer contracts to the devnet using the following command:

   ```bash
   make devnet-deploy
   ```

   This command runs both [`deploy_mock_aggregator`](https://github.com/smartcontractkit/chainlink-starknet/blob/develop/examples/contracts/aggregator_consumer/scripts/src/mock_aggregator/deploy_mock_aggregator.cairo) and [`deploy_aggregator_consumer`](https://github.com/smartcontractkit/chainlink-starknet/blob/develop/examples/contracts/aggregator_consumer/scripts/src/consumer/deploy_aggregator_consumer.cairo) scripts.
