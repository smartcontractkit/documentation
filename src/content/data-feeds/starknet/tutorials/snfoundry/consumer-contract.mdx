...
1. Update the `aggregator_address` variable within the `main` function with the ETH / USD Chainlink aggregator contract address on Starknet Sepolia : <CopyText text="[ADDRESS_TO_UPDATE]" code/>.

1. Run the `deploy_aggregator_consumer` script to deploy a consumer contract to Starknet Sepolia. Use the following command:
...
1. In `/scripts/src/consumer/`, open the [`set_answer.cairo`](https://github.com/smartcontractkit/chainlink-starknet/blob/develop/examples/contracts/aggregator_consumer/scripts/src/consumer/set_answer.cairo) script and update the `consumer_address` variable with your deployed consumer address.
...
1. Run the following command to update the answer:
   ...
   ```bash
   Transaction hash = 0x7f096dc41a3f93fcad2b1d82396f92472d67dee840e1a447aa3691f58589baa
   Waiting for transaction to be accepted (59 retries / 295s left until timeout)
   Result::Ok(InvokeResult { transaction_hash: 3591274435682716112592822792656399461090392655710476322066689010471752211370 })
   command: script run
   status: success
   ```

1. In `/scripts/src/consumer/`, open the [`read_answer.cairo`](https://github.com/smartcontractkit/chainlink-starknet/blob/develop/examples/contracts/aggregator_consumer/scripts/src/consumer/read_answer.cairo) script and update the `consumer_address` variable with your deployed consumer address.
