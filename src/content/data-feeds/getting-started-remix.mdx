...
Chainlink Data Feeds are the fastest way to connect your smart contracts to real-world data such as asset prices, reserve balances, and L2 sequencer health. Each feed is aggregated by many independent Chainlink node operators and published onchain through a decentralized oracle network, so no single node can manipulate the price your contract reads.

Each price feed has an onchain address and functions that enable contracts to read pricing data from that address.

In this guide you will fetch the pricing data from a price feed, for example the [BTC / USD feed](https://data.chain.link/feeds/ethereum/mainnet/btc-usd), using the [Remix IDE](https://remix.ethereum.org/). No local installation is required.
...
<Aside type="note" title="Why a proxy?">
  ...
  upgrades an aggregator, Chainlink repoints the proxy to the new implementation and your consumer contract keeps
  working unchanged. Always read from the proxy address listed on the
  [Price Feed Addresses](/data-feeds/price-feeds/addresses) page, not from the underlying aggregator.
</Aside>

## Getting the feed address

The [Price Feed Addresses](/data-feeds/price-feeds/addresses) page lists all the feeds available on each network. This guide uses the BTC / USD feed on Sepolia, with the proxy address: <CopyText text="0x1b44F3514812d835EB1BDB0acB33d3fA3351Ee43" code />
...
You can read Data Feeds directly from the feed proxy without deploying any consumer contract. This is useful for backends, bots, dashboards, and pre-trade checks. The feed proxy is a public contract and `latestRoundData()` is a `view` function, so anyone can call it directly from a script or CLI.
...
- The `getChainlinkDataFeedLatestAnswer()` function
  - Calls `latestRoundData()` on the `dataFeed` object and returns the `answer` variable.
  - The full response from `latestRoundData()` includes `roundId`, `startedAt`, `updatedAt`, and `answeredInRound`. See the [API Reference](/data-feeds/api-reference) for details.
...
1. Open the **Deploy & Run Transactions** tab on Remix and set the **Environment** to **Browser Extension**, then select **Sepolia Testnet - MetaMask**. The contract must run in a Web3 context because it reads from another onchain contract (the price feed). Running in the "Remix VM" will not work.
...
1. In the **Contract** dropdown, select `DataConsumerV3`, then click **Deploy** to deploy the contract to Sepolia.
...
1. MetaMask prompts you to confirm the deployment transaction. Click **Confirm** to approve it. This sends an onchain transaction that deploys the contract to Sepolia.
...
1. The returned answer is an integer with no decimal point. The BTC / USD feed uses **8 decimals**, so an answer of `7836308000000` represents a BTC / USD price of `78363.08`. Each feed uses a different number of decimals. Find the exact value on the [Price Feed Addresses](/data-feeds/price-feeds/addresses) page by checking the **More Details** checkbox, or call the `decimals()` function on the feed to get the decimal count programmatically.
...
**Only if another smart contract needs the price onchain.** A consumer contract exists to wrap a feed read in your own contract's logic so that _your other contracts_ can use the price onchain: collateral checks, settlements, circuit breakers, and so on. The read is atomic with your onchain action and verifiable on the blockchain.
