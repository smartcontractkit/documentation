...
Because a proxy has references to current and all previous underlying aggregators, it needs a way to fetch data from the correct aggregator. The `roundId` is computed in the [proxy contract](https://github.com/smartcontractkit/chainlink/blob/master/contracts/src/v0.7/dev/AggregatorProxy.sol) as shown in the following example:
...
- `phaseId` is incremented each time the underlying aggregator implementation is updated. It is used as a key to find the aggregator address.
