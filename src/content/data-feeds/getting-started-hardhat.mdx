...
<Aside type="caution" title="Keep your private key safe">
  ...
  environment variables or an encrypted keystore, so no `.env` file is required. If you do use a `.env` file, add it to
  your `.gitignore`.
</Aside>
...
<Accordion title="Using a standalone ethers.js script (no Hardhat project needed)" number={1.1}>
...
<Accordion title="Using the Hardhat console (if you already have a Hardhat project set up)" number={1.2}>
...
1. Install the Chainlink contracts npm package:
   ...
   Hardhat resolves `@chainlink/contracts` from `node_modules` automatically, so no remapping file is needed (unlike Foundry).
...
The BTC / USD feed returns `8` for decimals. Scale the raw answer by `10 ** decimals` as shown in [step 1](#read-offchain).
