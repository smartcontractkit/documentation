...
### TWSE price granularity (tick size)

TWSE applies an exchange-mandated, price-tiered minimum tick size to listed equities: the minimum price increment widens in steps as the share price rises, reaching NT$0.5 or more for mid- and higher-priced instruments. As a result, TWD-denominated streams can appear to move in discrete increments rather than continuously, particularly for higher-priced names where the applicable tick is larger relative to the share price.

This quantization is a property of the underlying exchange's price formation and is expected behavior, not an indication of reduced feed precision or a data quality issue. Protocols that derive metrics such as percentage price changes or volatility bands from TWSE streams should account for this tick-size granularity when calibrating deviation thresholds, to avoid false positives triggered by normal tick-to-tick movement.
