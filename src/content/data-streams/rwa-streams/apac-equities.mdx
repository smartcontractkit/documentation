...
<Aside type="note" title="lastTradedPrice availability varies by market">
  ...
  quote-only markets, so `lastTradedPrice` is not meaningfully populated for those streams. China, Hong Kong, and
  Taiwan streams source trade data in addition to quotes.
</Aside>
