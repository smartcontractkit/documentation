...
<Aside type="caution" title="Important: Timestamp Scope">
  The `lastSeenTimestampNs` field indicates the timestamp for the `mid` price **only**. This timestamp does **not**
  apply to other fields in the report such as `bid`, `ask`, `bidVolume`, `askVolume`, or `lastTradedPrice`. Different
  fields may have been updated at different times.
</Aside>
...
Markets may stop updating due to exchange outages, circuit breakers, trading halts, or corporate actions. Use the `lastSeenTimestampNs` field (the timestamp for the `mid` price) to detect staleness by comparing it against the current time. When this timestamp stops advancing, it indicates the underlying venue has stopped providing updates.
