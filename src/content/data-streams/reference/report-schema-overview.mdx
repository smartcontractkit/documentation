...
To look up stream IDs programmatically instead of browsing the catalog, use the [Discovery endpoint](/data-streams/reference/data-streams-api/discovery-endpoint).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        <a href="/data-streams/reference/report-schema-v14">Futures (v14)</a>
      </td>
      <td>Futures price data with continuous roll price series</td>
      <td>
        <code>midPrice</code>, <code>bidPrice</code>, <code>askPrice</code>, <code>expiryTime</code>,{" "}
        <code>contractMonth</code>, <code>goldmanRollPrice</code>, <code>marketStatus</code>
      </td>
    </tr>
    <tr>
      <td>
...
[Chainlink RWA streams](/data-streams/rwa-streams) deliver real-world asset pricing through three report schemas:
...
- **[Futures (v14)](/data-streams/reference/report-schema-v14)** — futures price data with contract expiration, market status, and continuous roll price series. Used by [Futures streams](/data-streams/rwa-streams/futures).
...
The [Futures (v14)](/data-streams/reference/report-schema-v14) report schema provides price data for exchange-traded futures contracts. Each report includes consensus mid, bid, and ask prices (`midPrice`, `bidPrice`, `askPrice`), contract expiration (`expiryTime`), the first day of notice (`firstDayOfNotice`), the tracked contract month (`contractMonth`), market status (`marketStatus`), and a staleness measure (`lastSeenTimestampNs`).

Because futures contracts expire, streams using this schema expose three **generic** feeds per instrument that always track the current front, second, and third contract. The schema also includes continuous roll price series — `goldmanRollPrice`, `interpolatedGoldmanRollPrice`, and `currentBusinessDay` — for integrators who want a smoothed price instead of a hard cutover at contract expiration. See the [Futures](/data-streams/rwa-streams/futures) reference for rotation methodology and the [Futures (v14) reference](/data-streams/reference/report-schema-v14) for complete field specifications.
