...
- **RWA (v8)** uses [Report Schema v8](/data-streams/reference/report-schema-v8) — standard RWA format with consolidated mid price, market status, and freshness indicators. Used by the majority of RWA streams.
- **RWA+ (v11)** uses [Report Schema v11](/data-streams/reference/report-schema-v11) — adds bid/ask prices, volume depth, last traded price, nanosecond-precision timestamps, and expanded market status values. Supports extended trading sessions where applicable.

### RWA (v8)

[Report Schema v8](/data-streams/reference/report-schema-v8) provides real-world asset pricing with essential fields: consensus median price (`midPrice`), market status (`marketStatus`), and staleness measure (`lastUpdateTimestamp`).
