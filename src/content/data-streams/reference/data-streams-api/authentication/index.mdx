...
All requests to the Data Streams API (both REST and WebSocket) require three authentication headers, with one exception: the [Discovery endpoint](/data-streams/reference/data-streams-api/discovery-endpoint) does not require authentication to list publicly available streams. Authenticate to see the [hidden streams](/data-streams/reference/data-streams-api/discovery-endpoint#hidden-streams) your API key is entitled to.
