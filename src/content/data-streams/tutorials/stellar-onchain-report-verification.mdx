...
1. Open `contracts/consumer_example/Cargo.toml` and update it to match the following. Lines highlighted in green are the changes from the generated default:
   ...
   />
   - **Line 2**: Rename the package from `hello-world` to `consumer_example`. - **Line 3**: Update the version to
   `0.1.0`. - **Line 8**: Remove `"lib"` from `crate-type`, keeping only `"cdylib"`. This ensures the output is a
