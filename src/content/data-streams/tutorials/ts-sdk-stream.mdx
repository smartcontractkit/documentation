...
1.  Subscribe to a [testnet crypto stream](/data-streams/crypto-streams?networkType=testnet). The below example executes the application, subscribing to the `ETH/USD` crypto stream:
...
The `fullReport` field in each streamed report is the signed payload you pass directly to `VerifierProxy.verify()` (or `verifyBulk()`) for onchain verification. No transformation or re-encoding is needed — the raw bytes from the stream are the input.

From the example output above, the `fullReport` for Report #1 looks like this:

```

```

This payload is ABI-encoded and contains the report header, the DON-signed report body, and the signatures used for onchain verification. You do not need to decode or inspect it before passing it to the Verifier — the `VerifierProxy` handles all of that.

In TypeScript, `report.fullReport` holds this value and can be passed directly to your verification contract:

```typescript
// report.fullReport is the value to pass as `payload` to VerifierProxy.verify()
const decodedData = decodeReport(report.fullReport, report.feedID)
```

Refer to the [Verify report data onchain](/data-streams/tutorials/evm-onchain-report-verification) tutorial for a step-by-step walkthrough of the full onchain verification flow.
