...
In this tutorial, you'll learn how to use Chainlink Data Streams with the _[Streams Direct](/data-streams#streams-direct-using-the-data-engine-with-your-own-bot) implementation_ and a [WebSocket connection](/data-streams/reference/streams-direct-interface-ws). You'll set up your Go project, listen for real-time reports from the Data Engine, decode the report data, and log their attributes to your terminal.
...
1.  Insert the provided code into `client-ws.go` and `decoder.go` to allow your application to establish a WebSocket connection, listen for new reports, and decode them:
...
1.  Create `wsConnectAndListen.go` at the root of your project directory:

    ```bash
    touch wsConnectAndListen.go
    ```

    Then, open `wsConnectAndListen.go` and insert the following code to establish a WebSocket connection and listen for real-time data reports from the Data Engine:

    <CodeSample src="samples/DataStreams/StreamsDirect/ws/wsConnectAndListen.go" />

1.  Launch the WebSocket listener by running the following command in your terminal:
...
1.  - Open `decodeFullReportHex.go` and insert the sample code provided below. It uses the `DecodeFullReportAndReportData` function from the `internal` package to decode the `FullReport` part of the message.
...
1.  Run the decoding script by executing the following command in your terminal:
    ...
    The `decodedReport` contains structured data about the report, including the `FeedId`, `FeedVersion`, and specific report data based on the report version (e.g., `V2Report` for [Basic Schema](/data-streams/reference/report-schema#basic-schema) reports and `V3Report` for [Premium Schema](/data-streams/reference/report-schema#premium-schema) reports).
...
| ---------------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Feed ID                | `0x00027bbaff688c906a3e20a34fe951715d1018d262a5b66e38eda027a674cd1b` | The unique identifier for the Data Streams feed. In this example, the feed is ETH/USD on Arbitrum Sepolia with the Basic Report schema.                                                            |
| Feed Version           | `2`                                                                  | The feed report schema version.                                                                                                                                                                    |
| Valid From Timestamp   | `1707986325`                                                         | The start validity timestamp for the report, indicating when the data becomes relevant.                                                                                                            |
| Observations Timestamp | `1707986325`                                                         | The timestamp indicating when the data was captured.                                                                                                                                               |
| Native Fee             | `35965683271400`                                                     | The fee to pay in the native blockchain token (e.g., testnet ETH on Arbitrum Sepolia) for the onchain verification of the report data. With 18 decimals. For readability: `0.035965683271400` ETH. |
| Link Fee               | `4988454836662200`                                                   | The fee to pay in LINK tokens for the onchain verification of the report data. With 18 decimals. For readability: `4.4988454836662200` LINK.                                                       |
| Expires At             | `1708072725`                                                         | The expiration timestamp of the report, indicating the point at which the data becomes outdated.                                                                                                   |
| Benchmark Price        | `2780428200000000000000`                                             | The observed price in the report, with 18 decimals. For readability: `2,780.428200000000000000` ETH per USD.                                                                                       |
...
1. Report data decoding: Depending on the report version (`V1Report`, `V2Report`, or `V3Report`) extracted from the `feedID`, the function proceeds to decode the report data into a structured format.
