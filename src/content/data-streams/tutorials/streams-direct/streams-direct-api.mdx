...
Set the required environment variables in your terminal session to authenticate with the Data Streams Aggregation Network API:
    ...
    - `BASE_URL` is the REST endpoint to poll for specific reports. See the [Streams Direct Interface](/data-streams/reference/streams-direct/streams-direct-interface-api#domains) guide for more information.
...
1.  Ensure the required environment variables are still defined in your terminal session.
    ...
    Otherwise, [set them](/data-streams/tutorials/streams-direct/streams-direct-api#set-environment-variables) again.
