...
1. Insert the following code example in your `stream-feeds.go` file:
...
1. Set up the SDK client configuration within `stream-feeds.go`:
...
1. For this example, you'll subscribe to the ETH/USD Data Streams feed. This feed ID is <CopyText text="0x000359843a543ee2fe414dc14c7e7920ef10f4372990b79d6361cdc0dd1ba782" code/>. See the [Data Streams Feed IDs](/data-streams/stream-ids) page for a complete list of available assets.
...
- Reading streams: The [`Read`](https://github.com/smartcontractkit/data-streams-sdk/blob/07b90307840f9e0f5f86644cc538d2e81ae870dc/go/stream.go#L266) method on the returned Stream object is continuously called within a loop. This method blocks until new data is available, ensuring that all incoming reports are captured as soon as they are broadcasted.

- Decoding reports: For each received report, the SDK's [`Decode`](https://github.com/smartcontractkit/data-streams-sdk/blob/07b90307840f9e0f5f86644cc538d2e81ae870dc/go/report/report.go#L26) function parses and transforms the raw data into a structured format (`v3.Data`). This decoded data includes data such as the benchmark price and [bid and ask](/data-streams/concepts/liquidity-weighted-prices) prices.
