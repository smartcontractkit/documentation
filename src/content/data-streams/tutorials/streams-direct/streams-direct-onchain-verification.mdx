...
1. In the **Contract** section, select the `ClientReportsVerifier` contract and fill in the Arbitrum Sepolia **verifier proxy address**: <CopyText text="0x2ff010DEbC1297f19579B4246cad07bd24F2488A" code/>. You can find the verifier proxy addresses on the [Streams Addresses](/data-streams/crypto-streams) page.
...
When deploying the contract, you define the verifier proxy address for the stream you want to read from. You can find this address on the [Streams Addresses](/data-streams/crypto-streams) page. The verifier proxy address provides functions that are required for this example:
