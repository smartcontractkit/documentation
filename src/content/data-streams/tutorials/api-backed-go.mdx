...
metadata:
  title: "Fetch and Decode Backed Tokenized Asset Data with Go SDK | Chainlink Data Streams"
  description: "Learn how to use the Go SDK to fetch and decode Backed Tokenized Asset market data reports with V10 schema from Chainlink Data Streams."
  keywords: ["Go SDK", "Golang", "Backed Tokenized Assets", "V10 Reports", "API Tutorial", "Data Streams"]
whatsnext:
  {
...
In this tutorial, you'll learn how to use the [Data Streams SDK](/data-streams/reference/go-sdk) for Go to fetch and decode [V10 reports](/data-streams/reference/report-schema-v10) for [Backed Tokenized Asset streams](/data-streams/backed-streams) from the Data Streams Aggregation Network. You'll set up your Go project, retrieve reports, decode them, and log their attributes.
...
You'll start with the set up of your Go project. Next, you'll fetch and decode reports for both single and multiple [Backed Tokenized Asset streams](/data-streams/backed-streams), and log their attributes to your terminal.
...
import (
   ...
   v10 "github.com/smartcontractkit/data-streams-sdk/go/report/v10" // Import the v10 report schema for Backed Tokenized Asset streams.
)
...
1. For this example, you will read from a Backed Tokenized Asset stream. See the [Data Streams Backed streams](/data-streams/backed-streams) page for a complete list of available streams.
...
| `marketStatus`          | `uint32`  | Status of the real-world equity market. Possible values: `0` (Unknown), `1` (Closed), `2` (Open), `3` (Halted) |
...
| `activationDateTime`    | `uint32`  | When the next corporate action takes effect (set to `0` if none is scheduled)                                  |
...
1. For this example, you will read from two Backed Tokenized Asset streams. Run your application. Replace `[STREAM_ID_1]` and `[STREAM_ID_2]` with your stream IDs.
   ...
   ------------------------------------------
   US Equity Price         : 123456
   Market Status           : OPEN
   Staleness Timestamp     : 1753551893
   Tokenised Price         : 123456
   Current Multiplier      : 1.0
   Reason                  : dividend
   Activation Timestamp    : 1753551893
   New Multiplier          : 1.1
   ------------------------------------------
   2024-12-14T17:53:30-05:00 Raw report data for Stream ID [STREAM_ID_2]: {"fullReport":"0x...","feedID":"[STREAM_ID_2]","usEquityPrice":123456,"tokenisedPrice":123456,"currentMultiplier":1.0,"marketStatus":"OPEN","stalenessTimestamp":1753551893,"reason":"dividend","activationTimestamp":1753551893,"newMultiplier":1.1}
...
1. Report decoding with [`report.Decode`](https://github.com/smartcontractkit/data-streams-sdk/blob/main/go/report/report.go#L30):
   ...
   - Uses `v10.Data` schema for Backed Tokenized Asset streams (use `v3.Data` for Crypto streams)
