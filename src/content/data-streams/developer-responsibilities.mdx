...
Developers should not use the report's last-seen or observation timestamp (e.g., `observationsTimestamp`) to infer whether a stream is active or to select among multiple streams. The **market status flag** included in price reports is the authoritative signal for determining whether a given market is open. Relying on timestamps for stream selection may produce incorrect behavior during periods of low activity, market closures, or delayed openings.
...
- **Determining stream applicability:** Do not rely on the last-seen timestamp (e.g., `observationsTimestamp` or similar report-level timestamps) to determine which price stream to use or whether a stream is currently valid. Timestamps indicate when an observation was last recorded, not whether the market is currently open or the stream is actively priced. Use the **market status flag** included in price reports to programmatically determine market availability and decide which stream to consume.
...
- **Implementing Contingency Logic:** In extreme circumstances, including situations beyond the control of Chainlink node operators, Chainlink Data Streams may experience periods of unavailability or performance degradation. When a WebSocket connection is dropped, user systems must manage reconnections effectively. REST requests can also fail transiently, including during routine maintenance rotations across the active-active deployment, and must be retried rather than treated as a hard failure — see [Retry Guidance for REST Requests](/data-streams/architecture#retry-guidance-for-rest-requests). Developers are responsible for creating contingency plans tailored to their specific application needs, such as:
  ...
  - Implementing retry logic with exponential backoff for REST requests, per the [Retry Guidance](/data-streams/architecture#retry-guidance-for-rest-requests),
