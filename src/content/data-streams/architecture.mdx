...
Streams Direct facilitates seamless, offchain price updates through a robust mechanism designed for real-time data delivery. Here is an example of how your Client will benefit from low-latency market data directly from the Data Streams Aggregation Network.
...
1. The Data Streams Aggregation Network sends price reports via WebSocket, giving the Client instant access to updated market data.
...
1. If the Data Streams Aggregation Network identifies missed reports, it sends an array of these reports back. This mechanism allows the Client to update its cache and to keep the data set complete and up-to-date.
