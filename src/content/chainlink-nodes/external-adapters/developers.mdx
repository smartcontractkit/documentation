...
The [**Official Chainlink External Adapter Monorepo (NodeJS)**](https://github.com/smartcontractkit/external-adapters-js) contains many examples to help get you started.
...
Additional data may be specified in the spec to be used by the adapter. This can be useful for requesting data from a REST endpoint where the keys and values can be specified by the requester. For example, if the REST endpoint supports the following:
