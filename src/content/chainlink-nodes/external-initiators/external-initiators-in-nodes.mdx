...
<a href="https://github.com/smartcontractkit/chainlink/wiki/External-Initiators">
  Additional external initiator reference
</a>

<Aside type="note">
  The External Initiator can only initiate [webhook jobs](/chainlink-nodes/oracle-jobs/all-jobs#webhook-jobs) that have
  been linked to it. Trying to initiate a job that is not linked will give an unauthorised error.
</Aside>
