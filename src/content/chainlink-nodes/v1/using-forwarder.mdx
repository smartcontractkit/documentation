...
<Aside type="note">
  ...
  request](/chainlink-nodes/oracle-jobs/all-jobs#direct-request-jobs) jobs. You can use the same strategy for different
  job types, VRF and OCR. Supporting different job types on the same Chainlink node reduces your infrastructure and
  maintenance costs.
</Aside>
