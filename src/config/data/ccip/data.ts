...
import verifiersDescMainnetv120 from "@config/data/ccip/v1_2_0/mainnet/verifiers-desc.json" with { type: "json" }
...
import verifiersDescTestnetv120 from "@config/data/ccip/v1_2_0/testnet/verifiers-desc.json" with { type: "json" }
...
export function getAllTokenLanes({
  ...
  for (const sourceChain in lanesReferenceData) {
    ...
    for (const destinationChain in sourceData) {
      ...
      // Check if the token is supported (supportedTokens is an object keyed by token symbol)
...
/**
 * Load verifier descriptions for a specific environment and version
 */
export const loadVerifiersDescriptionData = ({
  environment,
  version,
}: {
  environment: Environment
  ...
  let verifiersDescriptionData: VerifiersDescriptionConfig

  if (environment === Environment.Mainnet && version === Version.V1_2_0) {
    verifiersDescriptionData = verifiersDescMainnetv120 as unknown as VerifiersDescriptionConfig
  } else if (environment === Environment.Testnet && version === Version.V1_2_0) {
    verifiersDescriptionData = verifiersDescTestnetv120 as unknown as VerifiersDescriptionConfig
  } else {
    throw new Error(`Invalid environment/version combination for verifier descriptions: ${environment}/${version}`)
  }

  return { verifiersDescriptionData }
}

/**
 * Get a description for a specific verifier by ID
 */
export const getVerifierDescriptionById = ({
  id,
  environment,
  version = Version.V1_2_0,
}: {
  ...
  environment: Environment
  version?: Version
}): VerifierDescription | undefined => {
  const { verifiersDescriptionData } = loadVerifiersDescriptionData({ environment, version })
  return verifiersDescriptionData[id]
}
...
export const getVerifier = ({
  ...
  const exactMetadata = addressMap[address]
  const matchingAddress =
    exactMetadata !== undefined
      ? address
      : Object.keys(addressMap).find((candidate) => candidate.toLowerCase() === address.toLowerCase())

  const metadata = exactMetadata ?? (matchingAddress ? addressMap[matchingAddress] : undefined)
  if (!metadata) {
    return undefined
  }
