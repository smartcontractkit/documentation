...
func main() {
    ...
    if decodedReport.FeedVersion == 2 && decodedReport.V2Report != nil {
        fmt.Printf("Valid From Timestamp: %d\n", decodedReport.V2Report.ValidFromTimestamp)
        fmt.Printf("Observations Timestamp: %d\n", decodedReport.V2Report.ObservationsTimestamp)
        fmt.Printf("Native Fee: %s\n", decodedReport.V2Report.NativeFee.String())
        fmt.Printf("Link Fee: %s\n", decodedReport.V2Report.LinkFee.String())
        fmt.Printf("Expires At: %d\n", decodedReport.V2Report.ExpiresAt)
        fmt.Printf("Benchmark Price: %s\n", decodedReport.V2Report.BenchmarkPrice.String())
    } 
}