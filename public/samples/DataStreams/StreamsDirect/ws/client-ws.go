...
var baseUrl = os.Getenv("BASE_URL") //Example: https://ws.testnet-dataengine.chain.link
...
func ConnectAndListen(ctx context.Context) error {
	conn, err := openWebsocketConnection(ctx)
	if err != nil {
		return err
	}
	defer conn.Close()
...
func openWebsocketConnection(ctx context.Context) (*websocket.Conn, error) {
