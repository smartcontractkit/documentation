...
contract StreamsUpkeepWithErrorHandler is ILogAutomation, StreamsLookupCompatibleInterface {
  ...
  event ErrorTestLog(uint256 indexed errorCode);
  ...
  /**
   * @dev This function is intended for off-chain simulation by Chainlink Automation to pass in the data reports fetched
   * from Data Streams.
   ...
   * @return performData Encoded data indicating success and including the original `values` and `extraData`, to be used
   * in `performUpkeep`.
   */
   ...
   * @dev All error codes are documented at:
   * https://docs.chain.link/chainlink-automation/guides/streams-lookup-error-handler#error-codes
  ...
  function checkErrorHandler(
    ...
    if (errorCode == 0) {
      ...
      _upkeepNeeded = true;
      reportSuccess = true;
    } else if (errorCode == 808_400 || errorCode == 808_401) {
  ...
  function performUpkeep(
    ...
    if (reportSuccess) {
      ...
      (uint256 errorCode, bytes memory extraData) = abi.decode(payload, (uint256, bytes));
