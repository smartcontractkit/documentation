...
contract VRFCoordinatorV2_5Mock is SubscriptionAPI, IVRFCoordinatorV2Plus {
    ...
    function fulfillRandomWordsWithOverride(
        ...
        bool nativePayment = uint8(req.extraArgs[req.extraArgs.length - 1]) ==
            1;

        uint256 rawPayment = i_base_fee +
            ((startGas - gasleft()) * i_gas_price);
        if (!nativePayment) {
    ...
    function _fromBytes(
        ...
        if (bytes4(extraArgs) != VRFV2PlusClient.EXTRA_ARGS_V1_TAG)
        ...
        returns (uint256)
    {
        ...
        bytes memory extraArgsBytes = VRFV2PlusClient._argsToBytes(
            _fromBytes(_req.extraArgs)
        );
