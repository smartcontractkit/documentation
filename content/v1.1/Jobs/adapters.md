...
updatedAt: "2020-10-14T21:26:10.313Z"
---
...
[/block]
The adapter walks the `copyPath` specified and returns the value found at that result. If returning JSON data from an [external adapter](doc:external-adapters), you will need to use this adapter to parse the response.
...
[/block]
The adapter walks the `path` specified and returns the value found at that result. If returning JSON data from the [HttpGet](doc:adapters#section-httpget) or [HttpPost](doc:adapters#section-httppost) adapters, you must use this adapter to parse the response.
