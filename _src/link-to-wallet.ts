...
const addChainToWallet = async (chainId: string, ethereum: MetaMaskInpageProvider) => {
  ...
  const [signerAddress] = (await ethereum.request({ method: 'eth_requestAccounts' })) as string[];
  await ethereum.request({
...
window.onload = async () => {
  ...
  tokenAddressElements.forEach((element) => {
    ...
    button.className = 'cl-button--ghost cl-button--ghost-sm';
    button.style.marginLeft = '10px';
    const parameters: AddToWalletParameters = { ...defaultWalletParameters, address };
