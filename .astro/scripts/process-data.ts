  ...
  for (const network of page.networks) {
      ...
      if (
        ...
        if (
          ...
          (threshold = Number.parseInt(
            contract.config.relativeDeviationThresholdPPB
              ? contract.config.relativeDeviationThresholdPPB
              : "",
            10
          ))
        ...
        liveContracts[contractKey] = {
          ...
          heartbeat: contract.heartbeat
            ? contract.heartbeat
            : contract.config?.maxContractValueAge || "",
    ...
    if (contents.proxies) {
      for (const proxyKey of Object.keys(contents.proxies)) {
        ...
        if (
          liveContracts[proxy.aggregator] &&
          !proxy.name.includes("Healthcheck")
        ) {
          proxyList.push({
            pair: proxy.name,
            assetName: liveContracts[proxy.aggregator].assetName || "",
            deviationThreshold:
              liveContracts[proxy.aggregator].deviationThreshold,
