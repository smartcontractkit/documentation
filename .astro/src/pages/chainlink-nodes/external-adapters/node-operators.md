...
You can add external adapters to a Chainlink node by creating a bridge in the Node Operators Interface. Each bridge must have a unique name and a URL for the external adapter. If a job has a [Bridge Task](/chainlink-nodes/oracle-jobs/jobs/task-types/bridge/), the node searches for a bridge by name and uses that bridge as your external adapter. Bridge names are case insensitive.
...
To add jobs that use the bridge, add a [Bridge Task](/chainlink-nodes/oracle-jobs/jobs/task-types/bridge/) to your job. The `bridge` task defined in the example below is defined as `fetch` and the name of the bridge is `soccer-data`.
...
The easiest way to test if your external adapter is working is to use a [Webhook Job](/chainlink-nodes/oracle-jobs/jobs/job-types/webhook/).
...
The easiest way is to setup a [Webhook Job](/chainlink-nodes/oracle-jobs/jobs/job-types/webhook/) that uses the external adapter, and manually set the parameter.
