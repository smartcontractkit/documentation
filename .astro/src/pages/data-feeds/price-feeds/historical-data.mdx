...
To learn how data feeds update, see the [Decentralized Data Model](/docs/architecture-decentralized-model/#aggregator) page.
...
As show in the following sequence diagram, to get historical data, call the `getRoundData` [function](/docs/data-feeds/price-feeds/api-reference/#getrounddata) function and provide `roundId` as a parameter.
...
When you query historical data, it is important to know when you reach the end of the history of the underlying aggregator. As an example, if the `latestRoundData` [function](/docs/data-feeds/price-feeds/api-reference/#latestrounddata) of the [LINK / USD feed on Ethereum Mainnet](/docs/data-feeds/price-feeds/addresses/?network=ethereum) returns `roundId = 92233720368547771158`, you can use this value to compute the `phaseId` and `aggregatorRoundId`:
...
Using _13078_ as the current aggregator's round, get its historical data by looping over the [`getRoundData` function](/docs/data-feeds/price-feeds/api-reference/#getrounddata):
...
Loop over the [`getRoundData` function](/docs/data-feeds/price-feeds/api-reference/#getrounddata). Start at _73786976294838206465_ and increment it until you get a revert. This means that you reached the last round for the underlying aggregator. The same process could be repeated for previous `phaseIds` (3,2,1).
...
The [`getRoundData` function](/docs/data-feeds/price-feeds/api-reference/#getrounddata) returns the following values:
